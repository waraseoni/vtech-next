...
export async function getGroqChatResponse(
  ...
  try {
    ...
    const errorMessage =
      (error instanceof Error ? error.message : String(error)) || JSON.stringify(error);
          ...
          if (nameMatch && argsMatch) {
            ...
            if (fallbackHistory.length > 0 && firstMsg && firstMsg.role === "system") {
              fallbackHistory[0] = {
                role: "system",
                content:
                  "You are the V-Tech Assistant. You must answer the user's prompt based ONLY on the data in the System Alert below in conversational language. DO NOT request more data. DO NOT attempt to use tools. NO <function> tags.",
              };
            }
