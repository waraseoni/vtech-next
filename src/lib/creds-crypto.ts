...
function getKey(): Buffer {
  const secret =
    process.env.SELLER_CREDS_ENCRYPTION_KEY || process.env.LICENSE_SERVICE_SERVICE_ROLE_KEY || "";
  if (!secret) {
    throw new Error(
      "SELLER_CREDS_ENCRYPTION_KEY (ya LICENSE_SERVICE_SERVICE_ROLE_KEY) set nahi hai — credentials encrypt/decrypt nahi ho sakte"
    );
  }
