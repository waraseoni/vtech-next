...
export const geminiTools: AiTool[] = [
  {
    ...
    description:
      "Fetches a quick high-level summary of the business including total clients, total jobs, and total revenue.",
    parameters: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      "Fetches the most recent jobs/transactions. Can filter by status (0=Pending, 1=In Progress, 2=Done, 3=Paid, 4=Cancelled, 5=Delivered).",
    parameters: {
      type: "object",
      properties: {
        limit: {
  ...
  },
  {
    ...
    description:
      "Returns statistics about service/repair jobs within a date range. For a single day, use same date for both start and end.",
    parameters: {
      type: "object",
      properties: {
        start_date: {
          type: "string",
          description: "Start date (YYYY-MM-DD). For single day, same as end_date.",
        },
        end_date: {
          type: "string",
          description: "End date (YYYY-MM-DD). For single day, same as start_date.",
        },
      },
  ...
  },
  {
    ...
    description:
      "Searches the database for a customer by their name and returns their details like balance, phone, address, and recent jobs.",
    parameters: {
      type: "object",
      properties: {
        customer_name: {
  ...
  },
  {
    ...
    description:
      "Searches the database for a specific job/transaction by its Exact Job ID and returns its full details including status, client name, item, fault, and amount.",
    parameters: {
      type: "object",
      properties: {
        job_id: {
  ...
  },
  {
    ...
    description:
      "Fetches a comprehensive financial report for a specific date range. Includes Total Revenue (Repairs + Direct Sales), Total Cash In, Expenses, and Net Profit. For a single day, use the same date for both 'from' and 'to'.",
    parameters: {
      type: "object",
      properties: {
        from: {
          type: "string",
          description:
            "Start date (YYYY-MM-DD). If user asks for a single specific day (e.g. '23 March'), use that same date for both 'from' and 'to'.",
        },
        to: {
          type: "string",
          description:
            "End date (YYYY-MM-DD). If user asks for a single specific day, MUST be same as 'from'.",
        },
      },
      required: ["from", "to"],
    },
  ...
  },
  {
    ...
    description:
      "Returns current stock levels for all products with low-stock flags (quantity vs alert_quantity). Use for inventory, stock, kaunsi cheez khatam hone wali hai.",
    parameters: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      "Returns clients with positive net balance, computed exactly like the Clients page: balance = opening_balance + delivered repairs (status 5) + direct sales billed + loans given - total payments (incl. discounts). Sorted by highest balance. Use for pending balance, kis customer se kitna paisa baaki hai, dues.",
    parameters: {
      type: "object",
      properties: {
        limit: {
  ...
  },
  {
    ...
    description:
      "Fetches expenses grouped by category for a date range with total. For a single day, use the same date for both 'from' and 'to'. Admin only.",
    parameters: {
      type: "object",
      properties: {
        from: {
  ...
  },
  {
    ...
    description:
      "Fetches attendance summary (present/half-day days) per mechanic for a date range. For a single day, use the same date for both 'from' and 'to'.",
    parameters: {
      type: "object",
      properties: {
        from: {
  ...
  },
  {
    ...
    description:
      "Returns lender loans (remaining amounts) and active client loans with EMI. Admin only.",
    parameters: {
      type: "object",
      properties: {},
      required: [],
...
export async function executeGeminiTool(
  ...
  if (ADMIN_ONLY_TOOLS.has(name) && role !== "admin") {
    return {
      error: `'${name}' sirf Admin ke liye hai. Staff ise nahi dekh sakta. Ye jaankari lene ke liye Admin se baat karein.`,
    };
  }
  ...
  try {
    if (name === "get_business_summary") {
      ...
      const { data: revenueData } = await pageAll(
        supabase.from("transaction_list").select("amount").in("status", [3, 5]).eq("del_status", 0)
      );
      const totalRevenue =
        revenueData?.reduce((sum, job) => sum + (Number(job.amount) || 0), 0) || 0;
    ...
    if (name === "get_job_statistics") {
      ...
      const { data: revData } = await pageAll(
        supabase
          .from("transaction_list")
    ...
    if (name === "get_financial_report") {
      ...
      const [repairRes, directRes, paymentRes, expenseRes, loanRes, attendanceRes, mechanicRes] =
        await Promise.all([
          pageAll(
            supabase
              .from("transaction_list")
      ...
      const commissions = (repairRes.data || []).reduce(
        (sum, j) => sum + (Number(j.mechanic_commission_amount) || 0),
        0
      );
      const shopExpenses = (expenseRes.data || []).reduce(
        (sum, e) => sum + (Number(e.amount) || 0),
        0
      );
      const emiPayments = (loanRes.data || []).reduce(
        (sum, l) => sum + (Number(l.amount_paid) || 0),
        0
      );
      const totalDiscounts = (paymentRes.data || []).reduce(
        (sum, p) => sum + (Number(p.discount) || 0),
        0
      );
      const totalBusinessExpenses =
        salaries + commissions + shopExpenses + emiPayments + totalDiscounts;
      ...
      const paymentsReceived = (paymentRes.data || []).reduce(
    ...
    if (name === "get_customer_details_by_name") {
      ...
      const tokens = customerName
        ...
        .split(/[\s,._-]+/)
        .map(sanitize)
        .filter((t: string) => t && !STOP_WORDS.has(t));
      ...
      if (tokens.length === 1) {
        ...
        const { data } = await pageAll(
          supabase.from("client_list").select("*").eq("delete_flag", 0).or(conds)
        );
        customers = data;
      } else if (tokens.length > 1) {
        // Multi-word â†’ fetch active clients and score token matches in JS (small dataset)
        const { data: all } = await pageAll(
          supabase.from("client_list").select("*").eq("delete_flag", 0)
        );
      ...
      if (!customers || customers.length === 0) {
        ...
        const { data: jobsObj } = await supabase
          .from("transaction_list")
          .select("job_id, client_name, item, fault, status, amount, date_created")
          .eq("del_status", 0)
          .or(ors)
        ...
        return {
          result: `Customer not found in client list, but found these recent jobs matching '${customerName}'`,
          recent_jobs: jobsObj,
        };
      }
    ...
    if (name === "get_job_details_by_id") {
      ...
      if (isNumeric) {
        ...
        query = query
          .or(`item.ilike.%${jobId}%,fault.ilike.%${jobId}%`)
          .order("date_created", { ascending: false })
          .limit(5);
      }
      ...
      const jobClientIds = [
        ...new Set(jobRows.map((j) => j.client_name).filter(Boolean)),
      ] as string[];
    ...
    if (name === "get_business_alerts") {
      ...
      const [invRes, prodRes] = await Promise.all([
        ...
        pageAll(
          supabase.from("product_list").select("id, name, alert_quantity").eq("delete_flag", 0)
        ),
      ]);
      const qty: Record<number, number> = {};
      (invRes.data || []).forEach((i) => {
        qty[i.product_id] = (qty[i.product_id] || 0) + (Number(i.quantity) || 0);
      });
      const lowStock = (prodRes.data || [])
        .filter(
          (p) => (Number(p.alert_quantity) || 0) > 0 && (qty[p.id] || 0) <= Number(p.alert_quantity)
        )
        .map((p) => ({
          product_id: p.id,
          name: p.name,
          quantity: qty[p.id] || 0,
          alert_quantity: Number(p.alert_quantity),
        }));
      if (lowStock.length)
        alerts.push({
          type: "low_stock",
          severity: "warning",
          title: "Low Stock â€” Reorder Needed",
          items: lowStock,
        });

      // 2. Pending jobs older than 2 days
      const { data: pendJobs } = await pageAll(
        supabase
          .from("transaction_list")
      ...
      const pending = (pendJobs || [])
        .map((j) => ({
          job_id: j.job_id,
          item: j.item,
          status_label: STATUS_MAP[j.status] || "Unknown",
          days_pending: Math.max(
            0,
            Math.floor((Date.now() - new Date(j.date_created).getTime()) / 86400000)
          ),
        }))
        .filter((j) => j.days_pending >= 2)
        .slice(0, 10);
      if (pending.length)
        alerts.push({
          type: "pending_jobs",
          severity: "info",
          title: "Pending Jobs (2+ days old)",
          items: pending,
        });

      // 3. Clients whose payment_due_date crossed â€” only if balance is still outstanding
      const { data: dueClients } = await pageAll(
        supabase
          .from("client_list")
          .select("id, firstname, lastname, contact, payment_due_date, opening_balance")
          .not("payment_due_date", "is", null)
      );
      const crossed = (dueClients || []).filter(
        (c) => c.payment_due_date && c.payment_due_date <= today
      );
      ...
      if (crossed.length) {
        ...
        dueDate = crossed
          .map((c) => ({
            ...
            outstanding:
              Math.round(balanceFromMaps(m, c.id, Number(c.opening_balance) || 0) * 100) / 100,
          }))
          .filter((c) => c.outstanding > 0);
      }
      if (dueDate.length)
        alerts.push({
          type: "due_payment_date",
          severity: "warning",
      ...
      const { data: mechs } = await pageAll(
        supabase
          .from("mechanic_list")
          .select("id, firstname, lastname")
          .eq("delete_flag", 0)
      ...
      const { data: attToday } = await pageAll(
        supabase.from("attendance_list").select("mechanic_id").eq("curr_date", today)
      );
      ...
      if (isAdmin) {
        const [clientsRes, txRes, dirRes, payRes, loanRes] = await Promise.all([
          pageAll(
            supabase
              .from("client_list")
              .select("id, firstname, lastname, contact, opening_balance")
              .eq("delete_flag", 0)
          ),
          pageAll(
            supabase
        ...
        if (cLoans.length)
          alerts.push({
            type: "active_loans",
            severity: "info",
            title: "Active Client Loans (Admin)",
            items: cLoans,
          });
      }
      ...
      return {
        ...
        note: isAdmin
          ? undefined
          : "Financial alerts (outstanding/loans) sirf Admin ko dikhte hain.",
      };
    }
    ...
    if (name === "get_inventory_status") {
      const [invRes, prodRes] = await Promise.all([
        ...
        pageAll(
          supabase
            .from("product_list")
            .select("id, name, alert_quantity, price")
            .eq("delete_flag", 0)
        ),
      ]);
      const qty: Record<number, number> = {};
      ...
      const items = (prodRes.data || []).map((p) => {
        ...
        return {
          ...
          low_stock: alert > 0 && quantity <= alert,
          location: locMap.get(p.id) || "—",
        };
      });
      return {
        total_products: items.length,
        low_stock_count: items.filter((x) => x.low_stock).length,
        inventory: items,
      };
    }
      ...
      const list = (clientsRes.data || [])
        .map((c) => {
          ...
          return {
            ...
            loan_given: loan,
            total_paid: paid,
            net_balance: Math.round(balanceFromMaps(m, c.id, ob) * 100) / 100,
          };
        })
        ...
        .slice(0, limit);
      return {
        outstanding_clients: list,
        note: "Balance = opening balance + delivered repairs (status 5) + direct sales - service payments + active loans - loan repayments.",
      };
    }
    ...
    if (name === "get_attendance_report") {
      ...
      const [attRes, mechRes] = await Promise.all([
        pageAll(
          supabase
            .from("attendance_list")
            .select("mechanic_id, status, curr_date")
            .gte("curr_date", from)
            .lte("curr_date", to)
        ),
        pageAll(
          supabase.from("mechanic_list").select("id, firstname, lastname").eq("delete_flag", 0)
        ),
      ]);
      const mechName: Record<number, string> = {};
      (mechRes.data || []).forEach((m) => {
        mechName[m.id] = `${m.firstname} ${m.lastname}`.trim();
      });
      const perMech: Record<
        number,
        {
          mechanic_id: number;
          name: string;
      ...
      (attRes.data || []).forEach((a) => {
        if (!perMech[a.mechanic_id])
          perMech[a.mechanic_id] = {
            mechanic_id: a.mechanic_id,
            name: mechName[a.mechanic_id] || `#${a.mechanic_id}`,
            present_days: 0,
            half_days: 0,
            total_days: 0,
          };
      ...
      return {
        from,
        to,
        summary: Object.values(perMech),
        note: "Based on attendance records in range.",
      };
    }
    ...
    if (name === "get_loan_status") {
      const [lendersRes, loanPaysRes, cLoansRes, clientsRes] = await Promise.all([
        pageAll(
          supabase
            .from("lender_list")
            .select("id, fullname, contact, loan_amount, interest_rate, emi_amount, status")
        ),
        pageAll(supabase.from("loan_payments").select("lender_id, amount_paid")),
        pageAll(
          supabase
            .from("client_loans")
            .select("id, client_id, principal_amount, total_payable, emi_amount, status, loan_date")
        ),
        pageAll(supabase.from("client_list").select("id, firstname, lastname")),
      ]);
      const paid: Record<number, number> = {};
      (loanPaysRes.data || []).forEach((p) => {
        paid[p.lender_id] = (paid[p.lender_id] || 0) + (Number(p.amount_paid) || 0);
      });
      const lenderLoans = (lendersRes.data || []).map((l) => {
        ...
        return {
          ...
          remaining: Math.round((amt - paidAmt) * 100) / 100,
          status: l.status,
        };
      });
      const clientName: Record<number, string> = {};
      (clientsRes.data || []).forEach((c) => {
        clientName[c.id] = `${c.firstname} ${c.lastname}`.trim();
      });
      const clientLoans = (cLoansRes.data || [])
        .filter((l) => l.status === 1)
        .map((l) => ({
          loan_id: l.id,
...
async function buildLiveContext(role: AiRole): Promise<string> {
  ...
  const [txAll, invRes, prodRes, clientsRes, dirRes, payRes, loanRes] = await Promise.all([
    ...
    pageAll(supabase.from("client_payments").select("client_id, amount, discount, loan_id")),
    pageAll(
      supabase.from("client_loans").select("id, client_id, total_payable, status").eq("status", 1)
    ),
  ]);
  ...
  if (lowStock.length) {
    lines.push(
      `- Low stock (reorder needed): ${lowStock.map((p) => `${p.name} (${p.quantity}/${p.alert})`).join(", ")}`
    );
  } else {
  ...
  const m = buildDueMaps({
    repairs: jobs.filter((t) => t.status === 5),
    directSales: dirRes.data,
    payments: payRes.data,
    loans: loanRes.data,
  });
  ...
  if (isAdmin) {
    const deliveredToday = jobs.filter(
      (j) => j.status === 5 && String(j.date_completed || "").startsWith(today)
    );
    const todayRevenue = deliveredToday.reduce((s, j) => s + (Number(j.amount) || 0), 0);
    lines.push(
      `- Today delivered jobs: ${deliveredToday.length} (revenue ₹${todayRevenue.toLocaleString("en-IN")})`
    );
