...
export default function NotificationSettings() {
  ...
  const handleEnable = async () => {
    ...
    if (res.ok) {
      setMsg({
        type: res.mode === "denied" ? "error" : "success",
        text:
          res.mode === "denied"
            ? "Notifications browser me block hain — settings se allow karein."
            : res.mode === "unsupported"
              ? "Ye browser Push notifications support nahi karta."
          ...
          {!status.vapidConfigured && (
            <div className="flex items-start gap-3 bg-amber-500/8 border border-amber-500/20 rounded-xl px-4 py-3">
              ...
              <p className="text-xs text-amber-400/90 leading-relaxed">
                <strong>VAPID public key</strong> configured nahi hai. Push enable karne ke liye
                production me{" "}
                <code className="font-mono bg-black/30 px-1 rounded">
                  NEXT_PUBLIC_VAPID_PUBLIC_KEY
                </code>{" "}
                env var set karna hoga.
              </p>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-3">
            {status.enabled ? (
              ...
              <button
                ...
                disabled={busy || !status.vapidConfigured}
