...
export default function LocationPicker({
  ...
  const set = (key: keyof LocationParts) => (raw: string) => onChange({ ...value, [key]: raw });
  ...
  const fields: {
  ...
  }[] = [
    {
      key: "zone",
      label: "Zone / Area",
      placeholder: "e.g. Main Shop, Store Room",
      weight: "sm:col-span-2",
    },
    ...
    <div className="space-y-2.5">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-4 gap-2 ${compact ? "" : "gap-2.5"}`}>
        {fields.map(({ key, label, placeholder, weight }) => {
          ...
          const opts = (sug[key] || []).filter(
            (o) => o && o.toLowerCase() !== value[key].toLowerCase()
          );
          return (
