...
export default function SpotJobsModal({
  spot,
  onClose,
}: {
  spot: SpotRef | null;
    ...
    (async () => {
      ...
      try {
        let q = supabase
          ...
          .limit(200);
        q = spot.id ? q.eq("location_id", spot.id) : q.eq("uniq_id", spot.name).eq("del_status", 0);
  ...
  const jobsUrl = spot.id
    ? `/jobs?spot=${spot.id}`
    : `/jobs?search=${encodeURIComponent(spot.name || "")}`;

  return (
    <div
      className="no-print fixed inset-0 z-[80] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
      onClick={onClose}
    >
      <div
                        ...
                        {past.slice(0, 20).map((j) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                className={`text-[9px] font-bold border rounded-md px-1 py-0.5 flex-shrink-0 ${badge.cls}`}
                              >
                                {badge.label}
                              </span>
                            </Link>
