...
export default function DeveloperPage() {
  ...
  const openKit = useCallback(async (row: Row) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setKitMsg(data.error || "Prefill load fail");
        return;
      }
      setPrefill(data);
  ...
  const push = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setPushMsg(data.error || "Push fail hua");
        return;
      }
    ...
    >
      <div className="space-y-5">
                  ...
                  <tbody className="divide-y divide-[#1a2234]">
                    {rows.map((r) => (
                      <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider border ${
                              r.active
        ...
        <p className="text-[10px] text-slate-600 font-semibold">
          ...
          <span className="font-mono">app-info.json</span> hota hai. License service URL/key seller
          ke deployment ke env se aati hai. Data central{" "}
          <span className="font-mono">vtech_licence</span> project se.
        </p>
      </div>
      ...
      {kitFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white dark:bg-[#12161f] border border-slate-200 dark:border-[#21293d] rounded-2xl p-5 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 font-bold mt-1">
                  {prefill?.licenseKey || kitFor.license_key}
                </p>
              </div>
              <button
            ...
            {kitMsg && (
              <p
                className={`mt-3 text-xs font-semibold rounded-xl px-4 py-3 border ${kitMsg.startsWith("Package") || kitMsg.startsWith("Prefill") ? "text-red-600 dark:text-red-400 bg-red-500/10 border-red-500/20" : "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20"}`}
              >
                {kitMsg}
              </p>
            )}
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-[#1a2234]">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
                      Push to Vercel (client ke apne account par)
                    </span>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <SecretField
                        ...
                        placeholder="Client ke account ka token"
                      />
                    </div>
                    <div>
                    ...
                    >
                      {pushBusy ? (
                        <>
                          <Loader2 size={14} className="animate-spin" /> Pushing + Deploying...
                        </>
                      ) : (
                        <>
                          <Rocket size={14} /> Push Env & Redeploy Production
                        </>
                      )}
                    </button>
                    {pushMsg && (
                      <p
                        className={`text-[11px] font-semibold rounded-xl px-3 py-2.5 border ${pushMsg.startsWith("Push fail") ? "text-red-600 dark:text-red-400 bg-red-500/10 border-red-500/20" : "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20"}`}
                      >
                        {pushMsg}
                      </p>
                    )}
