...
const MODE_INFO: Record<SyncMode, { title: string; desc: string }> = {
  auto: {
    title: "Auto",
    desc: "Task Scheduler har 15 min me Supabase → MariaDB sync kar dega. Koi dhyan nahi dena padega.",
  },
  manual: {
    title: "Manual",
    desc: "Auto-sync band. Jab bhi data fresh karna ho, 'Sync Now' dabao — wahi ek tarika.",
  },
  off: {
    title: "Off",
    desc: "Sync bilkul band — na auto, na Sync Now. Jab tak ise Auto/Manual pe wapas na rakho, kuch nahi hoga.",
  },
};
        ...
        <div className="bg-red-500/10 border border-red-500/30 rounded-2xl px-5 py-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-red-400/60 text-xs mt-1">
              Is tool ko sirf admin/developer role wale khol sakte hain.
            </p>
          </div>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-2xl px-5 py-3.5">
          <p className="text-red-400 text-xs font-bold mb-1 flex items-center gap-1.5">
            <XCircle size={13} /> Sync fail ho gaya — error:
          </p>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-sm font-black ${last.status === "OK" ? "text-emerald-400" : "text-red-400"}`}
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                <div className="bg-[#0d1117] border border-[#21293d] rounded-xl px-3 py-2.5">
                  ...
                  <p
                    className={`text-lg font-black mt-1 ${last.mismatches > 0 ? "text-amber-400" : "text-white"}`}
                  >
                    {last.mismatches}
                  </p>
                </div>
              ...
              {last.status !== "OK" && last.details && (
                <pre className="text-red-400/70 text-[11px] font-mono whitespace-pre-wrap bg-red-500/5 border border-red-500/20 rounded-xl p-3">
                  {last.details}
                </pre>
              )}
            </div>
