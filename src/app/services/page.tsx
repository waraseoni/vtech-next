...
export default function ServicesPage() {
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ name: "", description: "", price: "", hsn: "" });
    setFormErr("");
    setShowModal(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) {
      setFormErr("Service name zaroori hai!");
      return;
    }
    if (!form.price || parseFloat(form.price) < 0) {
      setFormErr("Valid price daalo!");
      return;
    }
    setSaving(true);
    try {
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-[#21293d]">
              <h3 className="font-bold text-white flex items-center gap-2">
                {editing ? (
                  <>
                    <Edit3 size={16} className="text-blue-400" /> Edit Service
                  </>
                ) : (
                  <>
                    <Plus size={16} className="text-blue-400" /> Add Service
                  </>
                )}
              </h3>
              <button
