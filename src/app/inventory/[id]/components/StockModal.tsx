...
export default function StockModal({ productId, stock, onClose, onSaved, productName, productLocationLabel }: StockModalProps) {
  const isEdit = !!stock;
      ...
      >
        {success && (
          <div className="absolute inset-0 z-10 bg-[#161b27] flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-white font-extrabold text-lg">{isEdit ? "Updated!" : "Stock Added!"}</p>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#21293d]">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center border ${isEdit ? "bg-amber-500/10 border-amber-500/25" : "bg-blue-500/10 border-blue-500/25"}`}>
              {isEdit ? <Edit3 size={16} className="text-amber-400" /> : <ArrowDownToLine size={16} className="text-blue-400" />}
            </div>
            <div>
              <h3 className="text-base font-extrabold text-white leading-none">{isEdit ? "Edit Stock Entry" : "Add New Stock"}</h3>
        ...
        <form onSubmit={handleSubmit} className="flex-1 min-h-0 flex flex-col">
          <div className="px-5 py-5 space-y-5 overflow-y-auto flex-1 min-h-0">
            ...
            {suppliers.length > 0 && (
              <div>
                <label className="block text-[10px] font-extrabold uppercase tracking-widest text-slate-600 mb-2.5">
                  <span className="flex items-center gap-1.5"><Package size={10} className="text-slate-700" /> Supplier (Optional)</span>
                </label>
                <SearchableSelect value={supplierId || null}
                  options={suppliers.map(s => ({ id: s.id, label: s.name }))}
                  onSelect={v => setSupplierId(v)} placeholder="-- Select Supplier --" clearLabel="-- Select Supplier --" />
              </div>
            )}
            ...
            <div>
              <label className="block text-[10px] font-extrabold uppercase tracking-widest text-slate-600 mb-2.5">
                <span className="flex items-center gap-1.5"><Calendar size={10} className="text-slate-700" /> Stock Date <span className="text-red-500">*</span></span>
              </label>
              <input type="date" required value={stockDate} max={today}
              ...
              <div className="flex gap-2 mt-2">
                {[
                  ...
                  <button key={label} type="button" onClick={() => setStockDate(val)}
          ...
          <div className="px-5 pb-5 pt-1 flex gap-2.5 flex-shrink-0">
            <button type="submit" disabled={saving || success}
              ...
              {saving ? <><Loader2 size={16} className="animate-spin" /> Saving...</> : success ? <><CheckCircle2 size={16} /> Saved!</> : <><Save size={16} /> {isEdit ? "Update Entry" : "Add Stock"}</>}
            </button>
