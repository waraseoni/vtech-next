...
// ─── Sprint 4 #17 (SSR): inventory list ka client hissa ─────────────────────
// Server `page.tsx` pehla data laata hai (products + stock + role, places
// khaali). Service-role wali locations (`/api/locations/by-product`) mount
// par merge hoti hain (progressive) + Refresh/scan/add par full fetchData.
...
// Service-role locations API (RLS anon ko block karta hai — isliye client par
// hi rehta hai). Server SSR me places khaali aate hain, mount par merge hote
// hain. fetchData (refresh) bhi isi se guzarta hai — single code path.
async function fetchLocationsMap(ids: number[]): Promise<Map<number, LocRow[]>> {
  const locMap = new Map<number, LocRow[]>();
  if (!ids.length) return locMap;
  try {
    ...
    // Fail-safe: khali map (stock independent RPC se aata hai)
  }
  return locMap;
}
...
export function InventoryClient({
  ...
  // Server se aaya data = pehla paint (koi client waterfall nahi)
  ...
  const [userRole, setUserRole] = useState<string>(initialRole || "staff");

  // Role server se aati hai (source of truth). Ye fallback sirf tab jab
  // server role na de paya ho (seedha-client-mount edge).
  useEffect(() => {
    if (initialRole) return;
    import("@/lib/supabase").then(({ supabase, getCachedUser }) => {
  ...
  // ── Locations progressive merge (mount par ek baar) ──────────────────────
  // Server places khaali bhejta hai (service-role); ye effect unhe bharta hai.
  useEffect(() => {
    let alive = true;
    (async () => {
    ...
    // mount-once: initialProducts server props hain, dobara merge nahi
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // ── Fetch (full refresh: Refresh button + scan/add modal onSaved) ────────
  const fetchProducts = async (silent = false) => {
    ...
    try {
      ...
      const [poLinkRes, locMap, stockMap] = await Promise.all([
        supabase
          .from("inventory_list")
      ...
      const result: ProductStock[] = pl.map((p) => {
        ...
        const { place, places: placePaths } = placesOf(locMap, p.id);
        return {
          id: p.id,
          name: p.name,
  ...
  // ── Loading (sirf manual refresh par — pehla load server se) ─────────
  if (loading) {
