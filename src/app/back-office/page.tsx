  ...
  const sections: Section[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: <Users />,
          color: "purple",
          desc: "Client balance management",
        },
        {
          name: "Lenders",
          href: "/lenders",
...
function ModuleCard({ item }: { item: OfficeItem }) {
  const colorMap: Record<string, string> = {
    ...
    emerald:
      "from-emerald-500 to-emerald-700 shadow-emerald-500/20 text-emerald-400 border-emerald-500/20",
    blue: "from-blue-500 to-blue-700 shadow-blue-500/20 text-blue-400 border-blue-500/20",
    purple:
      "from-purple-500 to-purple-700 shadow-purple-500/20 text-purple-400 border-purple-500/20",
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div
          className={`w-12 h-12 bg-gradient-to-br ${colorMap[item.color].split(" shadow")[0]} rounded-2xl flex items-center justify-center text-white shadow-xl ${colorMap[item.color].split("text")[0].split("shadow")[1]} group-hover:scale-110 transition-transform duration-300`}
        >
          {item.icon}
        </div>
        <ArrowRight
