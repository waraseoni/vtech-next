...
const SCHEMA_FILES = [
  {
    id: "drop",
    label: "00_drop_all.sql",
    desc: "Clean slate — saare tables/functions/triggers drop",
    color: "red",
  },
  {
    id: "baseline",
    label: "baseline_schema.sql",
    desc: "Full schema — tables + RLS + functions + triggers + buckets",
    color: "green",
  },
  {
    id: "migrations",
    label: "00_full_schema.sql",
    desc: "Lightweight schema — tables + functions (bina RLS ke)",
    color: "blue",
  },
] as const;
...
const DEPLOY_STEPS = [
  {
    step: 1,
    title: "Supabase Project Banao",
    cmd: null,
    desc: "supabase.com → New Project → Region ap-south-1 → Password save karo",
  },
  {
    step: 2,
    title: "Schema Clean Karo",
    cmd: "00_drop_all.sql",
    desc: "SQL Editor → New Query → drop_all paste → Run",
  },
  {
    step: 3,
    title: "Schema Lagao",
    cmd: "baseline_schema.sql",
    desc: "SQL Editor → New Query → baseline paste → Run",
  },
  {
    step: 4,
    title: "Verify Karo",
    cmd: "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public';",
    desc: "36 tables aane chahiye",
  },
  {
    step: 5,
    title: "Vercel Deploy",
    cmd: null,
    desc: "New Project → GitHub repo select → Env vars set karo",
  },
  {
    step: 6,
    title: "Env Variables",
    cmd: null,
    desc: "NEXT_PUBLIC_SUPABASE_URL, ANON_KEY, SERVICE_ROLE_KEY, LICENSE_SERVICE_URL, LICENSE_SERVICE_ANON_KEY",
  },
  {
    step: 7,
    title: "Setup Page",
    cmd: null,
    desc: "Client ka URL kholo → /setup → Admin account banao",
  },
  {
    step: 8,
    title: "License Activate",
    cmd: null,
    desc: "Client ko key do → Settings → License Activation → Activate",
  },
];
  ...
  const runSchemaCheck = async () => {
    ...
    try {
      ...
      checks.push({
        ...
        status: error ? "ok" : "ok",
        detail: error
          ? "RLS enforced (query restricted)"
          : `profiles accessible (${count || 0} rows)`,
      });
    } catch {
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6 space-y-5">
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="bg-[#0d1117] border border-[#21293d] rounded-xl p-4 space-y-2">
                ...
                {[
                  {
                    cmd: "node scripts/supabase-restore.mjs backup.sql --dry-run",
                    desc: "Check only",
                  },
                  { cmd: "node scripts/supabase-restore.mjs backup.sql", desc: "Restore" },
                  {
                    cmd: "node scripts/supabase-restore.mjs backup.sql --force",
                    desc: "Skip confirm",
                  },
                ].map(({ cmd, desc }) => (
                  <div key={cmd} className="flex items-center justify-between gap-2 group">
            ...
            <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl px-4 py-3">
              <p className="text-[10px] text-amber-400/80 font-bold">
                .env.local me SUPABASE_DB_PASSWORD + NEXT_PUBLIC_SUPABASE_URL hona chahiye. Ya
                directly <code>--db-url</code> flag use karo.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#161b27] border border-[#21293d] rounded-2xl overflow-hidden">
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="pt-2">
              <a
                href="https://github.com/waraseoni/vtech-next/blob/main/docs/DEPLOYMENT_GUIDE.md"
                target="_blank"
