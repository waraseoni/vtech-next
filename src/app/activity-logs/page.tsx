...
const UserAvatar = ({
  ...
  image ? (
    ...
    <div
      className={`${cls} rounded-full flex items-center justify-center font-black flex-shrink-0 ${user_id === 0 ? "bg-amber-500/10 text-amber-500" : "bg-blue-500/10 text-blue-500"}`}
    >
...
export default function ActivityLogsPage() {
    ...
    try {
      ...
      if (debouncedSearch) {
        countQuery = countQuery.or(
          `action.ilike.%${debouncedSearch}%,details.ilike.%${debouncedSearch}%`
        );
      ...
      if (mechIds.length > 0) {
        ...
        const { data: mechs } = await supabase
          ...
          .in("id", mechIds);
        mechs?.forEach((m) =>
          mechsMap.set(m.id, { name: `${m.firstname} ${m.lastname}`, image: m.image_path || null })
        );
      }
  ...
  const getModuleIcon = (mod: string) => {
    switch (mod) {
      ...
      case "Jobs":
        return <UserCog size={12} className="text-amber-400" />;
      case "Inventory":
        return <Package size={12} className="text-purple-400" />;
      case "Sales":
        return <ShoppingCart size={12} className="text-emerald-400" />;
      default:
        return <Info size={12} className="text-muted" />;
    }
  };
  ...
  const getRelatedLink = (module: string, metaId: string | null) => {
    ...
    if (m.includes("transaction") || m.includes("job"))
      return { href: `/jobs/${metaId}/view`, label: "View Job" };
    ...
    if (m.includes("inventory") || m.includes("product"))
      return { href: "/inventory", label: "Inventory" };
    return null;
  };

  const formatDetails = (details: string | null, module: string) => {
    if (!details) return <span className="italic text-app">—</span>;
  ...
  return (
    <div className="min-h-screen bg-app text-white p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-panel border border-app rounded-2xl p-5">
          <div className="flex flex-wrap items-end gap-4">
            <div className="flex-1 min-w-[240px]">
              <label className="block text-[10px] font-black uppercase tracking-widest text-muted-2 mb-1.5 ml-1">
                Search Logs
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-app border border-app rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:border-blue-500/50 transition-all shadow-inner"
                />
              </div>
            </div>
            ...
            <div className="w-full sm:w-48">
              ...
              <select
                ...
                className="w-full bg-app border border-app rounded-xl px-4 py-2.5 text-sm outline-none focus:border-blue-500/50 [color-scheme:dark]"
              >
            ...
            <div className="flex items-center gap-2">
              <div>
                ...
                <input
                  ...
                  className="bg-app border border-app rounded-xl px-3 py-2 text-sm [color-scheme:dark]"
                />
              </div>
              <div className="pt-6 text-app">—</div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-muted-2 mb-1.5 ml-1">
                  To
                </label>
            ...
            <button
              ...
              className="p-2.5 bg-app border border-app text-muted hover:text-red-400 rounded-xl transition-all"
            >
              <X size={18} />
            </button>
          </div>
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-muted-2 font-black uppercase">Show</span>
              <select
                value={pageSize}
                onChange={(e) => {
                ...
                className="bg-app border border-app rounded-lg px-2 py-1 text-xs outline-none [color-scheme:dark]"
              >
                {PAGE_SIZES.map((s) => (
        ...
        {/* Logs Table */}
        <div className="bg-panel border border-app rounded-2xl overflow-hidden shadow-2xl">
          {loading && !hasLoaded ? (
            <PageLoader icon={ScrollText} label="fetching audit logs..." tone="blue" />
          ) : (
            <>
              {loading && hasLoaded && (
                <div className="flex items-center gap-2 text-[11px] font-bold text-blue-400 px-5 py-2.5 bg-app/50 border-b border-app">
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-app/50 border-b border-app">
                      <th className="text-left px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-2">
                        Time
                      </th>
                      <th className="text-left px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-2">
                        User
                  ...
                  <tbody className="divide-y divide-[#21293d]">
                    {logs.map((log) => (
                      <tr key={log.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <UserAvatar
                              ...
                              user_id={log.user_id}
                            />
                            <span className="text-app-2 font-semibold">{log.username}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2 bg-app border border-app rounded-lg px-2.5 py-1 w-fit">
                    ...
                    {logs.length === 0 && (
                      <tr>
                        <td colSpan={6} className="p-20 text-center">
                          <History size={40} className="text-app mx-auto mb-4 opacity-20" />
                          <p className="text-muted-2 font-black uppercase tracking-widest text-xs">
              ...
              <div className="md:hidden divide-y divide-[#21293d]">
                {logs.map((log) => (
                  <div key={log.id} className="p-4 hover:bg-white/[0.02] transition-colors group">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-[10px] text-muted-2 font-bold">•</span>
                            <span className="text-[10px] text-muted font-bold">
                              {formatIST(log.date_created, {
                                day: "2-digit",
                                month: "short",
                      ...
                      <div className="flex items-center gap-1.5 bg-app border border-app rounded-lg px-2.5 py-1 w-fit flex-shrink-0">
                        {getModuleIcon(log.module)}
                        <span className="text-[9px] font-black uppercase tracking-tight text-muted">
                          {log.module}
                        </span>
                      </div>
                    </div>
                ...
                {logs.length === 0 && (
                  <div className="p-10 text-center">
                    <History size={32} className="text-app mx-auto mb-3 opacity-20" />
                    <p className="text-muted-2 font-black uppercase tracking-widest text-[10px]">
                      No activity logs found
                    </p>
                  </div>
                )}
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div className="flex items-center justify-between flex-wrap gap-3 px-5 py-4 border-t border-app bg-app/30">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={page === 0}
                      className="p-2 bg-app border border-app rounded-lg text-muted hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    >
                      <ChevronLeft size={16} />
                    </button>
