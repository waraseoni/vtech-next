    ...
    <div className="min-h-screen bg-white text-gray-900 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-gray-600 text-[11px] font-extrabold uppercase tracking-[0.25em] mt-2">
                Swaagat hai, Vikram ji! Aaj ka kaam shuru karein.
              </p>
            </div>
          </div>
          <Link 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Quick Actions Card – Consistent with theme */}
          <div className="lg:col-span-2 bg-white p-6 md:p-8 rounded-[2.5rem] border-2 border-gray-300 shadow-md">
            <div className="flex items-center gap-2 mb-6">
              <TrendingUp size={20} className="text-blue-600" />
              <h3 className="text-[11px] font-extrabold uppercase text-gray-500 tracking-[0.15em]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <ActionLink 
                ...
                icon={<Wrench size={22} />} 
                label="View Jobs" 
                sub="Manage all repairs" 
              />
              <ActionLink 
                href="/clients" 
                icon={<Users size={22} />} 
                label="Customers" 
                sub="List of all clients" 
              />
              <ActionLink 
                href="/inventory" 
                icon={<AlertCircle size={22} />} 
                label="Inventory" 
                sub="Check spare parts" 
              />
            </div>
          </div>

          {/* Pro Tip Card – Gradient with border & refined text */}
          <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-6 md:p-8 rounded-[2.5rem] border-2 border-gray-300 shadow-md text-white relative overflow-hidden group">
            <Sparkles 
              className="absolute -right-4 -top-4 text-white/10 rotate-12 group-hover:scale-150 transition-transform duration-700" 
              size={120} 
              strokeWidth={1} 
            />
            <div className="relative z-10 h-full flex flex-col">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-white/90">
                  Shop Secret
                </h4>
              </div>
...
// ===== Stat Card Component – Fully Consistent =====
function StatCard({ label, value, icon, color }: { 
  ...
  return (
    <div className="bg-white p-6 rounded-[2rem] border-2 border-gray-300 shadow-md flex items-center gap-5 transition-transform hover:-translate-y-1 duration-300">
      ...
      <div>
        <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-gray-500 leading-none mb-1">
          {label}
        </p>
        <div className="text-3xl font-extrabold italic text-gray-900 tracking-tight">
          {value}
        </div>
      </div>
    </div>
...
// ===== Action Link Component – Consistent with Jobs Page Buttons =====
function ActionLink({ href, icon, label, sub }: { 
  ...
  return (
    <Link 
      href={href} 
      className="group flex flex-col p-5 bg-white border-2 border-gray-300 hover:border-blue-400 rounded-2xl transition-all no-underline hover:shadow-md hover:-translate-y-0.5"
    >
      <div className="text-gray-500 group-hover:text-blue-600 transition-colors mb-3">
        {icon}
      </div>
      <div className="flex items-center justify-between">
        <span className="font-extrabold text-gray-800 uppercase text-xs tracking-wider group-hover:text-blue-700 transition-colors">
          {label}
        </span>
