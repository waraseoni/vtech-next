...
export default function MechanicLedger() {
  ...
  const [from, setFrom] = useState(() => {
    ...
    if (monthParam && /^\d{4}-\d{2}$/.test(monthParam))
      return format(startOfMonth(new Date(monthParam + "-01")), "yyyy-MM-dd");
  ...
  const [to, setTo] = useState(() => {
    ...
    if (t) return t;
    if (monthParam && /^\d{4}-\d{2}$/.test(monthParam))
      return format(endOfMonth(new Date(monthParam + "-01")), "yyyy-MM-dd");
  ...
  const changeMonth = (offset: number) => {
    ...
    const newFrom =
      offset > 0
        ? startOfMonth(addMonths(currentFrom, 1))
        : startOfMonth(subMonths(currentFrom, 1));
  ...
  const handlePrint = () => {
    window.open(
      `/api/print-mechanic-ledger?id=${mid}&from=${from}&to=${to}&mode=created`,
      "_blank"
    );
  };
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 no-print">
        <SummaryCard
          ...
          icon={<Wallet size={18} />}
          color="amber"
          sub="History before selection"
        />
        <SummaryCard
          label="Earned Wages"
          value={totalPeriod.wage}
          icon={<TrendingUp size={18} />}
          color="blue"
          sub="Daily pay for attendance"
        />
        <SummaryCard
          label="Payable Commission"
          value={totalPeriod.payable}
          icon={<CheckCircle size={18} />}
          color="emerald"
          sub="Delivered jobs only"
        />
        <SummaryCard
          label="Total Advance"
          value={totalPeriod.advance}
          icon={<IndianRupee size={18} />}
          color="rose"
          sub="Salary payouts & advances"
        />
      </div>
          ...
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#21293d]">
              ...
              <tr className="bg-amber-500/5 font-bold">
                <td
                  colSpan={6}
                  className="px-4 py-3 text-right text-amber-500 text-[10px] uppercase tracking-widest"
                >
                  Opening Balance (Brought Forward):
                </td>
                <td className="px-4 py-3 text-right text-amber-400 font-black">
                  {inr(openingBalance)}
                </td>
              </tr>
                ...
                <tr key={idx} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-emerald-400 font-black" title="Payable (Delivered)">
                        {row.commPayable > 0 ? inr(row.commPayable) : "—"}
                      </span>
            ...
            <tfoot className="bg-[#0d1117] border-t-2 border-[#21293d]">
              <tr className="font-bold">
                ...
                <td className="px-4 py-4 text-right bg-blue-500/10 text-blue-400 text-lg font-black">
                  {inr(ledgerData[ledgerData.length - 1]?.runningBal || openingBalance)}
                </td>
              </tr>
            </tfoot>
          </table>
      ...
      <div className="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-4 flex items-start gap-3 no-print">
        ...
        <div className="text-xs text-slate-500 space-y-1">
          ...
          <p>
            <strong className="text-emerald-400">Payable Commission:</strong> Only jobs with status
            &quot;Delivered&quot; are added to the running balance. Pending jobs show in blue but
            don&apos;t affect the balance until delivered.
          </p>
        </div>
      </div>
