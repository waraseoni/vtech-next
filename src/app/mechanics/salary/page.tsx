      ...
      if (activeTab === "report") {
        ...
        const getRate = (mid: number, dateStr: string, defaultRate: number) => {
          const hist = (salaryHist || []).find(
            (h) => h.mechanic_id === mid && h.effective_date <= dateStr
          );
          return hist ? parseFloat(hist.salary) : defaultRate;
        };

        const records: SalaryRecord[] = await Promise.all(
          mechs.map(async (m) => {
            const mid = m.id;
            ...
            const advPrev = (prevAdv.data || []).reduce(
            ...
            const commCurr = (currComm.data || []).reduce(
              (s, c) => s + (parseFloat(c.mechanic_commission_amount) || 0),
              0
            );
            const advCurr = (currAdv.data || []).reduce(
              (s, a) => s + (parseFloat(a.amount) || 0),
              0
            );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mt-0.5">
              Earnings, Deductions & Rate Master
            </p>
          </div>
        </div>
        ...
        <>
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl p-4 shadow-lg flex items-center justify-center gap-4 no-print">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center bg-[#0d1117] border border-[#21293d] rounded-full text-slate-400 hover:text-white transition-all"
            >
              <ChevronLeft size={20} />
            </button>
                    ...
                    : reportData.map((row, idx) => (
                        <tr key={row.id} className="hover:bg-white/[0.02] transition-colors group">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <Link
                                  ...
                                  className="text-white font-bold hover:text-blue-400 transition-colors no-underline"
                                >
                                  {row.name}
                                </Link>
                                <p className="text-[9px] text-slate-600 font-black uppercase mt-0.5">
                                  Click for ledger
                                </p>
                              </div>
                            </div>
                          ...
                          <td className="px-6 py-4 text-center font-bold">
                            <span className="text-emerald-400" title="Present">
                              {row.present}
                            </span>
                            <span className="text-slate-700 mx-1.5">|</span>
                            <span className="text-amber-400" title="Half Day">
                              {row.halfDays}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-right font-bold text-amber-400/80">
                            {inr(row.commission)}
                          </td>
                          <td
                            className={`px-6 py-4 text-right font-bold ${row.oldBalance >= 0 ? "text-blue-400/80" : "text-rose-400"}`}
                          >
                            {inr(row.oldBalance)}
                          </td>
                          ...
                          <td className="px-6 py-4 text-right bg-blue-500/5">
                            <span
                              className={`px-3 py-1 rounded-lg font-black ${row.netTotal >= 0 ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border border-rose-500/20"}`}
                            >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <Link
                              ...
                              className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/10 border border-purple-500/20 text-purple-400 hover:bg-purple-500 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition-all no-underline"
                            >
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={updateSalaryRate} className="p-6 space-y-5">
              <div className="space-y-1.5">
                ...
                <input
                  value={`${editingMech?.firstname} ${editingMech?.lastname}`}
                  readOnly
                  className="w-full px-4 py-3 bg-[#0d1117] border border-[#21293d] rounded-xl text-sm text-slate-400 font-bold outline-none"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[9px] text-slate-600 font-bold uppercase mt-1">
                  Select previous date for retroactive changes.
                </p>
              </div>
              <button
                type="submit"
                disabled={saving}
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-xl shadow-blue-600/25 flex items-center justify-center gap-2"
              >
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handlePayout} className="p-6 space-y-5">
              ...
              <button
                type="submit"
                disabled={saving}
                className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-xl shadow-emerald-600/25 flex items-center justify-center gap-2"
              >
