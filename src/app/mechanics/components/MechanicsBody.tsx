...
export default function MechanicsBody({
  ...
  const [rows, setRows] = useState<Mechanic[]>(initialMechanics);
  ...
  // Write ke baad list taba-dili se refresh (interactive mutation — initial
  // SSR load server se aata hai, ye sirf add/edit/delete/toggle ke baad).
  const refreshData = useCallback(async () => {
    ...
    if (data) setRows((data as Mechanic[]) || []);
    setRefreshing(false);
  }, []);
  ...
  const totalDailySalary = rows
    .filter((m) => m.status === 1)
    .reduce((s, m) => s + (m.daily_salary || 0), 0);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.firstname.trim()) {
      setFormErr("First name zaroori hai!");
      return;
    }
    if (!form.lastname.trim()) {
      setFormErr("Last name zaroori hai!");
      return;
    }
    ...
    if (isNaN(salary) || salary < 0) {
      setFormErr("Valid daily salary daalo!");
      return;
    }
      ...
      if (editing) {
        ...
        if (salary !== editing.daily_salary) {
          const { error: histErr } = await supabase
            ...
            .insert([{ mechanic_id: editing.id, salary, effective_date: today }]);
  ...
  const toggleStatus = async (m: Mechanic) => {
    ...
    if (!error) {
      await logActivity(
        ...
        "Mechanics",
        m.id,
        `Staff: ${name} → ${newStatus === 1 ? "Active" : "Inactive"}`
      );
      refreshData();
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          label="Inactive"
          value={String(inactiveMechanics)}
          color="amber"
        />
        <StatCard
          icon={<DollarSign size={18} />}
          label="Daily Salary"
          value={inr(totalDailySalary)}
          sub="Active only"
          color="purple"
        />
      </div>
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-[#21293d]">
              <h3 className="font-bold text-white flex items-center gap-2">
                {editing ? (
                  <>
                    <Edit3 size={16} className="text-blue-400" /> Edit Mechanic
                  </>
                ) : (
                  <>
                    <Plus size={16} className="text-blue-400" /> Add Mechanic
                  </>
                )}
              </h3>
              <button
