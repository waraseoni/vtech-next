...
export default function MechanicLedgerPage() {
  ...
  const fetchLedger = useCallback(async () => {
    ...
    opening += (prevComm || []).reduce(
      (s: number, c) => s + (c.mechanic_commission_amount || 0),
      0
    );
    ...
    (periodAtt.data || []).forEach((a) => {
      attMap[a.curr_date] = a.status;
    });
  ...
  useEffect(() => {
    if (mechanic) fetchLedger();
  }, [mechanic, fetchLedger]);
  ...
  const name = mechanic
    ...
    : "";
  const monthDisplay = new Date(fromDate).toLocaleDateString("en-IN", {
    month: "long",
    year: "numeric",
  });
      ...
      <div className="bg-[#161b27] border border-[#21293d] rounded-2xl px-5 py-4 flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() =>
              window.open(
                `/api/print-mechanic-ledger?id=${id}&from=${fromDate}&to=${toDate}`,
                "_blank"
              )
            }
            className="flex items-center gap-1.5 px-3 py-2 bg-[#1e2637] border border-[#2a3550] hover:bg-[#252f45] text-slate-400 hover:text-white rounded-xl text-xs font-bold transition"
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-[#1a2234]">
                <tr className="bg-amber-500/5">
                  <td colSpan={5} className="px-4 py-3 text-right font-bold text-amber-400">
                    Opening Balance:
                  </td>
                ...
                <tr className="bg-[#111520]">
                  ...
                  <td
                    className={`px-4 py-3 text-right font-black ${closingBalance >= 0 ? "text-emerald-400" : "text-red-400"}`}
                    style={{ fontSize: "1rem" }}
                  >
                    {inr(closingBalance)}
                  </td>
