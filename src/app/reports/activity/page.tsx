...
export default function ActivityLogPage() {
  ...
  const cleanOldLogs = async () => {
    if (
      !confirm(
        `Are you sure to clean up old activity logs? This will delete logs older than ${retention} days.`
      )
    )
      return;
    setCleaning(true);
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (searchTerm) {
        query = query.or(
          `action.ilike.%${searchTerm}%,details.ilike.%${searchTerm}%,module.ilike.%${searchTerm}%`
        );
      }
      ...
      if (modData) {
        const uniqueMods = Array.from(new Set(modData.map((m) => m.module)))
          .filter(Boolean)
          .sort() as string[];
  ...
  const getModuleColor = (mod: string) => {
    ...
    if (m.includes("product") || m.includes("inventory"))
  ...
  const getActionStyles = (action: string) => {
    ...
    if (a.includes("delete") || a.includes("removed"))
      return { color: "text-red-400 bg-red-400/10 border-red-400/20", icon: <Trash2 size={12} /> };
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 p-4 bg-blue-500/5 border border-blue-500/15 rounded-2xl">
        ...
        {cleanMsg && (
          <span className="text-xs font-bold text-emerald-400 whitespace-nowrap">{cleanMsg}</span>
        )}
      </div>
              ...
              {loading ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${getModuleColor(log.module)}`}
                      >
                        {log.module}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {(() => {
                        ...
                        return (
                          <div
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-wider ${style.color}`}
                          >
                            {style.icon}
                            {log.action}
                          </div>
