...
function DailyIncomeContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: repairs } = await pageAll(
        supabase
      ...
      const { data: sales } = await pageAll(
        supabase
          .from("direct_sales")
      ...
      const { data: attendance } = await pageAll(
        supabase
      ...
      const { data: mechanics } = await pageAll(
        supabase.from("mechanic_list").select("id, firstname, lastname, daily_salary")
      );
      const { data: salaryHistory } = await pageAll(
      ...
      const { data: commRows } = await pageAll(
        supabase
          .from("transaction_list")
      ...
      const { data: discRows } = await pageAll(
        supabase
      ...
      const { data: loanRows } = await pageAll(
        supabase
          .from("loan_payments")
      ...
      const { data: payRows } = await pageAll(
        supabase
          .from("client_payments")
      ...
      // 9. Staff advances
      const { data: advRows } = await pageAll(
        supabase
      ...
      const { data: lenders } = await pageAll(supabase.from("lender_list").select("id, fullname"));
      ...
      Object.values(histByMech).forEach((list) =>
      ...
      (attendance || []).forEach((a) => {
        ...
        const history = (histByMech[a.mechanic_id] || []).filter(
          (h) => h.effective_date <= a.curr_date
        );
      ...
      const commission = (commRows || []).reduce(
        (s, r) => s + (r.mechanic_commission_amount || 0),
        0
      );
      ...
      const spotSales = (sales || []).filter(
        (s) => s.client_id == null || s.client_id === 0 || s.client_id === ""
      );
      ...
      setDetails({
        repairs: (repairs || []).map((r) => ({
          ...r,
          client_name: clientName(r.client_name),
          mechanic_name: mechName(r.mechanic_id),
        })),
        sales: (sales || []).map((r) => ({
          ...r,
          client_name:
            r.client_id == null || r.client_id === 0 || r.client_id === ""
              ? "Walk-in"
  ...
  const modalTotal = (type: ModalType) => {
    ...
    if (type === "spot" || type === "sales")
      return rows.reduce((s, r) => s + (r.total_amount || 0), 0);
    if (type === "loan") return rows.reduce((s, r) => s + (r.amount_paid || 0), 0);
    if (type === "commission")
      return {
        job: rows.reduce((s, r) => s + (r.amount || 0), 0),
        comm: rows.reduce((s, r) => s + (r.mechanic_commission_amount || 0), 0),
      };
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            <button
              ...
              className="text-left bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-6 shadow-xl hover:-translate-y-0.5 transition-all group"
            >
            ...
            <button
              ...
              className="text-left bg-gradient-to-br from-rose-500 to-red-600 rounded-3xl p-6 shadow-xl hover:-translate-y-0.5 transition-all group"
            >
              <div className="flex items-center gap-2 mb-4">
            ...
            <button
              ...
              className={`text-left rounded-3xl p-6 shadow-xl hover:-translate-y-0.5 transition-all border ${
                data.net_cash_flow >= 0
                  ? "bg-gradient-to-br from-sky-500 to-blue-700 border-transparent"
            ...
            <button
              onClick={() => setModal("expenses")}
              className="text-left bg-gradient-to-br from-violet-600 to-purple-800 rounded-3xl p-6 shadow-xl hover:-translate-y-0.5 transition-all group"
            >
              <div className="flex items-center gap-2 mb-4">
              ...
              <table className="w-full text-sm">
                <tbody className="divide-y divide-[#21293d]">
                  ...
                  <tr className="bg-emerald-500/5 border-t-2 border-emerald-500/20">
                    <td className="px-6 py-4 text-emerald-400 font-black uppercase text-xs">
                      Net Cash Flow
                    </td>
                    <td className="px-6 py-4 text-right font-black text-emerald-400 text-lg">
                      {inr(data.net_cash_flow)}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full text-sm">
                <tbody className="divide-y divide-[#21293d]">
                  ...
                  <tr className="bg-rose-500/5 border-t-2 border-rose-500/20">
                    <td className="px-6 py-4 text-rose-400 font-black uppercase text-xs">
                      Total Business Exp
                    </td>
                    <td className="px-6 py-4 text-right font-black text-rose-400 text-lg">
                      {inr(data.total_business_expense)}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full text-sm">
                <tbody className="divide-y divide-[#21293d]">
                  ...
                  <tr className="bg-blue-500/5 border-t-2 border-blue-500/20">
                    <td className="px-6 py-4 text-blue-400 font-black uppercase text-xs">
                      Total Revenue
                    </td>
                    <td className="px-6 py-4 text-right font-black text-blue-400 text-lg">
                      {inr(data.total_revenue)}
                    </td>
                  </tr>
                  <tr>
                    <td colSpan={2} className="px-6 py-5 text-center">
                      ...
                      <p
                        className={`text-2xl font-black ${data.net_profit >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {inr(data.net_profit)}
                      </p>
                      ...
                      <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-[#21293d]">
                          {(details[modal] || []).map((row, i) => {
                            const d = (v: string) =>
                              v?.split("T")[0] || v?.split(" ")[0] || v || "";
                            if (modal === "payments")
                              return (
                                <tr key={i} className="hover:bg-white/[0.02]">
                                  <td className="px-4 py-3 text-white">{d(row.payment_date)}</td>
                            ...
                            if (modal === "advances")
                              return (
                                <tr key={i} className="hover:bg-white/[0.02]">
                                  <td className="px-4 py-3 text-white">{d(row.date_paid)}</td>
                                  <td className="px-4 py-3 text-indigo-300 font-bold">
                                    {row.staff_name}
                                  </td>
                            ...
                            if (modal === "salary")
                              return (
                                <tr key={i} className="hover:bg-white/[0.02]">
                                  <td className="px-4 py-3 text-white">{d(row.curr_date)}</td>
                                  <td className="px-4 py-3 text-indigo-300 font-bold">
                                    {row.name}
                                  </td>
                                  <td className="px-4 py-3 text-slate-400">
                              ...
                              return (
                                <tr key={i} className="hover:bg-white/[0.02]">
                                  ...
                                  <td className="px-4 py-3 text-right text-amber-400 font-bold">
                                    {inr(row.mechanic_commission_amount)}
                                  </td>
                                </tr>
                              );
                            if (modal === "discount")
                          ...
                          {modal === "commission" ? (
                            <tr>
                              ...
                              <td className="px-4 py-4 text-right font-black text-amber-400">
                                {inr(
                                  (modalTotal("commission") as { job: number; comm: number }).comm
                                )}
                              </td>
                            </tr>
                          ) : (
                            <tr>
                              ...
                              <td className="px-4 py-4 text-right font-black text-base text-white">
                                {inr(modalTotal(modal) as number)}
                              </td>
                            </tr>
                          )}
                        </tfoot>
          ...
          <div className="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-4 flex items-center gap-3 text-blue-400/80 text-xs font-bold leading-relaxed shadow-lg">
            ...
            <p>
              Accounting Logic: This report differentiates between <b>Revenue</b> (work billed) and{" "}
              <b>Cash Flow</b> (actual money collected). Total Cash In only counts actual payments
              received and spot sales to walk-in customers. Click on any row to view full details.
            </p>
          </div>
        </div>
      )}
