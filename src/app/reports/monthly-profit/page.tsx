...
export default function MonthlyProfitReport() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          pageAll(
            supabase
              ...
              .select("id, code, client_name, amount, mechanic_commission_amount, date_completed")
              .eq("status", 5)
              .gte("date_completed", start)
              .lte("date_completed", endTz)
          ),
          pageAll(
            supabase
              .from("direct_sales")
              .select("id, sale_code, client_id, total_amount, date_created")
          ...
          pageAll(
            supabase
              ...
              .select("id, client_id, amount, discount, remarks, created_at")
          ...
          pageAll(
            supabase
              .from("loan_payments")
              .select("id, lender_id, amount_paid, payment_date, remarks")
        ...
        const monthlyRecords: MonthlyData[] = months.map((mDate, idx) => {
          ...
          const commission = mJobs.reduce(
            (s, t) => s + (parseFloat(t.mechanic_commission_amount) || 0),
            0
          );

          // 2. Direct sales split (walk-in vs registered client)
          const mSales = (directSales || []).filter((d) =>
            String(d.date_created).startsWith(prefix)
          );
          const walkin = mSales
            .filter((d) => !d.client_id || d.client_id === 0 || d.client_id === "")
            .reduce((s, d) => s + (parseFloat(d.total_amount) || 0), 0);
          const clientSales = mSales
            .filter((d) => d.client_id && d.client_id !== 0 && d.client_id !== "")
            .reduce((s, d) => s + (parseFloat(d.total_amount) || 0), 0);
  ...
  const openModal = (type: string, monthNum: number, monthName: string) => {
    ...
    if (type === "repair_jobs") {
      ...
      detailData = (rawData.transactions || [])
        .map((t) => {
      ...
      detailData = (rawData.transactions || [])
        ...
        .filter(
          (t) =>
            istMonth(t.date_completed) === prefix && parseFloat(t.mechanic_commission_amount) > 0
        );
    } else if (type === "walkin_sales") {
      title = `Walk-in Direct Sales - ${monthName} ${year}`;
      detailData = (rawData.directSales || []).filter(
        (d) =>
          String(d.date_created).startsWith(prefix) &&
          (!d.client_id || d.client_id === 0 || d.client_id === "")
      );
    } else if (type === "client_sales") {
      title = `Client Direct Sales - ${monthName} ${year}`;
      detailData = (rawData.directSales || [])
        .map((d) => {
      ...
      detailData = (rawData.expenses || []).filter((e) =>
        String(e.date_created).startsWith(prefix)
      );
      ...
      detailData = (rawData.loanPayments || [])
        .map((lp) => {
      ...
      detailData = (rawData.clientPayments || [])
        .map((cp) => {
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="bg-[#0d1117] border-b border-[#21293d] text-[10px] font-black uppercase text-slate-500 tracking-widest">
                ...
                <th className="px-4 py-4 text-right" title="Direct walk-in sales">
                  Walk-in
                </th>
                <th className="px-4 py-4 text-right" title="Sales to registered clients">
                  Client Sales
                </th>
                <th className="px-4 py-4 text-right text-emerald-400" title="Total Revenue">
                  Revenue
                </th>
                <th className="px-4 py-4 text-right text-amber-400" title="Mechanic commission">
                  Commission
                </th>
                <th className="px-4 py-4 text-right" title="Shop expenses">
                  Expenses
                </th>
                <th className="px-4 py-4 text-right" title="Staff salary from attendance">
                  Salaries
                </th>
                <th className="px-4 py-4 text-right" title="Loan EMI payments">
                  EMI
                </th>
                <th className="px-4 py-4 text-right text-rose-400" title="Customer discounts given">
                  Discounts
                </th>
                <th className="px-4 py-4 text-right text-rose-400" title="Total Expenses">
                  Total Exp.
                </th>
                <th className="px-4 py-4 text-right" title="Net Profit">
                  Net Profit
                </th>
              ...
              {loading
                ...
                : data.map((row) => (
                    <tr key={row.month} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td
                        className={cellCls}
                        onClick={() => openModal("repair_jobs", row.monthNum, row.month)}
                      >
                        {inr(row.repair)}
                      </td>
                      <td
                        className={cellCls}
                        onClick={() => openModal("walkin_sales", row.monthNum, row.month)}
                      >
                        {inr(row.walkin)}
                      </td>
                      <td
                        className={cellCls}
                        onClick={() => openModal("client_sales", row.monthNum, row.month)}
                      >
                        {inr(row.clientSales)}
                      </td>
                      ...
                      <td
                        className="px-4 py-3 text-right font-bold text-amber-400 cursor-pointer hover:text-amber-300 hover:underline decoration-amber-500/50 underline-offset-4"
                        onClick={() => openModal("commission", row.monthNum, row.month)}
                      >
                        {inr(row.commission)}
                      </td>
                      <td
                        className={cellCls}
                        onClick={() => openModal("expenses", row.monthNum, row.month)}
                      >
                        {inr(row.expenses)}
                      </td>
                      <td
                        className={cellCls}
                        onClick={() => openModal("salaries", row.monthNum, row.month)}
                      >
                        {inr(row.salaries)}
                      </td>
                      <td
                        className={cellCls}
                        onClick={() => openModal("emi", row.monthNum, row.month)}
                      >
                        {inr(row.emi)}
                      </td>
                      <td
                        className={cellCls}
                        onClick={() => openModal("discounts", row.monthNum, row.month)}
                      >
                        {inr(row.discounts)}
                      </td>
                      ...
                      <td
                        className={`px-4 py-3 text-right font-black ${row.profit >= 0 ? "text-emerald-400" : "text-rose-500"}`}
                      >
                        {inr(row.profit)}
                      </td>
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="bg-[#0d1117] text-slate-400 text-[10px] uppercase tracking-widest">
                      ...
                      {modalConfig.type === "salaries" && (
                        <tr>
                          ...
                          <th className="px-4 py-3 text-center">Full Days</th>
                    ...
                    <tbody className="divide-y divide-[#21293d]">
                      {modalConfig.data.map((row, i) => {
                        if (["repair_jobs", "commission"].includes(modalConfig.type))
                          return (
                            <tr key={i} className="hover:bg-white/[0.02]">
                              <td className="px-4 py-3 text-white">
                                {new Date(row.date_completed).toLocaleDateString("en-IN", {
                                  timeZone: "Asia/Kolkata",
                                  day: "2-digit",
                                  month: "short",
                                  year: "numeric",
                              ...
                              <td className="px-4 py-3 text-right text-rose-400 font-bold">
                                {inr(parseFloat(row.amount) || 0)}
                              </td>
                              {modalConfig.type === "repair_jobs" && (
                                <td className="px-4 py-3 text-right text-amber-400 font-bold">
                                  {inr(parseFloat(row.mechanic_commission_amount) || 0)}
                                </td>
                              )}
                            </tr>
                          );
                        if (["walkin_sales", "client_sales"].includes(modalConfig.type))
                          return (
                            <tr key={i} className="hover:bg-white/[0.02]">
                              <td className="px-4 py-3 text-white">
                    ...
                    <tfoot className="bg-[#0d1117] border-t border-[#21293d]">
                      <tr>
                        ...
                        <td className="px-4 py-4 text-right text-rose-500 font-black text-base">
                          {inr(
                            modalConfig.data.reduce((sum: number, row) => {
                              if (modalConfig.type === "repair_jobs")
                                return sum + (parseFloat(row.amount) || 0);
                              if (modalConfig.type === "commission")
                                return sum + (parseFloat(row.mechanic_commission_amount) || 0);
                              if (
                                modalConfig.type === "walkin_sales" ||
                                modalConfig.type === "client_sales"
                              )
                              ...
                              if (modalConfig.type === "expenses")
                                return sum + (parseFloat(row.amount) || 0);
                              if (modalConfig.type === "salaries")
                                return sum + (row.full + row.half * 0.5) * row.rate;
                              if (modalConfig.type === "emi")
                                return sum + (parseFloat(row.amount_paid) || 0);
                              if (modalConfig.type === "discounts")
...
function StatCard({
  ...
  return (
    <div className="bg-[#161b27] border border-[#21293d] rounded-3xl p-5 shadow-xl group hover:border-indigo-500/30 transition-all">
      ...
      <h3
        className={`text-lg font-black tracking-tight ${isProfit ? (value >= 0 ? "text-emerald-400" : "text-rose-400") : "text-white"}`}
      >
        {inr(value)}
      </h3>
    </div>
