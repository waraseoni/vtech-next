...
function LoanReportContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: loans } = await pageAll(
        supabase
          .from("client_loans")
          .select(
      ...
      const { data: payments } = await pageAll(
        supabase
      ...
      for (const l of loans || []) {
        ...
        loanRows.push({
          ...
          remaining_installments:
            targetEmi > 0
              ? Math.ceil(
                  ((l.total_payable || 0) - received > 0 ? (l.total_payable || 0) - received : 0) /
                    targetEmi
                )
              : 0,
          status: l.status || 0,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">Client loans monthly performance</p>
        </div>
        <button
          onClick={() => window.open(`/api/print-loan?month=${month}`, "_blank")}
      ...
      <div className="bg-panel border border-app rounded-2xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto text-sm font-black text-white">
            Loan Performance — {monthLabel}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="bg-panel border border-app rounded-2xl p-4 text-center">
          ...
          <p className="text-base font-black text-app-2 mt-1">{inr(tTarget)}</p>
        </div>
        <div className="bg-panel border border-app rounded-2xl p-4 text-center">
          <p className="text-[10px] font-black uppercase text-muted-2 tracking-widest">
            Received
        ...
        <div className="bg-panel border border-app rounded-2xl p-4 text-center">
          <p className="text-[10px] font-black uppercase text-muted-2 tracking-widest">Pending</p>
          <p className="text-base font-black text-red-400 mt-1">{inr(tPending)}</p>
          <p className="text-[10px] font-black uppercase text-muted-2 tracking-widest mt-1">
            Inst. Left
          </p>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-12 text-muted-2 text-xs font-bold">
                    No loan data found
                  </td>
                </tr>
              ) : (
                rows.map((r, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-xs font-bold text-app-2">
                      {r.client_name}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-right text-app-2">
                      {inr(r.principal_amount)}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-center text-muted">
                      {r.interest_rate}%
                    </td>
                    <td className="px-3 py-2.5 text-xs text-right text-emerald-400">
                      {inr(r.total_payable - r.principal_amount)}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-right text-app-2">
                      {inr(r.emi_amount)}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-xs text-center font-bold text-yellow-400">
            ...
            <tfoot>
              <tr className="border-t-2 border-blue-500/30 bg-blue-500/5">
                <td colSpan={2} className="px-3 py-3 text-xs font-black text-muted text-right">
                  Total:
                </td>
                <td className="px-3 py-3 text-xs text-right font-black text-blue-400">
                  {inr(tPrincipal)}
                </td>
                <td />
                <td className="px-3 py-3 text-xs text-right font-black text-emerald-400">
                  {inr(tInterest)}
                </td>
                <td className="px-3 py-3 text-xs text-right font-black text-app-2">
                  {inr(tTarget)}
                </td>
                <td className="px-3 py-3 text-xs text-right font-black text-teal-400">
                  {inr(tReceived)}
                </td>
                <td className="px-3 py-3 text-xs text-right font-black text-red-400">
                  {inr(tPending)}
                </td>
              </tr>
            </tfoot>
