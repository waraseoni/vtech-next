...
const STATUS_META: Record<
  DueRow["status_key"],
  { label: string; icon: React.ReactNode; card: string; chip: string }
> = {
  overdue: {
    label: "Overdue",
    icon: <AlertTriangle size={20} />,
    card: "from-red-600 to-red-800",
  ...
  future: {
    ...
    card: "from-slate-500 to-slate-700",
    chip: "bg-muted/15 text-app-2 border-muted/30",
  },
  no_date: {
    label: "No Date",
    icon: <CalendarOff size={20} />,
    card: "from-slate-700 to-slate-900",
    chip: "bg-muted/15 text-muted border-muted/30",
  },
};
...
function DueRemindersContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: clients, error } = await supabase
        .from("client_list")
        .select(
          "id, firstname, middlename, lastname, contact, opening_balance, payment_due_date, payment_due_remarks, image_path"
        )
        .eq("delete_flag", 0);
      ...
      const built: DueRow[] = clients.map((c) => {
        ...
        if (c.payment_due_date) {
          ...
          status_key =
            diff < 0 ? "overdue" : diff === 0 ? "today" : diff <= 7 ? "upcoming" : "future";
        }
  ...
  const STAT_CARDS = [
    {
      key: "all",
      label: "Clients With Due",
      val: stats.total,
      amt: stats.amount,
      card: "from-violet-600 to-violet-800",
    ...
    },
    {
      ...
      val: stats.overdue,
      amt: 0,
      card: STATUS_META.overdue.card,
      icon: STATUS_META.overdue.icon,
    },
    {
      key: "today",
      label: "Due Today",
      val: stats.today,
      amt: 0,
      card: STATUS_META.today.card,
      icon: STATUS_META.today.icon,
    },
    {
      key: "upcoming",
      label: "Upcoming (7d)",
      val: stats.upcoming,
      amt: 0,
      card: STATUS_META.upcoming.card,
    ...
    },
    {
      ...
      val: stats.no_date,
      amt: 0,
      card: STATUS_META.no_date.card,
      icon: STATUS_META.no_date.icon,
    },
  ];
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-20">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-muted font-bold uppercase tracking-[0.3em]">
                Promised Payment Dues
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="bg-app border border-red-500/20 px-6 py-3 rounded-2xl flex flex-col items-end">
      ...
      {/* Filters */}
      <div className="bg-panel border border-app rounded-[2rem] p-5 flex flex-wrap items-end gap-6">
        <form onSubmit={handleFilter} className="flex flex-wrap items-end gap-6 flex-1">
          <div className="space-y-2">
            ...
            <select
              ...
              className="px-6 py-3 bg-app border border-app rounded-2xl text-sm text-app-2 outline-none focus:border-red-500 transition-all cursor-pointer"
            >
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="px-4 py-3 bg-app border border-app rounded-2xl text-sm text-app-2 outline-none focus:border-red-500 transition-all [color-scheme:dark]"
            />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-muted-2 tracking-[0.2em] ml-2">
          ...
          <button
            type="submit"
            className="px-8 py-3.5 bg-red-600 hover:bg-red-500 text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-red-600/20"
          >
            Refresh Report
          </button>
        </form>
        <div className="relative w-full lg:w-72">
          ...
          <input
            ...
            className="w-full pl-12 pr-6 py-3 bg-app border border-app rounded-2xl text-sm text-app-2 outline-none focus:border-red-500 transition-all"
          />
        </div>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center text-muted-2 italic">
                    Koi client dues ke saath nahi mila. 🎉
                  </td>
                </tr>
              ) : (
                filtered.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <Link
                              ...
                              className="text-white font-black hover:text-red-400 transition-colors"
                            >
                              {r.name}
                            </Link>
                      ...
                      <td className="px-6 py-5 text-right">
                        ...
                        <div className="text-[9px] text-muted-2 mt-0.5 font-semibold">
                          Open {inr(r.opening_balance)} · Rep {inr(r.total_repairs)} · DS{" "}
                          {inr(r.total_direct_sales)} · Loan {inr(r.total_loans)} · Paid{" "}
                          {inr(r.total_payments)}
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        {r.payment_due_date ? (
                          <span className="font-bold text-app-2">
                            {new Intl.DateTimeFormat("en-IN", {
                              ...
                              month: "short",
                              year: "numeric",
                            }).format(parseISTDate(r.payment_due_date))}
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-5">
                        <span
                          className={`px-3 py-1 rounded-lg text-[10px] font-black border uppercase tracking-wider ${meta.chip}`}
                        >
                          {meta.label}
                        </span>
                      </td>
                      <td className="px-6 py-5">
                        <span
                          className="text-xs text-muted max-w-[180px] truncate block"
                          title={r.payment_due_remarks || ""}
                        >
                          {r.payment_due_remarks || <span className="text-muted-2">—</span>}
                        </span>
                      </td>
                        ...
                        <span className="text-xs text-muted">
                          {r.last_reminder ? (
                            new Intl.DateTimeFormat("en-IN", {
                              day: "2-digit",
                              month: "short",
                            }).format(new Date(r.last_reminder))
                          ) : (
                            <span className="text-muted-2">Never</span>
                          )}
                          <span className="block text-[9px] text-muted-2">
                            {r.reminder_count} total
                          </span>
                        </span>
                      </td>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {r.payment_due_date && (
                            <button
                              onClick={async () => {
                                if (
                                  confirm(`Kya aap ${r.name} ki due date clear karna chahte hain?`)
                                ) {
                                  setDueModal({ client: r });
                                  setDueForm({ due_date: "", due_remarks: "" });
                                  await saveDueDate(true);
                                }
                              }}
                              title="Clear Due Date"
                              className="p-2.5 bg-muted/10 border border-muted/20 text-muted rounded-xl hover:bg-muted hover:text-white transition-all shadow-sm"
                            >
                              <X size={16} />
                            </button>
                          )}
          ...
          <div className="w-full max-w-md rounded-2xl border p-6 shadow-2xl bg-panel border-app">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Calendar size={16} className="text-red-400" /> Set Due Date —{" "}
                <span className="text-muted font-semibold">{dueModal.client.name}</span>
              </h3>
              <button
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-xl border text-sm bg-app border-app text-white focus:outline-none focus:border-red-500 transition-colors [color-scheme:dark]"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-muted mb-1.5">
                  Promise Note / Remarks
                </label>
                <textarea
                  rows={3}
                  ...
                  className="w-full px-3 py-2.5 rounded-xl border text-sm bg-app border-app text-white focus:outline-none focus:border-red-500 transition-colors resize-none"
                />
              </div>
              <div className="flex gap-3 pt-1">
                <button
                  onClick={() => saveDueDate(false)}
                  disabled={savingDue}
                ...
                >
                  {savingDue ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}{" "}
                  Save Due Date
                </button>
                <button
                  onClick={() => setDueModal(null)}
                  className="flex-1 py-2.5 rounded-xl font-bold text-sm border border-app-2 text-app-2 hover:bg-white/5 transition-all"
                >
                  Cancel
                </button>
              </div>
