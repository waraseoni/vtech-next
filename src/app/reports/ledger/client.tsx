...
function TBadge({ type }: { type: "Cash In" | "Cash Out" }) {
  return type === "Cash In" ? (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full text-[9px] font-extrabold">
      {type}
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-full text-[9px] font-extrabold">
      {type}
    </span>
  );
}
...
export default function LedgerReportClient({ fromDate, toDate }: Props) {
  ...
  const totalBusinessExpense =
    totalSalary + totalCommission + totalOtherExpenses + totalEmiPaid + totalDiscountGiven;
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] pb-20 font-sans">
      ...
      <div className="relative overflow-hidden bg-[#0d1117] border-b border-[#21293d]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
            <StatCard
              label="Net Revenue"
              value={rupee(totalIncome)}
              sub="Repair + all sales"
              icon={TrendingUp}
            ...
            <StatCard
              ...
              value={rupee(totalBusinessExpense)}
              sub="Salary+Comm+Exp+EMI"
              icon={TrendingDown}
            ...
            <StatCard
              label="Cash Received"
              value={rupee(totalCashInflow)}
              sub="Payments + walk-in"
              icon={Wallet}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 space-y-4">
            ...
            <table className="w-full">
              <tbody>
                ...
                <tr className={netProfit >= 0 ? "bg-emerald-500/10" : "bg-yellow-500/10"}>
                  ...
                  <td
                    className={`px-4 py-3 text-sm font-black text-right tabular-nums ${netProfit >= 0 ? "text-emerald-400" : "text-yellow-400"}`}
                  >
                    {rupee(netProfit, 2)}
                  </td>
                </tr>
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl overflow-hidden">
            ...
            <table className="w-full">
              <tbody>
                ...
                <PRow
                  ...
                  value={clientPaymentsReceived}
                  colorClass="text-emerald-400/80"
                  onClick={() => openModal("clientpay")}
                />
                <PRow
                  label="Walk-in Direct Sales (Cash)"
                  value={walkinIncome}
                  colorClass="text-emerald-400/80"
                  onClick={() => openModal("walkin")}
                />
                ...
                <PRow
                  label="Staff Advance / Salary Paid"
                  value={totalAdvanceGiven}
                  colorClass="text-red-400/80"
                  onClick={() => openModal("advance")}
                />
                <PRow
                  label="Shop Expenses Paid"
                  value={totalOtherExpenses}
                  colorClass="text-red-400/80"
                  onClick={() => openModal("shopexp")}
                />
                ...
                <tr className="bg-blue-500/10">
                  ...
                  <td
                    className={`px-4 py-3 text-sm font-black text-right tabular-nums ${netCash >= 0 ? "text-blue-400" : "text-red-400"}`}
                  >
                    {rupee(netCash, 2)}
                  </td>
                </tr>
            ...
            <div className="mx-4 mb-4 mt-3 bg-amber-500/5 border border-amber-500/15 rounded-xl p-3">
              ...
              <ul className="space-y-0.5 text-[10px] text-slate-600 list-disc list-inside">
                ...
                <li>
                  <Eye size={9} className="inline" /> icon click karo details ke liye.
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="bg-[#161b27] border border-[#21293d] rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#21293d] bg-[#111520]">
            ...
            <div className="flex gap-1.5">
              <span className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full text-[9px] font-extrabold">
                Cash In
              </span>
              <span className="px-2 py-0.5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-full text-[9px] font-extrabold">
                Cash Out
              </span>
            </div>
          </div>
                        ...
                        >
                          ...
                          <td className={`${tdCls} max-w-xs`}>
                            ...
                            {toNum(e.discount_amount) > 0 && (
                              <span className="text-red-400/60 text-[10px] ml-1">
                                (-{rupee(toNum(e.discount_amount), 2)})
                              </span>
                            )}
                          </td>
                          <td
                            className={`${tdCls} text-right tabular-nums text-emerald-400 font-bold`}
                          >
                          ...
                          <td
                            className={`${tdCls} text-right tabular-nums font-black ${bal >= 0 ? "text-blue-400" : "text-red-400"}`}
                          >
                            {rupee(bal, 2)}
                          </td>
                        </tr>
          ...
          <div className="flex flex-wrap justify-between items-center gap-3 px-4 py-3 border-t border-[#21293d] bg-[#111520]">
            {[
              ...
              { label: "Total Cash Out", value: totalCashOutflow, cls: "text-red-400" },
              {
                ...
                value: netCash,
                cls: netCash >= 0 ? "text-blue-400" : "text-red-400",
              },
            ].map(({ label, value, cls }) => (
          ...
          {[
            {
              ...
              renderRow: (e: Expense, i: number) => (
                <tr key={i} className={trCls}>
                  ...
                  <td className={`${tdCls} text-right text-red-400 font-bold tabular-nums`}>
                    {rupee(toNum(e.amount), 2)}
                  </td>
                </tr>
              ),
              total: totalOtherExpenses,
              totalCls: "text-red-400",
            },
            {
              ...
              renderRow: (a: AdvancePayment, i: number) => (
                <tr key={i} className={trCls}>
                  ...
                  <td className={`${tdCls} text-right text-amber-400 font-bold tabular-nums`}>
                    {rupee(toNum(a.amount), 2)}
                  </td>
                </tr>
              ),
              total: totalAdvanceGiven,
              totalCls: "text-amber-400",
            },
                  ...
                  {rows.length > 0 && (
                    <tfoot>
                      <tr className="border-t border-[#21293d] bg-[#111520]">
                        ...
                        <td
                          className={`px-3 py-2.5 text-xs font-black text-right tabular-nums ${totalCls}`}
                        >
                          {rupee(total, 2)}
                        </td>
                      </tr>
            ...
            <table className="w-full">
              <tbody>
                ...
                <tr className="bg-indigo-500/10">
                  ...
                  <td
                    className={`px-4 py-3 text-sm font-black text-right tabular-nums ${capital >= 0 ? "text-indigo-400" : "text-red-400"}`}
                  >
                    {rupee(capital, 2)}
                  </td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[10px] font-mono">
            <div className="bg-slate-900/50 rounded-lg p-2">
              ...
              <div className="text-slate-500 space-y-0.5">
                ...
                <div>
                  <span className="text-red-400">Total Expenses</span> = Staff Salary + Mechanic
                  Commission + Shop Expenses + Loan EMI + Discount
                </div>
                <div>
                  <span className="text-cyan-400">Net Profit</span> = Net Revenue − Total Expenses
                </div>
              </div>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-2">
              ...
              <div className="text-slate-500 space-y-0.5">
                ...
                <div>
                  <span className="text-red-400">Total Cash Out</span> = Staff Advance + Shop
                  Expenses + Loan EMI
                </div>
                <div>
                  <span className="text-amber-400">Net Cash Flow</span> = Total Cash In − Total Cash
                  Out
                </div>
              </div>
            </div>
        ...
        <div className="no-print">
          ...
          >
            ...
            <ChevronRight
              size={12}
              className={`transition-transform ${showStock ? "rotate-90" : ""}`}
            />
          </button>
        </div>
              ...
              <table className="w-full border-collapse">
                ...
                <tfoot>
                  <tr className="border-t border-[#21293d] bg-[#111520]">
                    ...
                    <td className="px-3 py-2.5 text-xs font-black text-right text-blue-400 tabular-nums">
                      {rupee(stockValue, 2)}
                    </td>
                  </tr>
                </tfoot>
      ...
      {modal === "repair" && (
        <Modal
          title={`Repair Jobs — ${displayFrom} to ${displayTo}`}
          icon={Wrench}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Job Code", "Client", "Items", "Mechanic", "Amount", "Commission", "Completed"].map(
                (h) => (
            ...
            {repairJobs.length === 0 ? (
              ...
              repairJobs.map((j) => (
                <tr key={j.id} className={trCls}>
                  ...
                  <td className={tdCls}>
                    {[j.client_firstname, j.client_middlename, j.client_lastname]
                      .filter(Boolean)
                      .join(" ")}
                  </td>
                  <td className={tdCls}>{j.item}</td>
                  <td className={tdCls}>
                    {j.mechanic_firstname} {j.mechanic_lastname}
                  </td>
                  <td className={`${tdCls} text-right text-emerald-400 font-bold tabular-nums`}>
                    {rupee(toNum(j.amount), 2)}
                  </td>
                  <td className={`${tdCls} text-right text-amber-400 font-bold tabular-nums`}>
                    {rupee(toNum(j.mechanic_commission_amount), 2)}
                  </td>
                  <td className={`${tdCls} whitespace-nowrap`}>
                    {safeFormatDate(j.date_completed)}
                  </td>
                </tr>
              ))
            )}
          ...
          {repairJobs.length > 0 && (
            <tfoot>
              <tr className="border-t border-[#21293d] bg-[#111520]">
                ...
                <td className={`${tdCls} text-right text-emerald-400 font-black tabular-nums`}>
                  {rupee(jobIncome, 2)}
                </td>
                <td className={`${tdCls} text-right text-amber-400 font-black tabular-nums`}>
                  {rupee(totalCommission, 2)}
                </td>
                <td />
              </tr>
            </tfoot>
      ...
      {modal === "walkin" && (
        <Modal
          title={`Walk-in Sales — ${displayFrom} to ${displayTo}`}
          icon={ShoppingCart}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Invoice", "Product", "Qty", "Unit Price", "Total", "Date"].map((h) => (
                <th key={h} className={thCls}>
                  {h}
                </th>
              ))}
            ...
            {walkinSales.length === 0 ? (
              ...
              walkinSales.map((s) => (
                <tr key={s.id} className={trCls}>
                  ...
                  <td className={`${tdCls} text-right tabular-nums`}>
                    {s.unit_price ? rupee(toNum(s.unit_price), 2) : "—"}
                  </td>
                  <td className={`${tdCls} text-right text-emerald-400 font-bold tabular-nums`}>
                    {rupee(toNum(s.total_amount), 2)}
                  </td>
      ...
      {modal === "clientsales" && (
        <Modal
          title={`Client Sales — ${displayFrom} to ${displayTo}`}
          icon={Users}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Invoice", "Client", "Product", "Qty", "Unit Price", "Total", "Date"].map((h) => (
                <th key={h} className={thCls}>
                  {h}
                </th>
              ))}
            ...
            {clientSales.length === 0 ? (
              ...
              clientSales.map((s) => (
                <tr key={s.id} className={trCls}>
                  <td className={tdCls}>{s.sale_code}</td>
                  <td className={tdCls}>
                    {s.client_firstname} {s.client_lastname}
                  </td>
                  <td className={tdCls}>{s.product_name || "Multiple Items"}</td>
                  <td className={`${tdCls} text-right`}>{s.quantity || "—"}</td>
                  <td className={`${tdCls} text-right tabular-nums`}>
      ...
      {modal === "clientpay" && (
        <Modal
          title={`Client Payments — ${displayFrom} to ${displayTo}`}
          icon={CreditCard}
          onClose={closeModal}
        >
          <thead>
            ...
            {clientPayments.length === 0 ? (
              ...
              clientPayments.map((p) => (
                <tr key={p.id} className={trCls}>
                  <td className={tdCls}>
                    {p.client_firstname} {p.client_lastname}
                  </td>
                  <td className={`${tdCls} whitespace-nowrap`}>{safeFormatDate(p.payment_date)}</td>
                  <td className={`${tdCls} text-right tabular-nums`}>
                    {rupee(toNum(p.amount), 2)}
                  </td>
                  <td className={`${tdCls} text-right text-red-400 tabular-nums`}>
                    {rupee(toNum(p.discount), 2)}
                  </td>
                  <td className={`${tdCls} text-right text-emerald-400 font-bold tabular-nums`}>
                    {rupee(toNum(p.amount) + toNum(p.discount), 2)}
                  </td>
          ...
          {clientPayments.length > 0 && (
            <tfoot>
              <tr className="border-t border-[#21293d] bg-[#111520]">
                ...
                <td className={`${tdCls} text-right text-emerald-400 font-black tabular-nums`}>
                  {rupee(clientPaymentsReceived, 2)}
                </td>
                <td className={`${tdCls} text-right text-red-400 font-black tabular-nums`}>
                  {rupee(totalDiscountGiven, 2)}
                </td>
                <td colSpan={3} />
              </tr>
            </tfoot>
      ...
      {modal === "commission" && (
        <Modal
          title={`Mechanic Commission — ${displayFrom} to ${displayTo}`}
          icon={Award}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Job Code", "Mechanic", "Job Amount", "Commission", "Comm %", "Completed"].map(
                (h) => (
                  <th key={h} className={thCls}>
                    {h}
                  </th>
            ...
            {commissionData.length === 0 ? (
              ...
              commissionData.map((c, i) => {
                const amt = toNum(c.amount),
                  comm = toNum(c.mechanic_commission_amount);
                return (
                  <tr key={i} className={trCls}>
                    <td className={tdCls}>{c.job_id}</td>
                    <td className={tdCls}>
                      {c.mechanic_firstname} {c.mechanic_lastname}
                    </td>
                    ...
                    <td className={`${tdCls} text-right`}>
                      {amt > 0 ? ((comm / amt) * 100).toFixed(1) : 0}%
                    </td>
                    <td className={`${tdCls} whitespace-nowrap`}>
                      {safeFormatDate(c.date_completed)}
                    </td>
                  </tr>
                );
              })
      ...
      {modal === "discount" && (
        <Modal
          title={`Customer Discounts — ${displayFrom} to ${displayTo}`}
          icon={Tag}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Client", "Date", "Amount Paid", "Discount", "Disc %", "Remarks"].map((h) => (
                <th key={h} className={thCls}>
                  {h}
                </th>
              ))}
                ...
                .map((p) => {
                  ...
                  return (
                    <tr key={p.id} className={trCls}>
                      ...
                      <td className={`${tdCls} text-right`}>
                        {amt + disc > 0 ? ((disc / (amt + disc)) * 100).toFixed(1) : 0}%
                      </td>
                      <td className={tdCls}>{p.remarks || "—"}</td>
                    </tr>
                  );
      ...
      {modal === "salaries" && (
        <Modal
          title={`Staff Salaries — ${displayFrom} to ${displayTo}`}
          icon={Users}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Mechanic", "Full Days", "Half Days", "Total Days", "Daily Rate", "Earned"].map(
                (h) => (
                  <th key={h} className={thCls}>
                    {h}
                  </th>
            ...
            {salaryDetails.length === 0 ? (
              ...
              salaryDetails.map((s, i) => (
                <tr key={i} className={trCls}>
                  ...
                  <td className={`${tdCls} text-right tabular-nums`}>
                    {rupee(toNum(s.daily_salary), 2)}
                  </td>
                  <td className={`${tdCls} text-right text-amber-400 font-bold tabular-nums`}>
                    {rupee(toNum(s.salary_earned), 2)}
                  </td>
                </tr>
              ))
            )}
      ...
      {modal === "advance" && (
        <Modal
          title={`Staff Advance Payments — ${displayFrom} to ${displayTo}`}
          icon={Users}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Date", "Staff", "Amount", "Reason", "Mode"].map((h) => (
                <th key={h} className={thCls}>
                  {h}
                </th>
              ))}
      ...
      {modal === "shopexp" && (
        <Modal
          title={`Shop Expenses — ${displayFrom} to ${displayTo}`}
          icon={Building2}
          onClose={closeModal}
        >
          <thead>
            <tr>
              {["Date", "Category", "Description", "Amount", "Mode", "Reference"].map((h) => (
                <th key={h} className={thCls}>
                  {h}
                </th>
              ))}
