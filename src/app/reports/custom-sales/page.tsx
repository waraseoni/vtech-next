    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="bg-panel border border-app rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-3 justify-between">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 shrink-0 bg-app border border-app rounded-xl text-xs font-bold text-app-2 hover:text-white hover:border-blue-500/40 hover:bg-panel-2 disabled:opacity-40 disabled:pointer-events-none transition-all"
        >
      ...
      {/* Range filter */}
      <div className="bg-panel border border-app rounded-2xl p-3 sm:p-4 space-y-3">
        <div className="flex flex-wrap items-end gap-2 sm:gap-3">
          <div>
            ...
            <div className="flex items-center gap-2 bg-app border border-app rounded-xl px-3 py-2">
              <Calendar size={13} className="text-blue-400/70 shrink-0" />
              <input
                type="date"
                value={from}
                onChange={(e) => setFrom(e.target.value)}
                className="bg-transparent text-sm font-bold text-app-2 outline-none [color-scheme:dark] w-full min-w-0"
              />
            </div>
          </div>
          <div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {presets.map((p) => (
            <button
              ...
              className="px-3 py-1.5 bg-app border border-app rounded-full text-[11px] font-bold text-muted hover:text-white hover:border-blue-500/40 transition"
            >
              {p.label}
            </button>
          ))}
          <span className="w-px h-4 bg-panel-2 mx-1 hidden sm:inline-block" />
          <button
            onClick={() => shiftMonth(-1)}
            className="inline-flex items-center gap-1 px-3 py-1.5 bg-app border border-app rounded-full text-[11px] font-bold text-muted hover:text-white hover:border-blue-500/40 transition"
          >
          ...
          <button
            onClick={resetRange}
            className="inline-flex items-center gap-1 px-3 py-1.5 bg-app border border-app rounded-full text-[11px] font-bold text-muted hover:text-white hover:border-amber-500/40 transition"
          >
      ...
      {/* Desktop table */}
      <div className="hidden md:block overflow-hidden rounded-2xl border border-app">
        {loading ? (
          <LoadingBlock label="Loading sales..." />
        ) : rows.length === 0 ? (
          <EmptyBlock message="No sales found in this range." />
        ) : (
              ...
              <tbody className="divide-y divide-[#1a2234]">
                {rows.map((r, i) => (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 font-bold text-app-2">{r.product_name}</td>
                    <td className="px-4 py-3 text-right text-app-2">{inr(r.price)}</td>
                    <td className="px-4 py-3 text-right font-bold text-app-2">{r.qty}</td>
              ...
              <tfoot className="sticky bottom-0 z-10 bg-panel-2">
                <tr className="text-[10px] font-black uppercase tracking-widest text-muted">
                  <th className="px-4 py-3 text-right" colSpan={5}>
                    Total ({fmtShort(from)} — {fmtShort(to)})
                  </th>
                  <th className="px-4 py-3 text-right text-app-2">{totalQty}</th>
      ...
      <div className="md:hidden">
        {loading ? (
          ...
          <div className="space-y-3">
            {rows.map((r, i) => (
              <div key={i} className="bg-panel border border-app rounded-2xl p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-black text-app-2 truncate">{r.product_name}</p>
                ...
                <div className="mt-3 pt-3 border-t border-app-2 space-y-2 text-[11px]">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-muted">
                      Qty <span className="text-app-2 font-bold">{r.qty}</span>
                    </span>
                    <span className="text-muted">
                      Rate <span className="text-app-2 font-bold">{inr(r.price)}</span>
                    </span>
                  </div>
                </div>
              </div>
