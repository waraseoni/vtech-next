...
function FinancialReportContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: txRangeData } = await pageAll(
        supabase
      ...
      const [svcRes, partsRes, directRes, expenseRes, emiRes, advanceRes, stockAddRes] =
        await Promise.all([
          ...
          pageAll(
            supabase
              .from("inventory_list")
              .select("product_id, quantity, stock_date")
      ...
      const stock_added_val = (stockAddRes.data || []).reduce(
        (s: number, r) => s + (r.quantity || 0) * (priceMap.get(r.product_id) || 0),
        0
      );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#161b27] border border-[#21293d] rounded-2xl p-4 no-print shadow-xl">
        <form onSubmit={handleFilter} className="flex flex-wrap items-end gap-4">
          ...
          <button
            type="submit"
            className="px-8 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-sm font-black uppercase tracking-wider transition-all shadow-lg shadow-purple-600/20 active:scale-95"
          >
            Update Analysis
          </button>
        </form>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#21293d]">
                    ...
                    <tr className="bg-red-500/5">
                      <td className="px-6 py-4 text-red-400 font-black uppercase text-xs">
                        Total Expenses (B)
                      </td>
                      <td className="px-6 py-4 text-right font-black text-red-400 text-lg">
                        {inr(totalCashExpenses)}
                      </td>
                    </tr>
                    ...
                    <tr className="bg-[#111520] border-t-2 border-purple-500/40">
                      ...
                      <td className="px-6 py-6 text-right font-black text-white text-2xl tracking-tighter">
                        {inr(cashProfit)}
                      </td>
                    </tr>
                  </tbody>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    <p className="text-2xl font-black">
                      {inr(data.current_stock_val - data.loan_pending)}
                    </p>
                    <p className="text-[10px] font-bold uppercase opacity-60">
                      Net Asset (Stock - Debt)
                    </p>
                  </div>
