...
function CashFlowPageInner() {
  ...
  const stats = useMemo(() => {
    ...
    const outflow =
      (data.totalAdvanceGiven || 0) + (data.totalOtherExpenses || 0) + (data.totalEmiPaid || 0);
  ...
  const navigateMonth = (dir: "prev" | "next") => {
    ...
    setFrom(startOfMonthIST(base));
    setTo(endOfMonthIST(base));
    setViewMode("month");
  };
                  ...
                  <div key={i} className="group relative">
                    <div className="relative bg-[#161b27] border border-[#21293d] rounded-3xl p-6 h-full transition-transform duration-300 group-hover:-translate-y-1 shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${cm[stat.color]} html[data-theme='light'] &:${lightColorMap[stat.color]}`}
                        >
                          <stat.icon size={18} />
                        </div>
                      </div>
              ...
              <div className="bg-[#161b27] border border-[#21293d] rounded-3xl overflow-hidden">
                ...
                <table className="w-full">
                  <tbody>
                    {[
                      {
                        label: "Client Payments (Recovery)",
                        value: data?.clientPaymentsReceived || 0,
                      },
                    ...
                    <tr className="bg-emerald-500/10 border-t border-[#21293d]">
                      ...
                      <td className="px-5 py-4 text-base font-black text-right text-emerald-400 tabular-nums">
                        {rupee(stats.inflow)}
                      </td>
                    </tr>
                  </tbody>
                ...
                <table className="w-full">
                  <tbody>
                    ...
                    <tr className="bg-rose-500/10 border-t border-[#21293d]">
                      ...
                      <td className="px-5 py-4 text-base font-black text-right text-rose-400 tabular-nums">
                        {rupee(stats.outflow)}
                      </td>
                    </tr>
                  </tbody>
                    ...
                    {sortedEntries.length === 0 ? (
                      ...
                      <>
                        {sortedEntries.map((entry, idx) => {
                          const prevBal = sortedEntries
                            .slice(0, idx)
                            .reduce(
                              (s, e) => s + (e.type === "Cash In" ? e.net_amount : -e.net_amount),
                              0
                            );
                          const bal =
                            prevBal +
                            (entry.type === "Cash In" ? entry.net_amount : -entry.net_amount);
                          return (
                            <tr
                              key={idx}
                            ...
                            >
                              ...
                              <td className="px-6 py-4">
                                <span
                                  className={`text-[9px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest ${
                                    entry.type === "Cash In"
                              ...
                              <td
                                className={`px-6 py-4 text-sm text-right font-black tabular-nums ${bal >= 0 ? "text-blue-400" : "text-rose-400"}`}
                              >
                                {rupee(bal)}
                              </td>
                            </tr>
                        ...
                        <tr className="border-t border-[#21293d] bg-[#111520]">
                          ...
                          <td className="px-6 py-5 text-sm text-right font-black text-emerald-400 tabular-nums">
                            {rupee(stats.inflow)}
                          </td>
                          <td className="px-6 py-5 text-sm text-right font-black text-rose-400 tabular-nums">
                            {rupee(stats.outflow)}
                          </td>
                          <td
                            className={`px-6 py-5 text-sm text-right font-black tabular-nums ${stats.net >= 0 ? "text-blue-400" : "text-rose-400"}`}
                          >
                            {rupee(stats.net)}
                          </td>
                        </tr>
