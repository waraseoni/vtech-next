...
function VyaparDarpanContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        deliveredTxIds.length > 0
          ? pageAll(
              supabase
                ...
                .select("qty, price")
                .in("transaction_id", deliveredTxIds)
            )
        ...
        pageAll(
          supabase
            ...
            .select("status, mechanic_list(daily_salary)")
            .gte("curr_date", from)
            .lte("curr_date", to)
        ),
        // 8. Discounts
      ...
      const { data: invAll } = await pageAll(
        supabase.from("inventory_list").select("product_id, quantity")
      );
      const { data: soldAll } = await pageAll(
        supabase.from("transaction_products").select("product_id, qty")
      );
      const { data: dsAll } = await pageAll(
        supabase.from("direct_sale_items").select("product_id, qty")
      );

      const invMap: Record<number, number> = {};
      (invAll || []).forEach(
        (r) => (invMap[r.product_id] = (invMap[r.product_id] || 0) + (r.quantity || 0))
      );
      const soldMap: Record<number, number> = {};
      (soldAll || []).forEach(
        (r) => (soldMap[r.product_id] = (soldMap[r.product_id] || 0) + (r.qty || 0))
      );
      (dsAll || []).forEach(
        (r) => (soldMap[r.product_id] = (soldMap[r.product_id] || 0) + (r.qty || 0))
      );
      ...
      (loanPaysRes.data || []).forEach(
        (p) => (paysByLender[p.lender_id] = (paysByLender[p.lender_id] || 0) + (p.amount_paid || 0))
      );
      ...
      setData({
        total_sales:
          (repairRes.data || []).reduce((s, r) => s + (r.amount || 0), 0) +
          (directRes.data || []).reduce((s, r) => s + (r.total_amount || 0), 0),
        parts_sell_value:
          (repPartsRes.data || []).reduce((s, r) => s + (r.qty || 0) * (r.price || 0), 0) +
          (dsPartsRes.data || []).reduce((s, r) => s + (r.qty || 0) * (r.price || 0), 0),
  ...
  const estimatedPartsCost = data.parts_sell_value * 0.9;
  const grossProfit = data.total_sales - estimatedPartsCost;
  const totalIndirectExpenses =
    data.shop_expenses + data.emi_paid + data.staff_salary + data.discounts;
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-[0.3em]">
                Business Mirror & Performance Analysis
              </p>
            </div>
          </div>
          <button
            onClick={() => window.print()}
            className="px-6 py-3 bg-[#1e2637] border border-[#2a3550] hover:border-indigo-500/40 text-slate-400 hover:text-white rounded-2xl text-sm font-black uppercase tracking-widest transition-all flex items-center gap-3 shadow-lg"
          >
      ...
      <div className="bg-[#161b27] border border-[#21293d] rounded-[2rem] p-5 no-print shadow-2xl flex flex-wrap items-end gap-6">
        <form onSubmit={handleFilter} className="flex flex-wrap items-end gap-6 w-full">
          ...
          <button
            type="submit"
            className="px-10 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-indigo-600/20 active:scale-95"
          >
            Refresh Analysis
          </button>
        </form>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div
                key={i}
                className="bg-[#161b27] border border-[#21293d] rounded-3xl p-5 shadow-xl group hover:border-indigo-500/30 transition-all"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`p-2 rounded-xl bg-${m.color}-500/10 border border-${m.color}-500/20`}
                  >
                    {m.icon}
                  </div>
            ...
            <div className="lg:col-span-8 space-y-6">
                ...
                <div className="p-8 space-y-6">
                  <div className="flex justify-between items-end border-b border-white/5 pb-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-black text-red-500/80 mt-1">
                        -{inr(estimatedPartsCost)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between bg-blue-500/5 p-6 rounded-2xl border border-blue-500/10 shadow-inner">
                    <h4 className="text-sm font-black text-blue-300 uppercase tracking-widest">
                      Gross Operating Profit
                    </h4>
                    <h4 className="text-3xl font-black text-blue-400 tracking-tighter">
                      {inr(grossProfit)}
                    </h4>
                  </div>
                </div>
              ...
              <div className="bg-[#161b27] border border-[#21293d] rounded-[2rem] overflow-hidden shadow-2xl">
                <div className="px-8 py-5 border-b border-[#21293d] bg-gradient-to-r from-red-600/10 to-transparent flex items-center justify-between">
                  <h3 className="text-sm font-black text-white uppercase tracking-[0.2em] flex items-center gap-3">
                    <TrendingDown size={18} className="text-red-400" /> Anya Kharche (Indirect
                    Expenses)
                  </h3>
                </div>
                <div className="p-0">
                  <table className="w-full text-sm">
                    <tbody className="divide-y divide-[#21293d]">
                      <tr className="hover:bg-white/[0.01] transition-colors">
                        <td className="px-8 py-4 text-slate-400 flex items-center gap-3">
                          <Users size={14} className="text-slate-600" /> Staff Salary (Attendance
                          Based)
                        </td>
                        <td className="px-8 py-4 text-right font-black text-red-400">
                          -{inr(data.staff_salary)}
                        </td>
                      </tr>
                      <tr className="hover:bg-white/[0.01] transition-colors">
                        <td className="px-8 py-4 text-slate-400 flex items-center gap-3">
                          <CreditCard size={14} className="text-slate-600" /> Shop Expenses &
                          Utility Bills
                        </td>
                        <td className="px-8 py-4 text-right font-black text-red-400">
                          -{inr(data.shop_expenses)}
                        </td>
                      </tr>
                      <tr className="hover:bg-white/[0.01] transition-colors">
                        <td className="px-8 py-4 text-slate-400 flex items-center gap-3">
                          <Landmark size={14} className="text-slate-600" /> Loan EMI Payments
                        </td>
                        <td className="px-8 py-4 text-right font-black text-red-400">
                          -{inr(data.emi_paid)}
                        </td>
                      ...
                      <tr className="hover:bg-white/[0.01] transition-colors">
                        <td className="px-8 py-4 text-slate-400 flex items-center gap-3">
                          <TrendingDown size={14} className="text-slate-600" /> Client Discounts
                          Given
                        </td>
                        <td className="px-8 py-4 text-right font-black text-red-400">
                          -{inr(data.discounts)}
                      ...
                      <tr className="bg-red-500/5">
                        <td className="px-8 py-5 text-red-400 font-black uppercase text-xs tracking-widest">
                          Total Indirect Expenses
                        </td>
                        <td className="px-8 py-5 text-right font-black text-red-400 text-xl">
                          {inr(totalIndirectExpenses)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              >
                ...
                <div className="relative text-center">
                  <h3 className="text-xs font-black uppercase tracking-[0.3em] opacity-80 mb-6">
                    Final Chittha
                  </h3>
              ...
              <div className="bg-[#161b27] border border-[#21293d] rounded-[2rem] p-8 shadow-2xl space-y-6">
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                    <p className="text-[10px] font-black text-emerald-500/60 uppercase tracking-widest mb-1">
                      Total Assets (Sampatti)
                    </p>
                    <h4 className="text-xl font-black text-white">{inr(totalAssets)}</h4>
                    <p className="text-[8px] text-slate-600 mt-0.5 uppercase">
                      Stock + Savings + Tools
                    </p>
                  </div>
                  <div className="p-4 bg-red-500/5 border border-red-500/10 rounded-2xl">
                    <p className="text-[10px] font-black text-red-500/60 uppercase tracking-widest mb-1">
                      Total Liabilities (Dindari)
                    </p>
                    <h4 className="text-xl font-black text-white">{inr(totalLiabilities)}</h4>
                    <p className="text-[8px] text-slate-600 mt-0.5 uppercase">
                      Loans + Salaries + Bills
                    </p>
                  </div>
                </div>
                <div className="pt-4 border-t border-white/5 text-center">
                  ...
                  <h3
                    className={`text-4xl font-black tracking-tighter ${netWorth >= 0 ? "text-white" : "text-red-500"}`}
                  >
                    {inr(netWorth)}
                  </h3>
                </div>
          ...
          <div className="bg-blue-500/5 border border-blue-500/20 rounded-3xl p-6 flex items-start gap-4 text-blue-400/80 text-xs font-bold leading-relaxed shadow-xl">
            ...
            <div className="space-y-2">
              <p className="uppercase tracking-widest text-[10px] font-black">
                Darpan Methodology Note:
              </p>
              <p>
                Vyapar Darpan uses the 90% Cost Assumption for parts valuation to estimate gross
                margins when exact purchase costs are unavailable. Net Worth is calculated by
                summing Inventory Value, Period Savings, and Fixed Tool Assets (₹50,000), minus
                Outstanding Loans, current month salaries, and pending expenses.
              </p>
            </div>
          </div>
        </div>
