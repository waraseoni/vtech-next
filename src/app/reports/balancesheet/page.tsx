...
function BalanceSheetContent() {
  ...
  const [selectedClient] = useState<{ id: number; name: string; opening_balance: number } | null>(
    null
  );
  ...
  const applyFilter = () => {
    if (filterType === "monthly") {
      ...
      setStartDate(`${selYear}-01-01`);
      setEndDate(`${selYear}-12-31`);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <MetricCard
          ...
          color={(summary?.netProfit ?? 0) >= 0 ? "emerald" : "red"}
          icon={(summary?.netProfit ?? 0) >= 0 ? <Up size={14} /> : <Down size={14} />}
        />
        <MetricCard
          label="स्टॉक वैल्यू"
        ...
        <MetricCard
          label="मैकेनिक बकाया"
          value={summary?.totalMechBalance || 0}
          color="amber"
          icon={<Wrench size={14} />}
        />
        <MetricCard
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-[10px] font-mono">
            ...
            <div className="bg-slate-900/50 rounded-lg p-2">
              ...
              <div className="text-slate-500 space-y-0.5">
                ...
                <div>
                  <span className="text-cyan-400">Net Profit</span> = Total Income − Total Expenses
                </div>
              </div>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-2">
              ...
              <div className="text-slate-500 space-y-0.5">
                ...
                <div>
                  <span className="text-red-400">Total Liabilities</span> = Staff Payable + Loan
                  Outstanding
                </div>
                <div>
                  <span className="text-purple-400">Capital</span> = Total Assets − Total
                  Liabilities
                </div>
              </div>
            </div>
...
function CustomerTab({
  ...
  data: CLRow[];
  totals: { totOB: number; totRep: number; totPay: number; totBal: number };
}) {
  return (
            ...
            <>
              {data.map((c) => (
                ...
                >
                  ...
                  >
                    {inr(c.opening_balance)}
                  </td>
                  <td className="px-3 py-2.5 text-xs text-right font-bold text-emerald-400">
                    {inr(c.total_repair_amount)}
                  </td>
                  <td className="px-3 py-2.5 text-xs text-right font-bold text-teal-400">
                    {inr(c.total_payment)}
                  </td>
                  <td
                    className={`px-3 py-2.5 text-xs text-right font-bold ${c.current_balance >= 0 ? "text-blue-400" : "text-red-400"}`}
                  >
              ...
              <tr className="border-t-2 border-blue-500/30 bg-blue-500/5">
                <td className="px-3 py-2.5 text-xs font-black text-blue-400">
                  कुल ({data.length})
                </td>
                <td />
                <td
                  className={`px-3 py-2.5 text-xs text-right font-black ${totals.totOB >= 0 ? "text-blue-400" : "text-red-400"}`}
                >
                  {inr(totals.totOB)}
                </td>
                ...
                <td className="px-3 py-2.5 text-xs text-right font-black text-teal-400">
                  {inr(totals.totPay)}
                </td>
                <td
                  className={`px-3 py-2.5 text-xs text-right font-black ${totals.totBal >= 0 ? "text-blue-400" : "text-red-400"}`}
                >
                  {inr(totals.totBal)}
                </td>
                <td />
          ...
          {data.length === 0 ? (
            ...
            data.map((m) => (
              ...
              >
                ...
                <td className="px-3 py-2.5 text-xs text-right font-bold text-emerald-400">
                  {inr(m.salary_due_in_period)}
                </td>
                <td className="px-3 py-2.5 text-xs text-right font-bold text-red-400">
                  {inr(m.advance_in_period)}
                </td>
                <td
                  className={`px-3 py-2.5 text-xs text-right font-bold ${m.balance_amount >= 0 ? "text-blue-400" : "text-red-400"}`}
                >
                  {inr(m.balance_amount)}
                </td>
              </tr>
            ))
          )}
            ...
            <>
              {data.map((s) => (
                ...
                >
                  ...
                  <td
                    className={`px-3 py-2.5 text-xs text-right font-bold ${s.remaining_stock <= 0 ? "text-red-400" : s.remaining_stock <= 5 ? "text-amber-400" : "text-emerald-400"}`}
                  >
                    {s.remaining_stock}
                  </td>
                  <td className="px-3 py-2.5 text-xs text-right font-bold text-blue-400">
                    {inr(s.stock_value)}
                  </td>
                </tr>
              ))}
              <tr className="border-t-2 border-blue-500/30 bg-blue-500/5">
                ...
                <td />
                <td />
                <td className="px-3 py-2.5 text-xs text-right font-black text-blue-400">
                  {inr(totalValue)}
                </td>
              </tr>
            </>
          ...
          {data.length === 0 ? (
            ...
            data.map((l) => (
              ...
              >
                ...
                <td className="px-3 py-2.5 text-xs text-right font-bold text-emerald-400">
                  {inr(l.paid_in_period)}
                </td>
                <td
                  className={`px-3 py-2.5 text-xs text-right font-bold ${l.balance_amount > 0 ? "text-red-400" : "text-emerald-400"}`}
                >
                  {inr(l.balance_amount)}
                </td>
                <td className="px-3 py-2.5">
                  <span
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="text-center py-8">
                    <Loader2 size={20} className="animate-spin text-blue-400 inline" />
                  </td>
                </tr>
              ) : entries.length === 0 ? (
                <tr>
                ...
                entries.map((e, i) => (
                  <tr key={i} className="border-t border-[#21293d]/50">
                    ...
                    <td className="px-3 py-2 text-xs text-right text-teal-400">
                      {e.credit > 0 ? inr(e.credit) : ""}
                    </td>
                    <td
                      className={`px-3 py-2 text-xs text-right font-bold ${e.balance >= 0 ? "text-blue-400" : "text-red-400"}`}
                    >
                      {inr(e.balance)}
                    </td>
                  </tr>
