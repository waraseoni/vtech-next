  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const fetchList = async (
        table: string,
        select: string,
        builder: (q: SupaBuilder) => SupaBuilder
      ) => {
        const list: DbRow[] = [];
        let page = 0;
        while (true) {
      ...
      const [
        ...
        fetchList(
          "transaction_list",
          "id, amount, mechanic_commission_amount, date_completed",
          (q) => q.eq("status", 5).gte("date_completed", start).lte("date_completed", endTz)
        ),
        fetchList("direct_sales", "id, client_id, total_amount, date_created", (q) =>
          q.gte("date_created", start).lte("date_created", end)
        ),
        fetchList("client_payments", "id, discount, created_at", (q) =>
          q.gte("created_at", start).lte("created_at", end)
        ),
        fetchList("expense_list", "amount, date_created", (q) =>
          q.gte("date_created", start).lte("date_created", end)
        ),
        fetchList("loan_payments", "amount_paid, payment_date", (q) =>
          q.gte("payment_date", start).lte("payment_date", end)
        ),
        fetchList("attendance_list", "mechanic_id, curr_date, status", (q) =>
          q.in("status", [1, 3]).gte("curr_date", start).lte("curr_date", end)
        ),
        fetchList("mechanic_list", "id, daily_salary", (q) => q.eq("status", 1)),
        fetchList("mechanic_salary_history", "mechanic_id, salary, effective_date", (q) =>
          q.order("effective_date", { ascending: false })
        ),
      ]);

      const getRate = (mechId: number, dateStr: string, defaultRate: number) => {
        const hist = (salaryHistory || []).find(
          (h) => h.mechanic_id === mechId && h.effective_date <= dateStr
        );
      ...
      const monthly: MonthData[] = MONTHS.map((mName, idx) => {
        ...
        let salary = 0;
        (attendance || [])
          .filter((a) => String(a.curr_date).startsWith(prefix))
          .forEach((a) => {
          ...
          <>
            <div className="p-5 grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3">
              <StatCard
                ...
                label="Repair Income"
                sub={inr(totals.repair)}
                color="blue"
              />
              <StatCard
                icon={<ShoppingCart size={16} />}
                label="Total Sales"
                sub={inr(totals.walkin + totals.clientSales)}
                color="purple"
              />
              <StatCard
                icon={<DollarSign size={16} />}
                label="Total Revenue"
                sub={inr(totals.revenue)}
                color="emerald"
              />
              <StatCard
                icon={<TrendingDown size={16} />}
                label="Salary + Commission"
                sub={inr(totals.salary + totals.commission)}
                color="amber"
              />
              <StatCard
                icon={<TrendingDown size={16} />}
                label="Total Expenses"
                sub={inr(totals.totalExp)}
                color="rose"
              />
              <StatCard
                icon={totals.profit >= 0 ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
                label="Net Profit"
                sub={inr(totals.profit)}
                ...
                <table className="w-full text-sm">
                  <thead className="bg-[#111520]">
                    <tr className="text-xs font-bold uppercase tracking-wider text-slate-500">
                      ...
                      <th className="text-right px-3 py-3" title="Walk-in direct sales">
                        Walk-in
                      </th>
                      <th className="text-right px-3 py-3" title="Client direct sales">
                        Client Sales
                      </th>
                      <th className="text-right px-3 py-3 text-emerald-400" title="Total income">
                        Revenue
                      </th>
                      <th
                        className="text-right px-3 py-3 text-amber-400"
                        title="Salaries paid (history rates)"
                      >
                        Salary
                      </th>
                      <th
                        className="text-right px-3 py-3 text-amber-400"
                        title="Mechanic commissions"
                      >
                        Commission
                      </th>
                      <th className="text-right px-3 py-3" title="Other shop expenses">
                        Expenses
                      </th>
                      <th className="text-right px-3 py-3" title="Loan EMI payments">
                        EMI
                      </th>
                      <th className="text-right px-3 py-3 text-rose-400" title="Discounts given">
                        Discount
                      </th>
                      <th className="text-right px-3 py-3 text-rose-400" title="Total expenses">
                        Total Exp.
                      </th>
                      <th className="text-right px-3 py-3" title="Revenue minus expenses">
                        Net Profit
                      </th>
                  ...
                  <tbody className="divide-y divide-[#1a2234]">
                    {stats.monthly.map((row) => (
                      <tr key={row.month} className="hover:bg-white/[0.02]">
                        ...
                        <td
                          className={`px-3 py-3 text-right font-black ${row.profit >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                        >
                          {inr(row.profit)}
                        </td>
                  ...
                  <tfoot className="bg-[#111520]">
                    <tr className="text-xs font-bold uppercase tracking-wider text-slate-300">
                      ...
                      <td className="px-3 py-3 text-right text-purple-400">
                        {inr(totals.clientSales)}
                      </td>
                      <td className="px-3 py-3 text-right font-black text-emerald-400">
                        {inr(totals.revenue)}
                      </td>
                      ...
                      <td
                        className={`px-3 py-3 text-right font-black ${totals.profit >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {inr(totals.profit)}
                      </td>
