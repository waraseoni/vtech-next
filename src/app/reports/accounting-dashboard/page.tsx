...
function AccountingDashboardContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        pageAll(
          supabase
            ...
            .select("amount")
            .eq("status", 5)
            .gte("date_completed", sTs)
            .lte("date_completed", eTs)
        ),
        pageAll(
          supabase
            .from("direct_sales")
            .select("total_amount, client_id")
            .gte("date_created", sTs)
        ...
        pageAll(
          supabase
            ...
            .gte("payment_date", sTs)
            .lte("payment_date", eTs)
        ),
        pageAll(
          supabase
            .from("expense_list")
      ...
      const walkinSales = (directSales || [])
        .filter((r) => r.client_id === null || r.client_id === 0 || r.client_id === "")
        .reduce((s: number, r) => s + num(r.total_amount), 0);
      const totalRevenue = serviceRevenue + salesRevenue;
      const commission = (commissions || []).reduce(
        (s: number, r) => s + num(r.mechanic_commission_amount),
        0
      );
      ...
      (attendance || []).forEach((a) => {
        ...
        const rate =
          hist.find((h) => !h.effective_date || h.effective_date <= a.curr_date)?.salary ??
          mechRate.get(a.mechanic_id) ??
          0;
      ...
      (expenseAll || []).forEach((r) =>
        catMap.set(r.category || "Other", (catMap.get(r.category || "Other") || 0) + num(r.amount))
      );
      const expenseCategories = [...catMap.entries()]
      ...
      (clients || []).forEach((c) =>
        clientName.set(c.id, [c.firstname, c.lastname].filter(Boolean).join(" ").trim())
      );
      (jobCust || []).forEach((r) => {
      ...
      const [
        ...
        pageAll(
          supabase.from("inventory_list").select("quantity, product_id").lte("stock_date", eTs)
        ),
        ...
        pageAll(
          supabase.from("client_payments").select("amount, discount").lte("payment_date", eTs)
        ),
      ...
      const inventoryValue = (invAll || []).reduce(
        (s: number, r) => s + num(r.quantity) * (prodPrice.get(r.product_id) || 0),
        0
      );
      ...
      (invAllStock || []).forEach((r) =>
        stockMap.set(r.product_id, (stockMap.get(r.product_id) || 0) + num(r.quantity))
      );
      const stockItems = [...stockMap.entries()]
        .filter(([, qty]) => qty > 0)
        .map(([pid, qty]) => ({
          name: prodName.get(pid) || "Unknown",
          qty,
          price: prodPrice.get(pid) || 0,
          value: qty * (prodPrice.get(pid) || 0),
        }))
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-20">
      ...
      <div className="bg-[#161b27] border border-[#21293d] rounded-[2rem] p-5 no-print">
        <form onSubmit={handleFilter} className="flex flex-wrap items-end gap-6">
          ...
          <button
            type="submit"
            className="px-8 py-3.5 bg-purple-600 hover:bg-purple-500 text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-purple-600/20"
          >
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-[#21293d] text-slate-300">
                        ...
                        <tr>
                          <td className="px-4 py-3 text-red-400">− Mechanic Commissions</td>
                          <td className="px-4 py-3 text-right text-red-400">
                            ({inr(data.commission)})
                          </td>
                        </tr>
                        <tr>
                          ...
                          <td className="px-4 py-3 text-right text-red-400">
                            ({inr(data.shopExpenses)})
                          </td>
                        </tr>
                        <tr>
                          <td className="px-4 py-3 text-red-400">− Loan EMI Payments</td>
                          <td className="px-4 py-3 text-right text-red-400">
                            ({inr(data.loanEmis)})
                          </td>
                        </tr>
                        <tr>
                          <td className="px-4 py-3 text-red-400">− Discounts Allowed</td>
                          <td className="px-4 py-3 text-right text-red-400">
                            ({inr(data.discounts)})
                          </td>
                        </tr>
                        <tr className="bg-purple-600">
                          <td className="px-4 py-3 font-black text-white">NET OPERATING PROFIT</td>
                          <td className="px-4 py-3 text-right font-black text-white">
                            {inr(data.netProfit)}
                          </td>
                        </tr>
                    ...
                    <div className="bg-[#0d1117] border border-[#21293d] rounded-2xl p-5 space-y-6">
                      ...
                      <div className="p-4 bg-[#111520] border border-[#21293d] rounded-xl text-xs text-slate-400">
                        ...
                        <p>
                          ...
                          <b className="text-white">Discount Policies</b> optimize karein.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-2">
                    <div className="p-6 bg-gradient-to-r from-purple-700 to-indigo-800 rounded-2xl text-center text-white">
                      ...
                      <div className="text-4xl font-black">
                        {inr(data.cashInflow - data.cashOutflow)}
                      </div>
                    ...
                    <table className="w-full text-sm">
                        ...
                        {data.stockItems.map((row, i) => (
                          <tr key={i}>
                            ...
                            <td className="px-5 py-3.5 text-center">
                              <span
                                className={`px-3 py-1 rounded-lg text-xs font-black ${row.qty < 5 ? "bg-red-500/15 text-red-400 border border-red-500/30" : "bg-purple-500/10 text-purple-400 border border-purple-500/20"}`}
                              >
                                {row.qty}
                              </span>
                            </td>
                      ...
                      <tfoot>
                        <tr className="bg-[#111520] border-t border-[#21293d]">
                          ...
                          <td className="px-5 py-3.5 text-right font-black text-purple-400">
                            {inr(data.inventoryValue)}
                          </td>
                        </tr>
                      </tfoot>
                    </table>
