          ...
          {loading ? (
            ...
            <div className="px-5 py-20 flex flex-col items-center text-center">
              ...
              <p className="text-slate-300 font-bold">
                No jobs were marked as &quot;Done&quot; on this date.
              </p>
              <p className="text-xs text-slate-500 mt-1">
                Try selecting a different date or mechanic.
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                  ...
                  {items.map((item, i) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-4 py-3.5">
                          <Link
                            href={`/jobs/${item.transaction_id}/view`}
                            className="font-bold text-blue-400 hover:text-blue-300 transition-colors block"
                          >
                            #{item.job_id}
                          </Link>
                          <div className="text-[10px] text-slate-500 mb-1">Code: {item.code}</div>
                          <span
                            className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded border ${statusConfig.color}`}
                          >
                            {statusConfig.label}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          <Link
                            href={`/clients/${item.client_id}/view`}
                            className="font-bold text-slate-200 hover:text-teal-400 transition-colors block"
                          >
                            {item.client_name}
                          </Link>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2">
                            <MechAvatar
                              image={item.mechanic_image}
                              name={item.mechanic_name}
                              cls="w-6 h-6 text-[10px]"
                            />
                ...
                <tfoot className="bg-[#111520]">
                  <tr className="text-xs font-black uppercase tracking-wider text-slate-400">
                    <th className="px-4 py-4 text-right" colSpan={6}>
                      Grand Total Today:
                    </th>
                    <th className="px-4 py-4 text-right text-teal-400 text-base">
                      {inr(totals.amount)}
                    </th>
                  </tr>
                </tfoot>
