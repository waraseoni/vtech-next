...
${el.innerHTML}
  ...
  const finalBalance =
    data && data.rows.length > 0 ? data.rows[data.rows.length - 1].balance : openingBal;
  const periodDebit = (data?.rows || [])
    .filter((r) => !r.isBroughtFwd)
    .reduce((s, r) => s + r.debit, 0);
  const periodCredit = (data?.rows || [])
    .filter((r) => !r.isBroughtFwd)
    .reduce((s, r) => s + r.credit, 0);
  const periodDiscount = (data?.rows || [])
    .filter((r) => !r.isBroughtFwd)
    ...
    <div className="space-y-5">
      ...
      {filterOpen && (
        <div className="bg-[#161b27] border border-[#21293d] rounded-2xl p-5 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              onClick={applyFilter}
              className="px-5 py-2 rounded-lg text-[11px] font-bold bg-blue-600 text-white"
            >
              Apply
            </button>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <p
            className={`text-2xl font-black mt-1 ${due > 0 ? "text-red-400" : due < 0 ? "text-emerald-400" : "text-white"}`}
          >
            {inr(due)}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl overflow-hidden shadow-2xl">
          <div
            id="ledger-statement"
            style={{ padding: 20, fontFamily: "Arial, sans-serif", fontSize: 13, color: "#333" }}
          >
            {/* Header */}
            <div
              style={{
            ...
            >
              <div>
                ...
                {data.firm.contact && (
                  <div style={{ fontSize: 11, color: "#666" }}>📞 {data.firm.contact}</div>
                )}
              </div>
              <div style={{ textAlign: "right" }}>
                <div
                  style={{ fontWeight: 900, fontSize: "1.5rem", letterSpacing: 2, color: "#555" }}
                >
                  STATEMENT
                </div>
                ...
                {isFiltered && (
                  <div style={{ fontSize: 10, color: "#6c757d", marginTop: 2 }}>
                    Period: {appliedFrom ? fmtDate(appliedFrom + "T00:00") : "All"} —{" "}
                    {appliedTo ? fmtDate(appliedTo + "T00:00") : "All"}
                  </div>
                )}
              </div>
            </div>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: "#001f3f", marginTop: 2 }}>
                  {data.client.name}
                </div>
                {data.client.contact && (
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div
                  style={{
                    fontSize: 18,
                    fontWeight: 900,
                    color: due > 0 ? "#c0392b" : due < 0 ? "#27ae60" : "#333",
                    marginTop: 2,
                  }}
                >
              ...
              >
                <thead>
                  <tr style={{ background: "#001f3f", color: "#fff" }}>
                    <th style={{ padding: "6px 5px", border: "1px solid #ddd", textAlign: "left" }}>
                      Date
                    </th>
                    <th style={{ padding: "6px 5px", border: "1px solid #ddd", textAlign: "left" }}>
                      Description
                    ...
                    <th
                      style={{ padding: "6px 5px", border: "1px solid #ddd", textAlign: "right" }}
                    >
                      Debit
                    </th>
                    <th
                ...
                <tbody>
                  {data.rows.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        style={{ padding: "14px 5px", textAlign: "center", color: "#888" }}
                      >
                        Koi entry nahi mili
                      </td>
                    </tr>
                      ...
                      >
                        ...
                        <td
                          style={{
                            ...
                            textAlign: "right",
                            fontWeight: 700,
                            color: r.balance > 0 ? "#c0392b" : r.balance < 0 ? "#27ae60" : "#333",
                          }}
                        >
                          {inr(r.balance)}
                        </td>
                ...
                <tfoot>
                  <tr style={{ background: "#f0f0f0", fontWeight: "bold" }}>
                    <td colSpan={3} style={{ padding: "6px 5px", border: "1px solid #ddd" }}>
                      Total ({appliedFrom || appliedTo ? "period" : "all-time"})
                    </td>
                    <td
                      style={{ padding: "6px 5px", border: "1px solid #ddd", textAlign: "right" }}
                    >
            ...
            <div style={{ fontSize: 10, color: "#6c757d", marginTop: 10 }}>
              Balance = Opening + Repairs + Direct Sales + Loans − Payments (incl. discount).{" "}
              {data.totals.loans > 0 ? `Loan total: ${inr(data.totals.loans)}. ` : ""}
              {data.totals.discount > 0 ? `Total discount: ${inr(data.totals.discount)}.` : ""}
            </div>
          </div>
        </div>
      )}
