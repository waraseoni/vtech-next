...
// ─── Sprint 4 #17 (SSR): direct-sales list ka client hissa ──────────────────
// Server `page.tsx` pehla data laata hai (props: sales/stats/sysinfo/filters).
// Fetch server ke paas hai — client me koi auto-fetch nahi:
//   • Apply/Month/Reset → router.push (server refetch → props sync)
//   • Delete → mutation + router.refresh()
// Behavior 1:1 same — sirf pehla load + har filter server se aata hai.
...
export interface DirectSalesInitial {
  initialSales: DirectSale[];
  initialStats: SaleStats;
  initialSysInfo: Record<string, string>;
  initialFrom: string;
  initialTo: string;
...
const PayBadge = ({ mode }: { mode: string }) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg border ${bg} ${border} ${color} text-[10px] font-bold`}
    >
      <Icon size={10} /> {mode}
    </div>
  );
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export function DirectSalesClient({
  initialSales,
  initialStats,
  initialSysInfo,
  initialFrom,
  ...
  // Server se aaya data = pehla paint + har navigation (koi client fetch nahi)
  ...
  const [stats, setStats] = useState<SaleStats>(initialStats);
  const [sysInfo] = useState<Record<string, string>>(initialSysInfo);

  // Server navigation (Apply/back/refresh) → naya data props me aata hai
  useEffect(() => {
  ...
  }, [initialSales, initialStats, initialFrom, initialTo, initialPayment]);
  ...
  const payBreakdown = useMemo(() => {
    ...
    sales.forEach((s) => {
      map[s.payment_mode] = (map[s.payment_mode] || 0) + s.total_amount;
    });
    return map;
  }, [sales]);
  ...
  // ── Navigation (server refetch — client me koi fetch nahi) ─────────────────
  ...
  const goCurrentMonth = () => updateUrl(startOfMonthIST(), endOfMonthIST(), "all");
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!error) {
      ...
      // Server se taza data (props sync effect list update karega)
      router.refresh();
  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-app pb-24">
        ...
        {showFilterModal && (
          <div
            className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[70] flex items-end justify-center px-0 pt-0 pb-[72px] md:pb-0"
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                {[
                  { label: "From Date", val: dateFrom, set: setDateFrom },
                ...
                <div className="flex gap-2.5 pt-1">
                  <button
                    onClick={() => {
                      updateUrl(startOfMonthIST(), endOfMonthIST(), "all");
                    }}
            ...
            {[
              {
                ...
                icon: BarChart3,
                color: "text-blue-400",
                grad: "from-blue-600/15 to-blue-700/5",
              ...
              },
              {
                ...
                icon: IndianRupee,
                color: "text-emerald-400",
                grad: "from-emerald-600/15 to-emerald-700/5",
              ...
              },
              {
                ...
                grad: "from-purple-600/15 to-purple-700/5",
              ...
              },
              {
                ...
                icon: Banknote,
                color: "text-teal-400",
                grad: "from-teal-600/15 to-teal-700/5",
              ...
              },
              {
                ...
                icon: Smartphone,
                color: "text-cyan-400",
                grad: "from-cyan-600/15 to-cyan-700/5",
      ...
      <div className="max-w-7xl mx-auto px-5 py-4 space-y-4">
        ...
        <div className="bg-panel border border-app rounded-2xl px-5 py-4">
          <div className="flex flex-wrap items-end gap-3">
            ...
            {/* Apply */}
            <button
              onClick={() => updateUrl(dateFrom, dateTo, paymentFilter)}
            ...
            <div className="flex items-center gap-1.5">
              {Object.entries(payBreakdown).map(([mode, amt]) => {
                ...
                return (
                  <span
                    key={mode}
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[10px] font-bold ${cfg.bg} ${cfg.border} ${cfg.color}`}
                  >
