  ...
  const fetchSale = useCallback(async () => {
    ...
    try {
      ...
      const clientName = c
        ...
        : null;
      const mechName = mechRes.data
        ? `${mechRes.data.firstname} ${mechRes.data.lastname}`
        : "Admin";
      const editorName =
        sd.last_edited_by === 0
          ? "Admin"
          : editorRes.data
            ? `${editorRes.data.firstname} ${editorRes.data.lastname}`
            : null;
      ...
      const items: SaleItem[] = (itemsRes.data || []).map((i) => ({
        ...i,
        product_name: pMap.get(i.product_id) || "Unknown",
        total: i.qty * i.price,
      }));
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 py-5">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    ...
                    <span
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold border ${payCfg.bg} ${payCfg.border} ${payCfg.color}`}
                    >
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/direct-sales/${sale.id}/edit`}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="px-5 py-1">
              ...
              {sale.client_address && (
                <InfoRow
                  label="Address"
                  value={
                    <span className="text-xs leading-relaxed max-w-[180px] text-right">
                      {sale.client_address}
                    </span>
                  }
                />
          ...
          <div className="theme-card border border-[#21293d] rounded-2xl overflow-hidden">
            ...
            <div className="px-5 py-1">
              <InfoRow
                label="Invoice No"
                value={<span className="text-blue-400 font-extrabold">{sale.sale_code}</span>}
              />
              <InfoRow label="Date" value={fmtIST(sale.date_created, false)} />
              <InfoRow
                label="Time"
                value={fmtIST(sale.date_created).split(",")[1]?.trim() || "—"}
              />
              <InfoRow
                label="Staff"
                value={
              ...
              <InfoRow
                ...
                value={
                  <span
                    className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-extrabold border ${payCfg.bg} ${payCfg.border} ${payCfg.color}`}
                  >
                ...
                <InfoRow
                  ...
                  value={
                    <span className="text-[10px]">
                      {sale.last_editor_name} · {fmtIST(sale.last_edited_date!)}
                    </span>
                  }
                />
              )}
              ...
              {subtotal !== sale.total_amount && (
                <div className="flex justify-between items-center text-xs">
                  ...
                  >
                    ...
                    {(sale.total_amount - subtotal).toLocaleString("en-IN", {
                      minimumFractionDigits: 2,
                    })}
                  </span>
                </div>
