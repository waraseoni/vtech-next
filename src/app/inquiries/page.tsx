...
const shiftMon = (s: string, n: number) => {
  const d = parseISTDate(s);
  return new Date(d.getFullYear(), d.getMonth() + n, 1);
};
const fmtDate = (s: string) =>
  formatIST(s.split("T")[0], { day: "2-digit", month: "short", year: "numeric" });
...
function InquiriesPageInner() {
  ...
  const prevMonth = () => {
    const d = shiftMon(fromDate, -1);
    applyFilter(firstOfMon(d), lastOfMon(d), statusFilter);
  };
  const nextMonth = () => {
    const d = shiftMon(fromDate, 1);
    applyFilter(firstOfMon(d), lastOfMon(d), statusFilter);
  ...
  const filteredMobile = useMemo(() => {
    return inquiries.filter((i) => {
      ...
      const ms =
        !q || [i.fullname, i.contact, i.email, i.message].some((v) => v.toLowerCase().includes(q));
      const st =
        mobileStatus === "all" ||
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    className={`flex items-center gap-1 px-2 py-1 rounded-full text-[9px] font-extrabold border ${
                      inq.status === 0
                ...
                <div className="flex items-center gap-3 text-[11px] text-slate-500 mb-3">
                  ...
                  <span className="flex items-center gap-1 truncate">
                    <Mail size={9} className="text-cyan-400" />
                    {inq.email}
                  </span>
                </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2.5 pt-1">
                  ...
                  <button
                    onClick={() => {
                      applyFilter(fromDate, toDate, statusFilter);
                      setShowFilterModal(false);
                    }}
                    className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-extrabold shadow-lg shadow-blue-500/20"
                  >
                    Apply
                  </button>
                </div>
        ...
        <div className="relative max-w-7xl mx-auto px-5 py-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              {allTimeStats.unread > 0 && (
                <button
                  onClick={() => applyFilter(firstOfMon(), lastOfMon(), "unread")}
              ...
              <div key={label}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    set(e.target.value);
                    applyFilter(
                      label === "From" ? e.target.value : fromDate,
                      label === "To" ? e.target.value : toDate,
                      statusFilter
                    );
                  }}
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {(["all", "unread", "read"] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => applyFilter(fromDate, toDate, f)}
