...
import type { Client, PaymentRow } from "@/lib/server-payments";
...
const fmtDate = (date: string) =>
  formatIST(date.slice(0, 10), { day: "2-digit", month: "short", year: "numeric" });
const paymentCode = (id: number) => `PY-${String(id).padStart(4, "0")}`;
const clientName = (c?: Client | null) =>
  c ? [c.firstname, c.middlename, c.lastname].filter(Boolean).join(" ").trim() : "-";
...
export default function PaymentsPageInner({ initialClients, initialPayments }: PaymentsPageProps) {
  ...
  const filtered = useMemo(() => {
    return payments.filter((p) => {
      ...
      return (
        paymentCode(p.id).includes(term) ||
        cn.toLowerCase().includes(term) ||
  ...
  const totals = useMemo(
    () =>
      filtered.reduce(
        (a, p) => ({ amount: a.amount + p.amount, discount: a.discount + (p.discount || 0) }),
        { amount: 0, discount: 0 }
      ),
    [filtered]
  );
  ...
  const viewReceipt = (p: PaymentRow) => {
    setReceiptPayment(p);
    setReceiptOpen(true);
  };
  const applyMonth = (offset: number) => {
    const r = getMonthRange(offset, fromDate);
    setFromDate(r.from);
    setToDate(r.to);
    setCurrentPage(1);
  };
    ...
    <AdminPage title="Payments" subtitle="Client payments management">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#161b27] border border-[#21293d] rounded-2xl p-4">
          ...
          <p className="mt-3 text-xl font-black text-purple-400">
            {fmtMoney(totals.amount + totals.discount)}
          </p>
        </div>
      </div>
              ...
              {paginated.map((p) => (
                <div key={p.id} className="rounded-2xl border border-[#21293d] bg-[#111520] p-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => deleteP(p.id)}
                      className="py-2 px-3 bg-red-600/20 border border-red-500/30 rounded-xl text-xs font-bold text-red-400"
                    >
                      <Trash2 size={12} />
                    </button>
                  </div>
