...
export default function LendersPage() {
  ...
  const getLenderTotals = useCallback(
    (lender: Lender) => {
  ...
  const openView = (l: Lender) => {
    setViewing(l);
    setShowViewModal(true);
  };

  const openPayEMI = (l: Lender) => {
    setPayingLender(l);
    setPayForm({ amount_paid: String(l.emi_amount || ""), payment_date: todayIST(), remarks: "" });
    setPayErr("");
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.fullname.trim()) {
      setFormErr("Lender name zaroori hai!");
      return;
    }
    if (!form.contact.trim()) {
      setFormErr("Contact zaroori hai!");
      return;
    }
    const amount = parseFloat(form.loan_amount);
    if (!amount || amount <= 0) {
      setFormErr("Valid loan amount daalo!");
      return;
    }
    const tenure = parseInt(form.tenure_months);
    if (!tenure || tenure <= 0) {
      setFormErr("Valid tenure daalo!");
      return;
    }
    ...
    <AdminPage title="Lenders" subtitle="Loan liye hue lenders se">
      <div className="bg-[#161b27] border border-[#21293d] rounded-2xl overflow-hidden">
        ...
        <div className="px-5 py-2 border-b border-[#1a2234] bg-[#0d1117]/50 grid grid-cols-3 gap-4 text-[10px] font-black uppercase tracking-widest text-slate-600">
          ...
          <div>
            Total Balance: <span className="text-amber-400 font-bold">{inr(totals.balance)}</span>
          </div>
        </div>
                  ...
                  return (
                    <tr key={lender.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-right font-black text-amber-400">
                        {inr(lender.emi_amount)}
                      </td>
                      <td className="px-4 py-3.5 text-right font-black text-emerald-400">
                        {inr(totalPaid)}
                      </td>
                      <td
                        className={`px-4 py-3.5 text-right font-black ${balance > 0 ? "text-red-400" : "text-emerald-400"}`}
                      >
                        {inr(balance)}
                      </td>
                      <td className="px-4 py-3.5 text-center">
                        <button
                          onClick={() => toggleStatus(lender)}
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {userRole === "admin" && (
                            <button
                              onClick={() => handleDelete(lender.id, lender.fullname)}
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-[#21293d]">
              <h3 className="font-bold text-white flex items-center gap-2">
                {editing ? (
                  <>
                    <Edit3 size={16} className="text-blue-400" /> Edit Lender
                  </>
                ) : (
                  <>
                    <Plus size={16} className="text-blue-400" /> Add Lender
                  </>
                )}
              </h3>
              <button
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">
                    ...
                    <button
                      type="button"
                      onClick={autoFill}
                      className="ml-2 text-blue-400 text-[9px] hover:text-blue-300"
                    >
                      (Auto)
                    </button>
                  </label>
                ...
                <div className="p-5 space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <DetailItem
                      label="Balance"
                      value={inr(balance)}
                      valueClass={`font-bold ${balance > 0 ? "text-red-400" : "text-emerald-400"}`}
                    />
