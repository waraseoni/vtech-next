...
export async function GET(request: NextRequest) {
  ...
  (sys || []).forEach((r) => {
    firmMap[r.meta_field] = r.meta_value;
  });
  ...
  if (from) {
    ...
    broughtFwd =
      openingBal +
      sum(preJ, "amount") +
      sum(preS, "total_amount") +
      sum(preL, "total_payable") -
      (sum(preP, "amount") + sum(preP, "discount"));
  }
  ...
  return NextResponse.json({
    client: {
      id: cid,
      name:
        [cd?.firstname, cd?.middlename, cd?.lastname].filter(Boolean).join(" ").trim() ||
        `Client #${cid}`,
      contact: cd?.contact ?? "",
      email: cd?.email ?? "",
