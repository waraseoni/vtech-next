...
export async function GET(request: NextRequest) {
  ...
  const enriched: { id: number; job_id: string; code: string | null; date_created: string; mechanic_id: number; m_name: string; service_amount: number; mechanic_commission_amount: number; }[] = [];
  ...
  for (const t of txns || []) {
    ...
    const { data: svcData } = await supabase.from("transaction_services").select("price").eq("transaction_id", t.id);
    const svcAmt = svcData?.reduce((s: number, r: { price: number }) => s + (r.price || 0), 0) || 0;
  ...
  const rows = enriched.filter((r) => r.mechanic_commission_amount > 0);
...
<body>
  ...
  <div class="summary-cards">
    ...
    <div class="summary-card">
      <div class="summary-label">Staff Count</div>
      <div class="summary-value">${summary.length}</div>
    </div>
  </div>
