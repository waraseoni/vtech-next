...
// ─── Sprint 4 #17 (SSR pilot): suppliers list ka client hissa ───────────────
// Server `page.tsx` pehla data laata hai (props); yahan search/modal/
// edit/delete interactivity + mutation ke baad refresh rehta hai.
// Behavior 1:1 same — sirf pehla load server se aata hai (no client waterfall).
...
export interface SuppliersInitial {
  initialRows: SupplierRow[];
  initialPersons: RawPersonRow[];
  initialPhones: RawPhoneRow[];
  initialDues: Record<number, number>;
  initialRole: string;
...
// Persons+phones → supplier-wise map. Server initial data AUR client refresh
// dono isi se guzarte hain (single transform, deverbatim fetchData se).
export function buildPersonsMap(
  personRows: RawPersonRow[],
  phoneRows: RawPhoneRow[]
): Record<number, ContactPerson[]> {
...
export function SuppliersClient({
  ...
  const [personsMap, setPersonsMap] = useState<Record<number, ContactPerson[]>>(() =>
    buildPersonsMap(initialPersons, initialPhones)
  );
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<SupplierRow | null>(null);
  const [err, setErr] = useState(initialError || "");
  const [userRole, setUserRole] = useState(initialRole || "staff");
  const [zoomSrc, setZoomSrc] = useState("");
  const [duesMap, setDuesMap] = useState<Record<number, number>>(initialDues);
  ...
  // Role fallback — server role bhejta hai; ye sirf seedha-client-mount par hai
  useEffect(() => {
  ...
  // Refresh — mutation (save/delete/toggle) ke baad. Pehla load server se
  // aata hai isliye mount par nahi chalta (SSR ka fayda).
  const fetchData = useCallback(async () => {
    ...
    setPersonsMap(
      buildPersonsMap(
        (personRows || []) as RawPersonRow[],
        (phoneRows || []) as RawPhoneRow[]
      )
    );
