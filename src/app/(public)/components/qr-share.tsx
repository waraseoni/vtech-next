...
export function QrShareModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      const canShareFile =
        !!qrFile && !!navigator.canShare && navigator.canShare({ files: [qrFile] });
  ...
  const SHARE_TARGETS = [
    // WhatsApp Web / app → contact search & select (forward jaise)
    {
      ...
      href: () =>
        `https://api.whatsapp.com/send?text=${encodeURIComponent(`${shareMsg().text}\n${shareMsg().url}`)}`,
    },
    {
      key: "tg",
      name: "Telegram",
      ...
      icon: <Send size={18} />,
      href: () =>
        `https://t.me/share/url?url=${encodeURIComponent(shareMsg().url)}&text=${encodeURIComponent(shareMsg().text)}`,
    },
    {
      key: "fb",
      name: "Facebook",
      ...
      icon: <Globe size={18} />,
      href: () =>
        `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareMsg().url)}`,
    },
    {
      key: "x",
      name: "X / Twitter",
      ...
      icon: <AtSign size={18} />,
      href: () =>
        `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareMsg().text)}&url=${encodeURIComponent(shareMsg().url)}`,
    },
    {
      key: "mail",
      name: "Email",
      ...
      icon: <Mail size={18} />,
      href: () =>
        `mailto:?subject=${encodeURIComponent(`${SITE.name} — Website`)}&body=${encodeURIComponent(`${shareMsg().text}\n${shareMsg().url}`)}`,
    },
  ];
