...
export default function ClientLoansPage() {
      ...
      const [loanRes, clientRes, paymentsRes] = await Promise.all([
        ...
        pageAll(
          supabase
            ...
            .select("id, loan_id, amount, discount")
            .not("loan_id", "is", null)
        ),
      ]);
  ...
  const clientName = (c?: Client | null) =>
    !c ? "-" : [c.firstname, c.middlename, c.lastname].filter(Boolean).join(" ");
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.client_id) {
      setFormErr("Client select karo!");
      return;
    }
    const principal = parseFloat(form.principal);
    if (!principal || principal <= 0) {
      setFormErr("Valid principal amount daalo!");
      return;
    }
    ...
    <AdminPage title="Client Loans" subtitle="Loans diye hue clients ko">
      <div className="bg-[#161b27] border border-[#21293d] rounded-2xl overflow-hidden">
        ...
        <div className="px-5 py-2 border-b border-[#1a2234] bg-[#0d1117]/50 grid grid-cols-4 gap-4 text-[10px] font-black uppercase tracking-widest text-slate-600">
          ...
          <div>
            Total Payable: <span className="text-emerald-400 font-bold">{inr(totals.payable)}</span>
          </div>
          <div>
            Total Balance: <span className="text-red-400 font-bold">{inr(totals.balance)}</span>
          </div>
        </div>
                  ...
                  return (
                    <tr key={loan.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-right font-black text-emerald-400">
                        {inr(loan.total_payable)}
                      </td>
                      <td className="px-4 py-3.5 text-right font-black text-amber-400">
                        {inr(loan.paid)}
                      </td>
                      <td className="px-4 py-3.5 text-right font-black text-red-400">
                        {inr(loan.balance)}
                      </td>
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-[#21293d]">
              <h3 className="font-bold text-white flex items-center gap-2">
                {editing ? (
                  <>
                    <Edit3 size={16} className="text-blue-400" /> Edit Client Loan
                  </>
                ) : (
                  <>
                    <Plus size={16} className="text-blue-400" /> Add Client Loan
                  </>
                )}
              </h3>
              <button
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <DetailItem
                  label="Total Payable"
                  value={inr(viewing.total_payable)}
                  valueClass="text-emerald-400 font-bold"
                />
                <DetailItem
                  label="Paid"
                  value={inr(viewing.paid)}
                  valueClass="text-amber-400 font-bold"
                />
                <DetailItem
                  label="Balance"
                  value={inr(viewing.balance)}
