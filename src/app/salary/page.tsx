...
function SalaryContent() {
  ...
  const [payTarget, setPayTarget] = useState<{ id: number; name: string; amount: number } | null>(
    null
  );
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const [allAtt, allComm, allAdv, allHist] = await Promise.all([
        fetchAllData(
          supabase
            ...
            .in("status", [1, 3])
            .lte("curr_date", nextMonthStart.slice(0, 10))
        ),
        fetchAllData(
          supabase
            .from("transaction_list")
            .select("mechanic_id, mechanic_commission_amount, status, date_completed")
            .in("mechanic_id", mechIds)
            .eq("status", 5)
            .lte("date_completed", nextMonthStart)
        ),
        fetchAllData(
          supabase
            ...
            .in("mechanic_id", mechIds)
            .lte("date_paid", nextMonthStart.slice(0, 10))
        ),
        fetchAllData(
          supabase
      ...
      const commList = (allComm || []).map((c) => ({
        ...c,
        istMonth: toISTString(new Date(c.date_completed)).slice(0, 7),
      }));
      ...
      const salaryRows: SalaryRow[] = enrichedMechs.map((m) => {
        ...
        const commPrevSum = commList
          .filter((c) => c.mechanic_id === m.id && c.istMonth < month)
        ...
        attList
          .filter(
            (a) =>
              a.mechanic_id === m.id && a.curr_date >= monthStart && a.curr_date < nextMonthStart
          )
          .forEach((a) => {
            const rate = getEffectiveRate(m.id, a.curr_date, defaultSal, histList);
            if (a.status === 3) {
        ...
        const currentAdv = advList
          .filter(
            (a) =>
              a.mechanic_id === m.id && a.date_paid >= monthStart && a.date_paid < nextMonthStart
          )
          .reduce((s, a) => s + (a.amount || 0), 0);
  ...
  const openLedger = async (r: SalaryRow) => {
    ...
    try {
      ...
      const { data: histList } = await supabase
        .from("mechanic_salary_history")
        .select("*")
        .eq("mechanic_id", r.id)
        .order("effective_date", { ascending: false });
      ...
      const [attAll, attPrev, commAllData, advAll, advPrev] = await Promise.all([
        ...
        fetchAllData(
          supabase
            .from("transaction_list")
            .select("job_id, code, mechanic_commission_amount, status, date_completed")
            .eq("mechanic_id", r.id)
            .eq("status", 5)
            .lte("date_completed", nextMonthBoundary)
        ),
        fetchAllData(
          supabase
            .from("advance_payments")
      ...
      const commAll = (commAllData || []).filter(
        (c) => toISTString(new Date(c.date_completed)).slice(0, 7) === month
      );
      const cp = (commAllData || [])
        .filter((c) => toISTString(new Date(c.date_completed)).slice(0, 7) < month)
        .reduce((s, x) => s + (x.mechanic_commission_amount || 0), 0);
      ...
      const dates = new Set([
        ...(attAll?.map((a) => a.curr_date) || []),
        ...(commAll?.map((c) => toISTString(new Date(c.date_completed)).split("T")[0]) || []),
        ...(advAll?.map((a) => a.date_paid) || []),
      ]);
      ...
      for (const d of Array.from(dates).sort()) {
        ...
        const adv =
          advAll?.filter((a) => a.date_paid === d).reduce((s, a) => s + (a.amount || 0), 0) || 0;
  ...
  const handlePaySalary = async () => {
    ...
    await supabase.from("advance_payments").insert({
      mechanic_id: payTarget.id,
      amount: payTarget.amount,
      date_paid: todayIST(),
      reason: `Salary Settlement for ${monthLabel}`,
    });
  ...
  const handleUpdateSalary = async () => {
    ...
    await supabase.from("mechanic_salary_history").insert({
      mechanic_id: salaryTarget.id,
      salary: parseFloat(newSalary),
      effective_date: newEffectiveDate,
    });
    await supabase
      .from("mechanic_list")
      .update({ daily_salary: parseFloat(newSalary) })
      .eq("id", salaryTarget.id);
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className="text-sm font-black text-blue-400 print:text-black hover:text-blue-300 print:pointer-events-none transition-colors"
                          >
                            {r.name}
                          </button>
                        </td>
                        ...
                        <td className="px-4 py-3 text-xs text-right font-bold text-red-400 print:text-red-700">
                          {inr(r.current_adv)}
                        </td>
                        <td className="px-4 py-3 text-right">
                          <span
                            className={`text-xs font-black px-2.5 py-1.5 rounded-lg print:p-0 print:bg-transparent print:border-none ${r.net_final >= 0 ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 print:text-green-700" : "bg-red-500/10 text-red-400 border border-red-500/20 print:text-red-700"}`}
                          >
                        ...
                        <td className="px-4 py-3 text-right print:hidden">
                          {r.net_final > 0 ? (
                            <button
                              onClick={() =>
                                setPayTarget({ id: r.id, name: r.name, amount: r.net_final })
                              }
                              className="px-3 py-1.5 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-lg text-xs font-bold hover:bg-emerald-500 hover:text-white transition-all shadow-sm"
                            >
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            setSalaryTarget({
                              id: m.id,
                              name: [m.firstname, m.lastname].join(" "),
                              salary: m.daily_salary,
                            });
                            setNewSalary(String(m.daily_salary));
                            setSalaryRateModal(true);
                          }}
                          className="px-3 py-1.5 bg-blue-500/20 border border-blue-500/30 text-blue-400 rounded-lg text-xs font-bold hover:bg-blue-600 hover:text-white transition-all shadow-sm flex items-center gap-1.5"
                        >
          ...
          <div className="bg-[#161b27] border border-[#21293d] rounded-2xl w-full max-w-sm shadow-2xl p-6">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-sm font-black text-white shadow-lg shadow-emerald-900/20 transition-all"
              >
                Confirm Payout
              </button>
              <button
                  ...
                  {ledgerLoading ? (
                    ...
                    ledgerData.map((e, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-right font-bold text-emerald-400/80">
                          {e.wage > 0 ? "+ " + inr(e.wage) : "—"}
                        </td>
                        <td className="px-4 py-3 text-xs text-right font-bold text-blue-400/80">
                          {e.comm > 0 ? "+ " + inr(e.comm) : "—"}
                        </td>
                        <td className="px-4 py-3 text-xs text-right font-bold text-red-400">
                          {e.adv > 0 ? "- " + inr(e.adv) : "—"}
                        </td>
                        <td
                          className={`px-4 py-3 text-sm text-right font-black ${e.balance >= 0 ? "text-emerald-400" : "text-red-400"}`}
                        >
                          {inr(e.balance)}
                        </td>
                      </tr>
