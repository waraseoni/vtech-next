...
export default function ManageJobPage() {
  ...
  const fetchMaster = useCallback(async () => {
    ...
    const [cRes, mRes, sRes, pRes] = await Promise.all([
      ...
      supabase
        .from("mechanic_list")
        .select("id, firstname, middlename, lastname, designation, commission_percent")
        .eq("status", 1)
        .order("firstname"),
      supabase
        .from("service_list")
    ...
    setDateCreated(
      job.date_created
        ? new Intl.DateTimeFormat("en-CA", {
            ...
            .format(new Date(job.date_created))
            .replace(", ", "T")
            .replace(",", "T")
        : todayISTDateTime()
    );
  ...
  const addProduct = (prdId: number) => {
    ...
    setSelProducts((prev) => [
      ...prev,
      { product_id: prod.id, product_name: prod.name, qty: 1, price: prod.price },
    ]);
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 uppercase tracking-wider">
                ...
                {!isEdit && jobId && (
                  <span className="ml-2 text-blue-400 font-black">Job #{jobId}</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link
                  href="/clients/new"
                  className="flex items-center gap-1 text-[9px] font-black text-blue-400 hover:text-blue-300 no-underline uppercase tracking-wider"
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Sharpy 10R Axis"
                className={iCls}
              />
            </div>
            <div>
              ...
              <input
                ...
                placeholder="e.g. No light output"
                className={iCls}
              />
            </div>
            <div>
                    ...
                    {selServices.map((s, i) => (
                      <tr key={i} className="hover:bg-white/[0.02]">
                        ...
                        <td className="px-3 py-2 text-right">
                          <input
                            ...
                            onChange={(e) =>
                              setSelServices((prev) =>
                                prev.map((x, j) =>
                                  j === i ? { ...x, price: parseFloat(e.target.value) || 0 } : x
                                )
                              )
                            }
                        ...
                        <td className="px-2 py-2">
                          <button
                            onClick={() => setSelServices((prev) => prev.filter((_, j) => j !== i))}
                  ...
                  <tbody className="divide-y divide-[#21293d]">
                    {selProducts.map((p, i) => (
                      <tr key={i} className="hover:bg-white/[0.02]">
                        ...
                        <td className="px-2 py-2">
                          <input
                            ...
                            onChange={(e) =>
                              setSelProducts((prev) =>
                                prev.map((x, j) =>
                                  j === i ? { ...x, qty: parseInt(e.target.value) || 1 } : x
                                )
                              )
                            }
                        ...
                        <td className="px-2 py-2">
                          <button
                            onClick={() => setSelProducts((prev) => prev.filter((_, j) => j !== i))}
                            className="text-red-400 hover:text-red-300 transition-colors"
                          >
                            <X size={13} />
                          </button>
