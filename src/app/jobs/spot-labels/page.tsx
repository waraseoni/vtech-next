...
import {
  ArrowLeft, Printer, Loader2, Trash2, X, Eye, Settings,
...
async function fetchAllLinkedLocationIds(): Promise<Set<number>> {
  ...
  for (;;) {
    const { data } = await supabase
      .from("transaction_list").select("location_id")
      .not("location_id", "is", null).range(from, from + 999);
...
export default function SpotLabelsPage() {
  ...
  const loadSpotsAndRefs = async (): Promise<Spot[]> => {
    const { data } = await supabase
      .from("locations").select("id, rack")
      .eq("kind", "job").eq("zone", "").order("rack");
    ...
    if (list.length > 0) {
      const { data: refData } = await supabase
        .from("transaction_list").select("location_id")
        .in("location_id", list.map(s => s.id)).eq("del_status", 0);
  ...
  const handleDeleteSpot = async (s: Spot) => {
    ...
    if (used > 0) { alert(`"${s.name}" par ${used} job(s) hain.`); return; }
    if (!confirm(`"${s.name}" delete karein?`)) return;
    setDeleting(s.id);
    const { data: anyRef } = await supabase.from("transaction_list").select("id").eq("location_id", s.id).limit(1);
    if (anyRef && anyRef.length > 0) { alert(`"${s.name}" linked hai.`); setDeleting(null); return; }
  ...
  const handleDeleteEmptySpots = async () => {
    ...
    try {
      ...
      if (!freshSpots.length) { alert("Koi spot nahi."); return; }
      const emptyIds = freshSpots.filter(s => !usedIds.has(s.id)).map(s => s.id);
      if (emptyIds.length === 0) { alert("Sabhi busy hain."); return; }
      ...
      for (let i = 0; i < emptyIds.length; i += 200) {
        ...
        const { error } = await supabase.from("locations").delete().eq("kind", "job").eq("zone", "").in("id", chunk);
  ...
  const handlePopupPrint = useCallback(() => {
    ...
    const pagesHtml = previewPages.map(pageSpots => {
      const cards = pageSpots.map(s => {
...
@media print{body{margin:0;padding:0}.card{box-shadow:none}}
  ...
  const SettingBtn = ({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) => (
    <button onClick={onClick}
      className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold border transition-all ${
        active ? "bg-blue-600/20 border-blue-500/40 text-blue-400" : "bg-transparent border-[#21293d] text-slate-500 hover:text-slate-300 hover:border-slate-500"
      }`}>
      {children}
    </button>
  );
  ...
  const PreviewCard = ({ spot }: { spot: Spot }) => {
    const padPct = CARD_PAD_PCT[margin];
    return (
      <div className="bg-white flex flex-col items-center text-center overflow-hidden"
        style={{
        ...
        <p style={{
          ...
          fontSize: "clamp(7px, 2.5vw, 14px)", lineHeight: 1.2,
          width: "100%", overflow: "hidden", textOverflow: "ellipsis",
          display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical",
          wordBreak: "break-word",
        }}>{spot.name}</p>
        <p style={{
          fontSize: "clamp(5px, 1.5vw, 9px)", fontWeight: 700, color: "#64748b",
        ...
        <p style={{
          fontSize: "clamp(5px, 1.5vw, 9px)", color: "#94a3b8", marginTop: "auto",
          wordBreak: "break-all", lineHeight: 1.2, whiteSpace: "nowrap",
          overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%",
        }}>Scan → Jobs @ {spot.name}</p>
      </div>
    );
  };
    ...
    <div className="min-h-screen bg-[#0d1117]">
      <div className="sticky top-0 z-10 bg-[#161b27] border-b border-[#21293d] px-4 py-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Link href="/jobs" className="bg-[#21293d] hover:bg-[#2a3550] text-slate-300 rounded-lg p-2 transition-colors no-underline">
            <ArrowLeft size={16} />
          </Link>
          <h1 className="text-sm font-black text-white">Spot QR Labels</h1>
          <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">{spots.length} spots</span>
        </div>
        <div className="flex items-center gap-2">
          {!loading && emptyCount > 0 && (
            <button onClick={handleDeleteEmptySpots} disabled={bulkDeleting}
          ...
          <button onClick={() => setPreviewOpen(true)} disabled={loading || spots.length === 0}
      ...
      {loading ? (
        ...
        <p className="text-center text-slate-600 text-sm py-24">Koi spot nahi — pehle jobs form me &quot;+&quot; se spots banao.</p>
      ) : (
        <div className="p-6 max-w-5xl mx-auto">
          {/^https?:\/\/(localhost|127\.0\.0\.1|(\d{1,3}\.){3}\d{1,3})(:\d+)?$/.test(window.location.origin) && (
            <div className="mb-4 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-2.5 text-[11px] font-bold text-amber-400">
              Warning: ye labels abhi <code>{window.location.origin}</code> ka QR banate hain. Production website par kholo.
            </div>
          )}
          <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${Math.min(gridCols, 6)}, 1fr)` }}>
            {spots.map(s => {
              const used = refs[s.id] || 0;
              return (
                <div key={s.id} onClick={() => setJobsSpot(s)}
                  ...
                  <div className="absolute top-1.5 right-1.5 flex items-center gap-1">
                    {used > 0 && <span className="text-[9px] font-black text-white bg-amber-500 rounded-full px-1.5 py-0.5">{used}</span>}
                    <button onClick={e => { e.stopPropagation(); handleDeleteSpot(s); }} disabled={deleting === s.id}
                  ...
                  <p className="font-black text-[#0d1117] text-sm leading-tight break-words w-full line-clamp-2">{s.name}</p>
                  ...
                  {urls[s.id]
                    ? <Image src={urls[s.id]} alt={`QR ${s.name}`} width={200} height={200} className="mx-auto w-full h-auto" unoptimized />
        ...
        <div className="fixed inset-0 z-[200] flex flex-col bg-[#0d1117]">
          <div className="flex items-center justify-between px-4 py-2.5 bg-[#161b27] border-b border-[#21293d] flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold text-slate-500 hidden sm:inline">
                {spots.length} labels · {totalPages} page{totalPages > 1 ? "s" : ""} · {gridCols}×{gridRows} · {labelsPerPage}/page
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="flex items-center gap-1 bg-[#111520] border border-[#21293d] rounded-lg px-1 py-0.5">
                <button onClick={zoomOut} className="p-1 text-slate-500 hover:text-white transition-colors rounded"><ZoomOut size={14} /></button>
                <button onClick={zoomFit} className="px-1.5 py-0.5 text-[10px] font-bold text-slate-400 hover:text-white transition-colors tabular-nums min-w-[40px] text-center">{zoom}%</button>
                <button onClick={zoomIn} className="p-1 text-slate-500 hover:text-white transition-colors rounded"><ZoomIn size={14} /></button>
              </div>
              <div className="w-px h-5 bg-[#21293d] mx-1" />
              <button onClick={handlePopupPrint} className="!text-white border-none rounded-lg px-4 py-2 font-bold text-xs cursor-pointer hover:opacity-90 flex items-center gap-1.5 bg-blue-600">
                <Printer size={13} /> Print
              </button>
              <button onClick={() => setPreviewOpen(false)} className="text-slate-400 hover:text-white p-2 rounded-lg hover:bg-white/5 transition-all">
                <X size={18} />
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-56 flex-shrink-0 bg-[#111520] border-r border-[#21293d] overflow-y-auto p-3 space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 block">Paper</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 block">Layout</label>
                <div className="flex gap-1">
                  {(["portrait", "landscape"] as Orientation[]).map(o => (
                    <SettingBtn key={o} active={orientation === o} onClick={() => setOrientation(o)}>
                      <RotateCw size={10} className={`inline mr-1 ${o === "landscape" ? "" : "-rotate-90"}`} />{o}
                    </SettingBtn>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1 mb-2">
                  {PRESET_GRIDS.map(g => (
                    <button key={g.label} onClick={() => { setGridCols(g.cols); setGridRows(g.rows); }}
                ...
                <div className="flex gap-1 items-center">
                      ...
                      {[2,3,4,5,6].map(c => (
                        <button key={c} onClick={() => setGridCols(c)}
                          className={`flex-1 py-1 rounded text-[10px] font-bold border transition-all ${
                            gridCols === c ? "bg-blue-600/20 border-blue-500/40 text-blue-400" : "bg-transparent border-[#21293d] text-slate-500 hover:text-slate-300"
                          }`}>{c}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-0.5">
                      {[2,3,4,5,6].map(r => (
                        <button key={r} onClick={() => setGridRows(r)}
                          className={`flex-1 py-1 rounded text-[10px] font-bold border transition-all ${
                            gridRows === r ? "bg-blue-600/20 border-blue-500/40 text-blue-400" : "bg-transparent border-[#21293d] text-slate-500 hover:text-slate-300"
                          }`}>{r}</button>
                      ))}
                    </div>
                  </div>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 block">Gap</label>
                <div className="flex gap-1">
              ...
              <div className="rounded-xl bg-[#161b27] border border-[#21293d] p-3 space-y-1.5 text-[11px]">
                <div className="flex justify-between"><span className="text-slate-500">Paper</span><span className="text-white font-bold">{paperSize.toUpperCase()} {orientation}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Grid</span><span className="text-blue-400 font-black">{gridCols} × {gridRows}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Per Page</span><span className="text-white font-bold">{labelsPerPage}</span></div>
                <div className="h-px bg-[#21293d]" />
                <div className="flex justify-between"><span className="text-slate-500">Total Pages</span><span className="text-blue-400 font-black text-sm">{totalPages}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Labels</span><span className="text-white font-bold">{spots.length}</span></div>
              </div>

              <p className="text-[9px] text-slate-600 leading-relaxed">Ctrl+P se print. Escape se close.</p>
            </div>

            {/* ── PREVIEW AREA ── */}
            <div ref={previewScrollRef} className="flex-1 overflow-auto bg-[#080c14] p-6">
              <div className="space-y-10" style={{ transform: `scale(${zoom / 100})`, transformOrigin: "top center" }}>
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        {pi > 0 && (
                          <button onClick={() => {
                            ...
                            if (!el) return;
                            el.querySelector(`[data-page="${pi - 1}"]`)?.scrollIntoView({ behavior: "smooth", block: "start" });
                          }} className="text-slate-600 hover:text-white transition-colors p-1 rounded"><ChevronLeft size={14} /></button>
                        )}
                        ...
                        {pi < totalPages - 1 && (
                          <button onClick={() => {
                            ...
                            if (!el) return;
                            el.querySelector(`[data-page="${pi + 1}"]`)?.scrollIntoView({ behavior: "smooth", block: "start" });
                          }} className="text-slate-600 hover:text-white transition-colors p-1 rounded"><ChevronRight size={14} /></button>
                        )}
                      </div>

                      <div data-page={pi} className="bg-white rounded-lg shadow-2xl overflow-hidden"
                        ...
                        <div className="w-full h-full" style={{
                          ...
                          {Array.from({ length: Math.max(0, labelsPerPage - pageSpots.length) }).map((_, ei) => (
                            <div key={`empty-${ei}`} className="rounded-lg" style={{ border: "1px dashed #e5e7eb" }} />
                          ))}
                        </div>
                      </div>
                    </div>
