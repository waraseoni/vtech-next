...
export function ClientFilterBar({ searchTerm, setSearchTerm, showFilter, setShowFilter, minBal, setMinBal, maxBal, setMaxBal, tabFilter, setTabFilter, sortField, toggleSort, sortDir, filteredSortedClients, clients, TABS }: any) {
  return (
    <div className="bg-[#161b27] border border-[#21293d] rounded-2xl p-4 space-y-3">
      ...
      {showFilter&&(
        <div className="flex flex-wrap gap-3 items-center pt-3 border-t border-[#21293d]">
          ...
          {(["balance","name","total_paid","date_created"] as any[]).map(f=>(
