...
export default function ClientsBody({
  ...
  const [waMsgType, setWaMsgType] = useState<
  ...
  const [bulkWaMsgType, setBulkWaMsgType] = useState<
    "welcome" | "reminder" | "followup" | "offer" | "greeting" | "custom"
  >("custom");
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, minBal, maxBal, tabFilter, sortField, sortDir]);
  ...
  const handleWaTypeChange = (type: typeof waMsgType) => {
    if (!waClient || type === "custom") return;
    setWaMsgType(type);
    const tpl = resolveTemplate(firmInfo, WA_TEMPLATE_KEY[type]);
    setWaText(
      substituteTemplate(tpl, {
        client_name: waClient.name,
        balance: inr(waClient.balance),
        ...firmVars(firmInfo),
      })
    );
  };
  ...
  const sendBulkWhatsApp = () => {
    ...
    selected.forEach((c) => {
      if (c.contact) {
        window.open(
          `https://wa.me/91${c.contact.replace(/\D/g, "")}?text=${encodeURIComponent(bulkWaText)}`,
          "_blank"
        );
      }
    });
  ...
  const filteredSortedClients = useMemo(() => {
    const s = searchTerm.toLowerCase().trim(),
      lo = minBal !== "" ? parseFloat(minBal) : -Infinity,
      hi = maxBal !== "" ? parseFloat(maxBal) : Infinity;
  ...
  const followupCount = useMemo(
    () => clients.filter((c) => c.balance <= 0 && daysSince(c.last_txn_date) > 30).length,
    [clients]
  );
  ...
  const TABS = [
    {
      ...
      ac: "text-blue-400 border-blue-500 bg-blue-500/10",
    },
    {
      id: "due" as TabFilter,
      label: "Due",
      count: clientsWithDue,
      ac: "text-red-400 border-red-500 bg-red-500/10",
    },
    {
      id: "high" as TabFilter,
      label: "High Risk",
      count: highRiskCount,
      ac: "text-orange-400 border-orange-500 bg-orange-500/10",
    },
    {
      id: "clear" as TabFilter,
      label: "Clear",
      count: totalCleared,
      ac: "text-emerald-400 border-emerald-500 bg-emerald-500/10",
    },
    {
      id: "followup" as TabFilter,
      label: "Follow-up",
      count: followupCount,
      ac: "text-teal-400 border-teal-500 bg-teal-500/10",
    },
  ];

  // Hydration-safe: SSR par mobile-detection unavailable hai, isliye pehle render
  // desktop table dikhata hai (isMobile=false). Hydration ke baad asli width set
  // hoti hai. Agar hydrated isMobile=true hai to mobile cards render honge.
  const effectiveMobile = isHydrated ? isMobile : false;
                ...
                >
                  <div className="p-4 space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={`text-[9px] font-extrabold px-2 py-1 rounded border ${meta.badge} flex-shrink-0`}
                      >
                        {meta.label}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-[#0d1117] rounded-xl px-4 py-3 border border-[#21293d]">
                      ...
                      <span
                        className={`text-lg font-black ${client.balance > 0 ? "text-red-400" : "text-emerald-400"}`}
                      >
                        {client.balance < 0 ? "−" : ""}
                        {inr(client.balance)}
                      </span>
                ...
                {paginatedClients.map((client, i) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                              <span
                                className={`text-[9px] font-extrabold px-2 py-0.5 rounded border ${meta.badge}`}
                              >
                                {meta.label}
                              </span>
                              {client.contact && (
                      ...
                      <td className="px-3 py-3.5 align-middle">
                        <div
                          className={`text-[15px] font-black leading-none ${client.balance > 0 ? "text-red-400" : "text-emerald-400"}`}
                        >
                          {client.balance < 0 ? "−" : ""}
                          {inr(client.balance)}
                        </div>
                        {client.balance > 0 ? (
                          <div className="mt-2 h-1.5 bg-[#1e2637] rounded-full overflow-hidden">
                            <div
                              className="h-full bg-gradient-to-r from-red-500 to-orange-400 rounded-full"
                              style={{
                                width: `${Math.min(100, (client.balance / 100_000) * 100)}%`,
                              }}
                            />
                          </div>
                        ) : (
                ...
                <tr className="bg-[#111520] border-t border-[#21293d]">
                  ...
                  <td className="px-3 py-3 text-right">
                    <span className="text-sm font-black text-red-400">
                      {inr(
                        paginatedClients.reduce((s, c) => s + (c.balance > 0 ? c.balance : 0), 0)
                      )}
                    </span>
                  </td>
...
function ActionDropdown({
  ...
  const items = [
    ...
    },
    {
      ...
      label: "Add Payment",
      href: `/clients/${clientId}/add-payment`,
  ...
  return (
    <div className="bg-[#161b27] border border-[#21293d] rounded-2xl p-4 flex items-center gap-3.5 hover:-translate-y-0.5 transition-transform duration-200 overflow-hidden">
      ...
      <div className="min-w-0 flex-1 overflow-hidden">
        ...
        <div
          className={`${isAmount ? "text-base" : "text-2xl"} font-black text-white tracking-tight leading-none truncate`}
        >
          {value}
        </div>
      </div>
