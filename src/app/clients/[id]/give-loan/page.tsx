    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (cd) {
          ...
          const [{ data: txns }, { data: sales }, { data: pays }, { data: loans }] =
            await Promise.all([
              supabase
          ...
          let loanRepaid = 0;
          const repairBilled = (txns || []).reduce(
            (s: number, j: { amount: number }) => s + (j.amount || 0),
            0
          );
          const directBilled = (sales || []).reduce(
            (s: number, d: { total_amount: number }) => s + (d.total_amount || 0),
            0
          );
          let totalPaid = 0;
          ...
          const loanGiven = (loans || []).reduce(
            (s: number, l: { total_payable: number }) => s + (l.total_payable || 0),
            0
          );
          setBalance(
            cd.opening_balance + repairBilled + directBilled - totalPaid + loanGiven - loanRepaid
          );
        }
      } catch (err) {
        ...
        {balance !== null && (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p
                className={`text-xl font-black mt-0.5 ${balance > 0 ? "text-red-400" : "text-emerald-400"}`}
              >
                {inr(balance)}
              </p>
            </div>
