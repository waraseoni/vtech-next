...
import { ArrowLeft, Edit3, Smartphone, History, Wallet, Phone, MapPin, Loader2 } from 'lucide-react';
...
export default function ViewClientProfile({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '25px' }}>
        ...
        <div style={profileCard}>
          ...
          <div style={{ ...infoBox, background: client.balance > 0 ? '#fff5f5' : '#f6fff9', padding: '15px', borderRadius: '10px', marginTop: '20px' }}>
            <Wallet size={18} color={client.balance > 0 ? '#dc3545' : '#28a745'}/> 
            <div>
              <small style={{ color: '#666', display: 'block' }}>Current Balance</small>
              <div style={{ fontSize: '20px', fontWeight: 'bold', color: client.balance > 0 ? '#dc3545' : '#28a745' }}>₹{client.balance || 0}</div>
            </div>
          </div>
        </div>
    ...
    <div style={{ textAlign: 'right' }}>
      {/* Delivered hone par amount Green aur Bold dikhega */}
      <div style={{ 
        fontWeight: 'bold', 
        fontSize: '16px', 
