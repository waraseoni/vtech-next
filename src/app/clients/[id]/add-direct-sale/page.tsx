  ...
  useEffect(() => {
    const fetchClient = async () => {
      ...
      const { data } = await supabase
        ...
        .single();
      if (data) {
        setClientName([data.firstname, data.middlename, data.lastname].filter(Boolean).join(" "));
      }
    };
    fetchClient();
  }, [clientId]);
          ...
          <div className="bg-[#161b27] rounded-2xl border border-[#21293d] p-5">
            ...
            <div className="space-y-2">
              {items.map((item, i) => (
                <div
                  key={i}
                  className="grid grid-cols-1 sm:grid-cols-[1fr_80px_110px_44px] gap-2 items-start"
                >
                  <input
                    type="text"
