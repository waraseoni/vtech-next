...
export default function ViewClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const totalPaid = payments
    .filter((p) => !p.loan_id)
    .reduce((s, p) => s + toNum(p.amount) + toNum(p.discount), 0);
  ...
  const loanRepaid = payments
    .filter((p) => p.loan_id && activeLoanIds.has(p.loan_id))
    .reduce((s, p) => s + toNum(p.amount) + toNum(p.discount), 0);
  const loanBalance = loanGiven - loanRepaid;
  const netBalance =
    (toNum(client?.opening_balance) || 0) + totalBilled - totalPaid + loanGiven - loanRepaid;
  const monthlyEmi = activeLoans.reduce((s, l) => {
    const paidL = payments
      .filter((pay) => pay.loan_id === l.id)
      .reduce((acc, pay) => acc + toNum(pay.amount) + toNum(pay.discount), 0);
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Link
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold no-underline transition-all"
            >
              <Edit3 size={13} /> Edit
            </Link>
            {userRole === "admin" && (
              <button
                onClick={() => {
                  if (confirm(`"${clientName}" ko delete karein?`)) router.replace("/clients");
                }}
                className="flex items-center gap-1.5 px-3 py-2 bg-red-600/20 border border-red-500/30 hover:bg-red-600 text-red-400 rounded-xl text-xs font-bold transition-all"
              >
          ...
          <div className="lg:col-span-4 theme-card border border-[#21293d] rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition-all"
              >
                <Plus size={13} /> Payment
              </button>
              <button
                onClick={() => setShowLoanModal(true)}
                className="flex items-center justify-center gap-1.5 px-3 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-bold transition-all"
              >
                <Wallet size={13} /> Give Loan
              </button>
              <button
                onClick={() => {
                  setSelectedLoanId(loans.find((l) => l.status === 1)?.id || null);
                  setShowEmiModal(true);
                }}
                className="flex items-center justify-center gap-1.5 px-3 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-bold transition-all"
              >
                <CreditCard size={13} /> Collect EMI
              </button>
              <Link
                href={`/jobs/new?client_id=${clientId}`}
                className="flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold no-underline transition-all"
              >
            ...
            {phone && (
              <div className="flex gap-2 mt-3">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 bg-[#25D366]/15 hover:bg-[#25D366]/25 border border-[#25D366]/30 text-[#4ade80] rounded-xl text-xs font-bold transition-all"
                >
                  <MessageCircle size={14} /> WhatsApp
                </button>
                <button
                  onClick={() => window.open(`sms:${phone}`, "_blank")}
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 bg-[#6366f1]/15 hover:bg-[#6366f1]/25 border border-[#6366f1]/30 text-indigo-400 rounded-xl text-xs font-bold transition-all"
                >
          ...
          <div className="lg:col-span-8 space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              >
                ...
                <p
                  className={`text-lg font-black ${netBalance > 0 ? "text-red-400" : "text-emerald-400"}`}
                >
                  {inr(netBalance)}
                </p>
              </div>
            ...
            {loanGiven > 0 && (
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="theme-card border border-[#21293d] rounded-xl p-4 text-center">
                  ...
                  <p
                    className={`text-lg font-black ${loanBalance > 0 ? "text-red-400" : "text-emerald-400"}`}
                  >
                    {inr(loanBalance)}
                  </p>
                </div>
                      ...
                      filteredRepairs.map((r) => (
                        <tr key={r.id} className="hover:bg-white/[0.02]">
                          ...
                          <td className="px-3 py-2.5 text-center">
                            <Link
                              href={`/jobs/${r.id}/view`}
                              className="text-blue-400 hover:text-blue-300 text-xs font-bold flex items-center justify-center gap-1"
                            >
                              <Eye size={12} /> View
                            </Link>
                          </td>
                    ...
                    {payments.length === 0 ? (
                      ...
                      payments.map((p) => (
                        <tr key={p.id} className="hover:bg-white/[0.02]">
                          ...
                          <td className="px-3 py-2.5">
                            {p.loan_id ? (
                              <span className="px-2 py-0.5 bg-purple-500/15 border border-purple-500/30 text-purple-400 rounded text-[10px] font-bold">
                                EMI
                              </span>
                            ) : (
                ...
                <div className="theme-panel-2 border border-[#21293d] rounded-xl p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-slate-500">
                    ...
                    <span className="font-bold text-amber-400">
                      {inr(
                        (parseFloat(loanAmount || "0") * (1 + parseFloat(loanRate || "0") / 100)) /
