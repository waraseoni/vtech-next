...
import {
  Images, RefreshCw, Trash2, Download, Copy, Check,
  Loader2, AlertCircle, CheckCircle, Link2,
} from "lucide-react";

type BucketFile = { name: string; url: string; size: number; created_at: string; referenced: boolean; refs: { label: string; href: string | null }[] };
type BucketData = { bucket: string; label: string; total: number; orphanCount: number; files: BucketFile[] };
...
export default function ImagesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 403) { setError("Sirf Admin is page ko dekh sakta hai"); return; }
  ...
  const visibleBuckets = activeBucket === "all" ? buckets : buckets.filter(b => b.bucket === activeBucket);
  const visibleFiles = visibleBuckets
    .flatMap(b => b.files.map(f => ({ ...f, bucket: b.bucket })))
    .filter(f => statusFilter === "all" ? true : statusFilter === "orphan" ? !f.referenced : f.referenced);
  ...
  const selectOrphans = () => {
    ...
    visibleBuckets.forEach(b => b.files.forEach(f => { if (!f.referenced) s.add(key(b.bucket, f.name)); }));
    setSelected(s);
  };

  const deleteFiles = async (items: { bucket: string; name: string }[]) => {
    if (items.length === 0) return;
    if (!confirm(`${items.length} image(s) permanently delete karni hain? Ye storage se hamesha ke liye hat jayengi — pahle download kar lena.`)) return;
    setDeleting(true);
    try {
      let removed = 0;
    ...
    <div className="min-h-screen bg-[#0d1117] font-sans pb-12">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6 space-y-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="text-right mr-1">
                <p className="text-[10px] text-slate-500 font-bold">{totalFiles.toLocaleString()} images · <span className="text-amber-400">{totalOrphans} orphan</span></p>
              </div>
        ...
        <div className="bg-[#161b27] border border-[#21293d] rounded-2xl px-5 py-3 flex items-center flex-wrap gap-2">
          ...
          {buckets.map(b => (
            <button key={b.bucket} onClick={() => setActiveBucket(b.bucket)}
            ...
            {(["all", "referenced", "orphan"] as const).map(st => (
              <button key={st} onClick={() => setStatusFilter(st)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide transition ${
                  statusFilter === st
                    ? st === "orphan" ? "bg-amber-500/20 border border-amber-500/40 text-amber-400"
        ...
        <div className="bg-[#161b27] border border-[#21293d] rounded-2xl px-5 py-2.5 flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => deleteFiles([...selected].map(k => {
        ...
        {/* Error / Loading / Grid */}
        {error ? (
          <div className="bg-red-500/10 border border-red-500/30 rounded-2xl px-5 py-6 text-red-400 text-sm font-bold text-center">{error}</div>
        ) : loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {visibleFiles.map(f => {
              ...
              return (
                <div key={k} className={`bg-[#161b27] border rounded-2xl overflow-hidden flex flex-col ${checked ? "border-blue-500/60" : f.referenced ? "border-[#21293d]" : "border-amber-500/40"}`}>
                  {/* Thumb */}
                  <div className="relative aspect-square bg-[#0d1117] flex items-center justify-center overflow-hidden">
                    ...
                    <Image src={f.url} alt={f.name} fill unoptimized className="w-full h-full object-contain p-1 cursor-zoom-in" sizes="200px" onDoubleClick={() => openImageLightbox(f.url, f.name)} />
                    <input
                      type="checkbox"
                      checked={checked}
                  ...
                  <div className="p-2.5 flex flex-col gap-1 flex-1">
                    <p className="text-[10px] font-mono text-slate-400 truncate" title={f.name}>{f.name}</p>
                    <div className="flex items-center justify-between gap-1">
                      <span className={`text-[10px] font-bold ${f.referenced ? "text-emerald-400" : "text-amber-400"}`}>
                        {f.referenced ? "✓ Connected" : "✗ Orphan"}
                      </span>
                      <span className="text-[9px] text-slate-600">{fmtSize(f.size)} · {fmtDate(f.created_at)}</span>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {f.refs.slice(0, 2).map((r, i) => (
                          r.href ? (
                            ...
                            <span key={i} className="text-[9px] font-bold bg-blue-500/10 border border-blue-500/30 text-blue-300 px-1.5 py-0.5 rounded-md truncate max-w-full">
                              {r.label}
                            </span>
                          )
                        ))}
                        {f.refs.length > 2 && (
                          <span className="text-[9px] text-slate-600 font-bold">+{f.refs.length - 2}</span>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-1 bg-[#0d1117] border border-[#21293d] rounded-lg px-2 py-1">
                      ...
                      <span className="text-[9px] font-mono text-blue-300 truncate flex-1" title={f.url}>{f.url}</span>
                      <button onClick={() => copy(k, f.url)} title="Copy link"
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      <button onClick={() => deleteFiles([{ bucket: f.bucket, name: f.name }])} disabled={deleting}
