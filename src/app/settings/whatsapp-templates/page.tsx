...
export default function WhatsAppTemplatesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    (infoRes.data || []).forEach((r) => {
      infoMap[r.meta_field] = r.meta_value;
    });
  ...
  const upsertMeta = async (metaField: string, metaValue: string): Promise<string | null> => {
    ...
    if (existing) {
      ...
      const { error: insErr } = await supabase
        .from("system_info")
        .insert({ meta_field: metaField, meta_value: metaValue });
  ...
  const restoreFactory = async () => {
    if (
      !confirm("Restore ORIGINAL factory defaults? This will overwrite all your custom defaults.")
    )
      return;
    setSaving(true);
    try {
                    ...
                    {history.map((h, i) => (
                      <tr key={h.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-4 py-3 font-bold text-white">
                          {TEMPLATE_LABELS[h.template_key] || h.template_key}
                        </td>
