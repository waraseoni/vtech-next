...
export default function SellerPage() {
  ...
  const remove = async (l: License) => {
    if (
      !confirm(
        `License delete karein? (${l.shop_name || l.license_key})\nActivations bhi hat jayenge.`
      )
    )
      return;
  ...
  const activeCount = licenses.filter(
    (l) =>
      l.status === "active" && (daysLeft(l.expires_at) === null || daysLeft(l.expires_at)! >= 0)
  ).length;
  const expiredCount = licenses.filter(
    (l) => l.status === "active" && daysLeft(l.expires_at) !== null && daysLeft(l.expires_at)! < 0
  ).length;
                  ...
                  {licenses.map((l) => {
                    ...
                    return (
                      <tr key={l.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider border ${statusBadge(l.status)}`}
                          >
                            {l.status}
                          </span>
                        </td>
      ...
      <div className="w-full max-w-lg bg-[#111520] border border-[#21293d] rounded-3xl p-6 shadow-2xl shadow-black/50 max-h-[90vh] overflow-y-auto">
        ...
        {createdKey ? (
          <div className="text-center py-6">
            ...
            <button
              onClick={onCreated}
              className="mt-5 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-black transition-all"
            >
              Done
            </button>
          </div>
          ...
          <form onSubmit={submit} className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Sharma Mobile Repair"
                required
              />
            </div>
            ...
            >
              {busy ? (
                ...
                <>
                  <KeyRound size={15} /> Create License + Generate Key
                </>
              )}
            </button>
          </form>
