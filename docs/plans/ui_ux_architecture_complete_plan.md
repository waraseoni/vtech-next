...
| 2 | Semantic tokens migration + `!important` delete (codemod) | 3 | 🔶 **Foundation done (2026-09-23)** — `@theme inline` me `--color-app-*` + short aliases (`bg-panel`, `text-muted` style utilities available) + helper classes `.card`/`.muted`/`.text-app*`/`.bg-app`/`.bg-panel`. **Pending:** codemod pages (hardcoded hex → tokens, top 20 patterns) + delete ~413 `!important` override rules (311 in L190–1800 override blocks). Override layer ABHI VALID hai (pages still depend on it) — sirf tab delete jab tak pages migrate na ho. |
...
| 9 | Keyboard shortcuts: g-prefix + `?` help + skip-to-content | 1 |
| 10 | Image compression on ALL uploads (messenger pattern reuse) | 1 |
