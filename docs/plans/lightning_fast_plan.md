...
| **GATE-C** | tsc+eslint+vitest + analyze:output + **Lighthouse 6-route vs baseline** | ✅ | PASS 2026-09-25: tsc 0 · eslint 0 (11 touched files) · vitest **131/131** · fresh `npm run build` OK. **CORRECTION (authed runs, same method as baseline — staff session, attached Chrome, LH 13.5.0):** clean-headless `/login` 90-94 **env artifact tha** (no session = halka shell path), real authed numbers neeche. **Chunks**: total 151→184 files, 4.81→5.77MB (FEATURE growth since Aug 28). `/login` initial JS 1019.5→**1047.9KB**. **Authed LH (baseline → ab):** dashboard 28→24 · LCP 9.64→**7.4s** · TBT 1865→**1170ms** · CLS 0.248→0.375 | clients 42→45 · LCP 10.13→**5.3s** · TBT 2096→2110 · CLS 0 | jobs 41→44 · LCP 10.23→**7.5s** · TBT 2668→3150 · CLS 0.001 | cash-flow 48→53 · LCP 5.63→**5.1s** · TBT 1730→1730 | monthly-profit 45→45 · LCP 5.88→**5.3s** · **TBT 980→7270ms REGRESSION (2 runs reproduced)** · TTI 9.48→11.9s | login 31→26 · LCP 9.72→**8.3s** · TBT 1527→1450 · CLS 0.248→0.375. **Verdict:** LCP har route par −9–48% (targets <4s/<5s NOT met — data+render bound, Phase D territory) · TBT mixed (single-run noise + data growth) · **monthly-profit root cause: react-dom chunk scripting 8055ms = giant day-grid tree hydration (styleLayout 2840ms), source me 87 map/td/day matches — follow-up: grid summarize/virtualize (is plan se bahar, naya item)**. Temp profiles + server cleaned. |
