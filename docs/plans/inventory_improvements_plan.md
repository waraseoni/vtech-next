...
- **Data-model cleanup (place\* → product_locations/locations) — PHASE-1 ✅ DONE (2026-09-20), Phase-2 ✅ DONE (2026-09-25):**
  ...
  - **✅ Phase-2 EXECUTED 2026-09-25 (user ran SQL in Dashboard, post-verify by session):** DROP set (best-practice deviation documented): 8 structured cols DROP (`inventory_list` + `product_list` × zone/rack/bin/box), `inventory_list.place` KEPT (10+ readers + RPC), trigger + function dropped. Post-verify LIVE: 8/8 cols gone · `place` intact · product_locations 43 rows intact · RPC alive · INSERT+DELETE test row (net zero, id=150) = write path OK. Gates: tsc 0 · vitest 131/131. Targeted backup JSONs: `C:\Windows\Temp\opencode\pgwork\backup_*_2026*` (126 inv + 97 prd rows, NOT committed — operational data).
  - **Re-evaluation evidence (2026-09-25, live DB read-only check, service_role REST):** active products **97**, `product_locations` rows **43** → **54 unmapped**. Unmapped ke **55 inventory rows me legacy place data = 0** (place/zone/rack/bin/box sab empty) + `product_list.place_*` bhi empty → **DROP se zero data loss** (checklist (a) substance me PASS — unmapped-with-data = 0). Read-point scope (c): sirf **3 files** — `lib/locations.ts` (canonical `partsFromRow`), `lib/stockValuation.ts` (helper use karta hai + kuch direct refs), `app/backup/page.tsx` export lists (:169-172, :260-263). Baaki (b) backup + (d) derivation decision + (e) DROP = **dedicated window + aapka confirm** chahiye — schema change bina window ke nahi.
  - **Adversarial re-verify (2026-09-25, user ke "inventory me use ho raha hai" sawal par — RUN se pehle):** (1) src me koi SELECT with place_* NAHI bacha (sirf optional TS types + `??` fallbacks — undefined-safe); inventory pages ke selects me place_* kabhi tha hi nahi. (2) Writers of place_* (app + SQL): NONE — stocktake/receive sirf `place` likhte hain, INSERT lists explicit (bina place_*). (3) DB objects: timestamp/cost triggers me place_* nahi; views/RLS me place nahi; 20260920 ke baad koi migration place_* chhooti nahi. (4) UI me jo location dikhta hai wo `place` (KEEP) + product_locations (KEEP) se hai — dropped cols invisible duplicates hain. (5) `place` KEEP + RPC unchanged + trigger DROP zaroori (warna har write fail). **Verdict: RUN SAFE** — shart: Vercel par 801fe7d deployed ho + SQL Editor me file chalane ke baad verify query (sirf `place` bache).
