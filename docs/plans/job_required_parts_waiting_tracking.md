...
- Change `transaction_products.product_id` to **nullable** (legacy rows untouched). ⚠️ **README (42P16 fix):** `product_id` composite PK `transaction_products_pkey (transaction_id, product_id)` ka hissa hai — PK-column ko seedha `drop not null` nahi kar sakte. Solution: composite PK → surrogate `id bigint identity` PK par shift; `product_id` nullable; partial unique index `(transaction_id, product_id) WHERE product_id IS NOT NULL` purani uniqueness bachata hai. Name `transaction_products_pkey` nahi badla (20260913000000_final_full_schema_idempotent.sql ise `conname` guard se re-add karta hai). Side-effect: PostgREST nested join ab is table par ban jata hai (pehle composite-PK ki wajah se fail hota).
...
### 4.2 Alter legacy (42P16 fix — product_id PK-column hai)
...
-- PK = surrogate id (conname guard pagarna idempotent schema ke saath compatible)
alter table only public.transaction_products add constraint transaction_products_pkey primary key (id);
-- Purani (transaction_id, product_id) uniqueness — sirf inventory items ke liye:
create unique index if not exists transaction_products_inventory_uniq
  on public.transaction_products (transaction_id, product_id) where product_id is not null;
```
