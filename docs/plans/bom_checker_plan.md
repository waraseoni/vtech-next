...
### Phase 3 — Saved BOM Templates ✅ DONE (2026-09-16)

**Scope:** Save/load reusable BOM lists

**Files created:**
- `src/app/api/bom-templates/route.ts` — GET (list, with items) + POST (create)
- `src/app/api/bom-templates/[id]/route.ts` — GET (single) + PUT (update) + DELETE
- `supabase/migrations/20260922_bom_templates.sql` + fold-in to consolidated full schema
- `src/lib/bomTemplates.ts` — `BomTemplateItem`/`BomTemplate` types + `itemsToLines()`

**Client (`src/app/inventory/bom-check/page.tsx`):**
- "Save as Template" button (textarea ke neeche) → modal: name + description
- Save items = current parsed lines + best-match `product_id` (catalog se)
- Saved Templates list (input panel me): Load (→ textarea as `name - qty` lines) / Edit (modal, PUT) / Delete (confirm)
- Hooks har render me consistent (guard early-return se pehle)
- Auth: `/api/bom-templates*` via `requireStaffWithRole()` (cookie+RLS, **no service role**)
...
CREATE POLICY rlslock_bom_templates_staff ON bom_templates
  FOR ALL TO authenticated USING (is_frontend_staff()) WITH CHECK (is_frontend_staff());
CREATE INDEX bom_templates_name_idx ON bom_templates (name);
CREATE INDEX bom_templates_creator_idx ON bom_templates (created_by);
```

**What it does:**
1. "Save as Template" button → modal with name + description
2. Saved to `bom_templates` with items JSON
3. "Load Template" list → populates BOM input textarea
...
### Phase 3 ✅ DONE (2026-09-16)
- [x] Save BOM → listed in Saved Templates (persisted `bom_templates`, survives refresh)
- [x] Load template → textarea populated with saved component lines
- [x] Edit template (name/description) via modal → PUT
