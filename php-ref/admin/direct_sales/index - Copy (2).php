...
<style>
    ...
    .sale-code-link:hover {
        color: #0056b3;
        text-decoration: underline;
    }
    .sale-code-link:active {
        color: #004085;
    }
    
    /* Mobile card sale code link */
    .sale-card .sale-code {
        cursor: pointer;
        transition: all 0.2s;
    }
    .sale-card .sale-code:hover {
        color: #0056b3;
    }
    ...
    /* Client avatar for mobile */
    .client-avatar-mobile {
        flex-shrink: 0;
    }
    
    /* Desktop table client photo */
    .client-photo-desktop {
        width: 50px;
        height: 50px;
        object-fit: cover;
        border-radius: 50%;
    ...
    @media print {
        ...
        .dropdown-menu,
        .whatsapp-badge-mobile,
        .badge.badge-success .fa-whatsapp {
            display: none !important;
        }
            ...
            <div class="table-responsive">
                <table class="table table-hover table-striped table-bordered" id="sales-table">
                    ...
                    <tbody>
                        <?php 
$i = 1;
// 1. Summary Query (Alag rakhein)
$summary_qry = $conn->query("SELECT COUNT(*) as total_sales, SUM(total_amount) as total_amount, AVG(total_amount) as avg_amount FROM direct_sales");
$summary = $summary_qry->fetch_assoc();

// 2. Main Sales Query with Last Editor Information
$sales_sql = "SELECT ds.*, 
...
// Check karein agar query fail hui ho
if(!$sales){
            ...
            <div class="card-view">
                ...
                <div id="salesCardsContainer">
                    ...
                    while($row = $sales->fetch_assoc()):
                        ...
                        // Last edited information for mobile
                        $last_edited_mobile = '';
                        if(!empty($row['last_editor_name'])) {
...
$(document).ready(function(){
    ...
    // Mobile card click event - sale code click
    $(document).on('click', '.sale-card .sale-code', function(e) {
        ...
        if (saleId) {
            window.location.href = './?page=direct_sales/view_sale&id=' + saleId;
        }
    });

    // Mobile card whole card click (except action buttons)
    ...
    function exportSalesToExcel() {
        ...
        var generatedDate = "<?php echo date('Y-m-d H:i:s'); ?>";
        var totalSales = <?= $summary['total_sales'] ?? 0 ?>;
        var totalAmount = "<?= number_format($summary['total_amount'] ?? 0, 2) ?>";
