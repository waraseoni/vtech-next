                ...
                <div class="row align-items-end mb-3">
                    <div class="col-md-8">
                        ...
                        <select id="product_sel" class="form-control select2" data-placeholder="Select Product Here">
                            ...
                            <?php 
                            // Same logic as manage_transaction.php - show all products with current available stock
                            $products = $conn->query("
...
$(function(){
    ...
    $('#add_product').click(function(){
        ...
        <tr>
            ...
            <td><input type="number" name="qty[]" class="form-control text-center" value="1" min="1" max="${stock}" required></td>
    ...
    // Qty or Price Change → Recalculate + Stock Validation
    $(document).on('input change', '[name="qty[]"]', function(){
        var tr = $(this).closest('tr');
        var qty = parseFloat($(this).val()) || 0;
        var maxStock = parseFloat($(this).attr('max')) || 999999;

        if(qty > maxStock){
            alert_toast("Available stock se zyada quantity nahi daal sakte! Max: " + maxStock, "warning");
            $(this).val(maxStock);
            qty = maxStock;
        }
    ...
    $('#sale-form').submit(function(e){
        ...
        $('#item-list tbody tr').each(function(){
            ...
            var max = parseFloat($(this).find('[name="qty[]"]').attr('max')) || 0;
            if(qty > max){
                stockError = true;
                return false;
            }
        ...
        if(stockError){
            alert_toast("Kisi product ki quantity available stock se zyada hai!", "error");
            return;
        }
