...
$summary_qry = $conn->query("SELECT COUNT(*) as total_sales, SUM(total_amount) as total_amount FROM direct_sales");
$summary = $summary_qry->fetch_assoc();

// 2. Main Sales Query (Fix with Staff Name)
$sales_sql = "SELECT ds.*, 
...
?>
                        <tr>
                            ...
                            <td><strong><?= $row['sale_code'] ?></strong>
                                <br><small>Sold by - <?= $row['staff_name'] ?: '<span class="text-muted">Admin</span>' ?></small>
                            </td>
                            <td>
