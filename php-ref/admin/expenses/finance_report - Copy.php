                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="salaryTable">
                        ...
                        <tbody>
                            ...
                            $salary_qry = $conn->query("SELECT a.*, CONCAT(m.firstname,' ',m.lastname) as name, m.status FROM advance_payments a INNER JOIN mechanic_list m ON a.mechanic_id = m.id WHERE {$salary_where} ORDER BY a.date_paid DESC");
                            $total_salary = 0;
                            ...
                            <tr>
                                ...
                                <td>
                                    <b class="<?php echo $staff_status_class ?>">
                                        ...
                                        <?php if($row['status'] == 0): ?>
                                            <small class="badge badge-secondary ml-1">Inactive</small>
                                        <?php endif; ?>
                                    </b>
                                </td>
...
$(function(){
    ...
    function toggleFilters(activeTab) {
        if(activeTab === 'salary') {
            ...
            $('select[name="category"]').val('all'); // Reset expense filter
        } else {
            $('#staff-filter-group').hide();
            $('#expense-filter-group').show();
            $('select[name="mechanic_id"]').val('all'); // Reset staff filter
        }
    }
    ...
    $('.edit_advance').click(function(){
        uni_modal("<i class='fa fa-edit'></i> Edit Advance", "salery/manage_advance.php?id=" + $(this).attr('data-id'))
    });
    ...
    $('.delete_advance').click(function(){
        if(!confirm("Are you sure you want to delete this advance record?")) return false;
    ...
    $('.delete_expense').click(function(){
        if(!confirm("Are you sure you want to delete this expense record?")) return false;
        var id = $(this).attr('data-id');
        $.ajax({
