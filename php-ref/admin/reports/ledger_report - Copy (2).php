...
// 2. DIRECT SALES INCOME (सीधी बिक्री से आय)
// Only walk-in customers (no client_id)
$sales_qry = $conn->query("SELECT SUM(total_amount) as total_amount 
                           FROM direct_sales 
                           WHERE (client_id IS NULL OR client_id = 0 OR client_id = '')
                           AND date_created BETWEEN '{$from} 00:00:00' AND '{$to} 23:59:59'");
$sales_data = $sales_qry->fetch_assoc();
$sales_total = $sales_data['total_amount'] ?? 0;
$direct_income = $sales_total;

// Direct sales ka detailed data
$direct_sales_detail = $conn->query("SELECT 
    ds.*,
...
// 3. CLIENT PAYMENTS RECEIVED (ग्राहकों से प्राप्त भुगतान)
...
// 4. TOTAL INCOME (कुल आय)
$total_income = $job_income + $direct_income;

// 5. STAFF SALARY (कर्मचारी वेतन)
...
// 6. STAFF COMMISSION (कर्मचारी कमीशन)
...
// 7. ADVANCE PAYMENTS (अग्रिम भुगतान)
...
// Total Cash Inflow (कुल नकद आवक)
$total_cash_inflow = $client_payments_received + $direct_income;
...
<div class="row mt-4">
    <div class="col-md-6">
        ...
        <table class="table table-sm table-hover">
            ...
            <tr>
                <td>
                    <a href="javascript:void(0)" class="text-primary" onclick="showDirectSalesDetails()" data-toggle="tooltip" title="Click to view details">
                        <i class="fas fa-eye mr-1"></i> Walk-in Direct Sales
                    </a>
                </td>
                <td class="text-right">₹ <?= number_format($direct_income, 2) ?></td>
            </tr>
            <tr class="bg-light">
                <th>Net Revenue</th>
    ...
    <div class="col-md-6">
        ...
        <table class="table table-sm table-hover">
            ...
            <tr>
                ...
                <td class="text-right text-success">₹ <?= number_format($direct_income, 2) ?></td>
            </tr>
            <tr class="bg-light">
                <th>Total Cash In</th>
        ...
        <div class="alert alert-warning mt-3">
            ...
            <ul class="mb-0 pl-3">
                ...
                <li>Cash inflow shows net amount received after discount</li>
...
<div class="modal fade" id="directSalesModal" tabindex="-1" role="dialog" aria-labelledby="directSalesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="directSalesModalLabel">
                    <i class="fas fa-shopping-cart mr-2"></i> Direct Sales Details
                    <small class="text-light">(<?= date("d M Y", strtotime($from)) ?> to <?= date("d M Y", strtotime($to)) ?>)</small>
                </h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-sm">
                        ...
                        <tbody>
                            <?php 
                            $direct_sales_total = 0;
                            if($direct_sales_detail->num_rows > 0):
                                while($row = $direct_sales_detail->fetch_assoc()):
                                    $direct_sales_total += $row['total_amount'];
                            ?>
                            <tr>
                                <td><?= $row['sale_code'] ?></td>
                        ...
                        <tfoot class="bg-light">
                            <tr class="font-weight-bold">
                                ...
                                <td class="text-right text-success">₹ <?= number_format($direct_sales_total, 2) ?></td>
                                <td></td>
                            </tr>
                        </tfoot>
                        <?php else: ?>
                        <tr><td colspan="6" class="text-center text-muted">No direct sales found in this period.</td></tr>
                        <?php endif; ?>
                    </table>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-sm">
                        ...
                        <tbody>
                            ...
                            $cp_total = 0;
                            $cp_discount_total = 0;
                            if($client_payments_detail->num_rows > 0):
...
<?php 
// LEDGER QUERY - AB DISCOUNT BHI SHOW KARENGE
$running_balance = 0;
...
<script>
    ...
    function showDirectSalesDetails() {
        $('#directSalesModal').modal('show');
    }
